...
不要把所有实现细节画进去。图只保留解释当前小节所需的主链路、关键分支和一两个约束点，并在当前组件或模块的职责边界停止。涉及全局 JSX → IR → Scene 管线时，只画与当前职责相接的入口 / 出口，再从当前导航查找并链接全局架构或编译原理页，不硬编码历史路径，不重复绘制整条管线。
...
中高难度内容的多阶段处理需配合 Flow 展示具体实例，不能只有步骤名称与函数框。

- 选一个最小但完整的对象贯穿全图，画出初始、关键中间态与最终态；用箭头连接快照，标注实际操作、顺序和必要参数
- 保持对象身份、颜色与比较尺度一致；标明固定参照和随对象变化的部分，避免布局位移被误认为业务变换
- 跟踪一个代表性点、字段或元素，给出可核对的中间结果；图前交代输入，图后解释结果与 Flow 阶段的对应关系
- 快照优先由真实公开能力生成；派生数值使用现有计算能力或准确公式，不能凭目测画结果
- 几何变化用 Scope 与基础图元，数据变化用实际结构快照；连接快照的箭头不要求把实例强塞进 Flow Entity，也不替代原有 Flow 图
- 只展示理解结论必需的变化，不照搬某个示例的节点数、变换种类或布局；中英文、桌面与窄屏均验证可读性
...
| 块状流程图（逻辑 + 解释） | 解释真实包边界、模块职责、数据 / 编译流程、入口与输出、可选依赖、扩展点或诊断路径；读者还要知道“在哪里、如何发生、由谁负责” | 常规节点统一有框，Secondary 可无描边，按需分组；节点可用第二行标明职责或实现锚点。主链突出真实执行 / 数据流，辅助关系用虚线退后 |
...
4. 同一张图的常规节点采用一致边界模式，Secondary 可用无描边中性底色表达辅助层级；两种风格的共用视觉约束仍以 `docs-figure-contract` 为准。
...
| `docs.logic.secondary`     | 全部 role                               | 相对本图主线的辅助、背景或派生内容；无描边、中性浅底 |
| `docs.logic.algorithm`     | `activity`                              | 算法、高复杂度或性能问题，`darkviolet`               |

普通节点省略 `kind`；Secondary 只表示相对本图主线的辅助、背景或派生内容，不等于普通、非强调、校验、关闭或输出。先确定本图解释主体：主链中的校验、输入与产物不自动降级；同一辅助系统在自身机制图中仍是主线。关闭或阻断用 `status`，不以 Secondary 代替。

Secondary 保留 role 形状，使用无描边中性底色（亮色 `#e4e4e4`、暗色 `#1b1b1b`）；文字适度弱化（亮色 `#555555`、暗色 `#bbbbbb`），底色不随 group 或类别色改变，状态通过文字表达，实例样式仍可覆盖。复用站点主题规则，不在消费图逐项写颜色。
...
点线只表示不存在于 Graph 模型中的教学几何参考，不伪装成 Flow Relation 或 `LogicFigureRelation`。错误、成功、警告与禁用使用 Graph `status`；普通节点的 `disabled` 额外使用 `[6, 4]` 虚线；Secondary 无描边时仅保留状态文字色。
...
节点优先表达“角色”，不是文件、函数或临时变量。边框必须表达分类、分组、容器或复杂流程边界；若角色无需分类且内容清晰简洁，整张图可以使用 `stroke="none"`、无填充的无框节点。同级常规节点边界保持一致；Secondary 可与有框主节点共存，以语义而非装饰区分边界。

- 默认不要使用对应圆形的 `event` role：圆形会随文本宽度显著增大，破坏流程的列宽和对齐。配置变更、初始化失败等带说明的触发条件优先使用 `activity` 的紧凑圆角矩形；仅当标签很短，且必须突出瞬时事件这一语义时才使用 `event`。

选择有框模式时使用下表：

| 节点类型     | stroke / fill                                | 使用方式                                                                                                              |
| ------------ | -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| 常规流程角色 | `gray` stroke + 同色低透明 fill              | 用于处理阶段与无须分类的稳定角色；统一 `cornerRadius={4}`。                                                           |
| 类别角色     | P2 类别色 stroke + 同色低透明 fill           | 用于读者必须区分的稳定角色类别；同色节点必须属于同一类别。                                                            |
| 当前重点角色 | 保持所属类别颜色                             | 单行节点可加粗正文正在解释的一到两个角色；双行节点使用常规字重标题，重点靠主链位置和正文指认                          |
| 辅助角色     | Secondary 的无描边中性底色                   | 仅用于相对本图主线的辅助依赖、背景或派生内容；保留 role 形状，连线按真实关系选择。                                    |
...
- 密集结构连接：使用 `gray` 细实线，`strokeOpacity` 取 `0.6–1`，可省略箭头头部，让布局暗示方向。
...
普通概念图不把文件路径、函数全名或完整条件表达式写进 path label。`docs-doc-mechanism` 的源码导览图可使用准确函数名加简短作用的两行 label，表示该边真实发生的转换；内部成员不是公共 API。放不下时调整布局或移到就近正文，不缩写源码标识符
...
- 普通中性节点使用默认外观；Secondary 使用站点中性底色，不为获取灰色外观而声明该 kind。
- 语义分组统一使用 `LogicFigureFrame` 的虚线边界，中性色与 `0.6–1` 透明度遵循 `docs-figure-contract`；不要手写空边界 `Node`。几何 / 参考边界不套用此规则。
- 颜色只表达类别，当前重点只调整标题字重；不用高饱和 fill。
- 常规节点保持一致边界；Secondary 使用独立的无描边中性底色，不要求主节点跟随变为无框。
...
- 同级常规节点边界是否一致，Secondary 是否确实表示本图辅助内容，有框常规节点是否都使用 `cornerRadius={4}`？
