...
6. `## 相关属性`：用 `<ComponentProps>` 汇总当前页的定义函数、Definition / type、宿主注册入口和输入引用字段；完整字段与类型链接到 API 参考
...
相关属性的位置与组件契约见 [相关属性](../../docs-doc-principle/references/component-props.md)，不另设手写 API 列表。
