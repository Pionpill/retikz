...
字典类组件页使用下面 5 类 section，**按顺序**出现；`How it works` / `Related` 可选，其余不要新增散乱顶级章节。需要额外内容时优先并入 `Usage`、`Examples`、`How it works`、`API overview` 或拆子页。
...
| `## API 概览 / API overview` | ✅   | 本页上手直接需要的 props / 伴随 API；可用 4 列表（`属性 / 类型 / 默认值 / 描述` / `Prop / Type / Default / Description`），无默认填 `—`，属性名 + 类型用反引号包；多组件合一页时按组件分子节，并链接完整 API Reference |
...
| `## 作为 Path 图形（Sugar）` | ✅   | 该 Sugar 组件的完整文档：`### 用法`（import + 骨架）/ `### 例子`（demo + 写法表）/ `### API 概览`（组合表）                                                       |
| `## 作为 Node 形状`          | ✅   | 该 shape 的完整文档：`### 用法` / `### 例子`（含 params / 几何 anchor）/ `### API 概览`（shape 值 / params / 边界表）                                             |
...
- 当前页字段表属于文中 API 介绍；How it works **不**重复列字段
...
- 不把本节叫作 API Reference，也不在此复制整个包的公开导出、完整 overload 或 schema 字段；它只服务当前组件页的阅读主线。完整 TypeScript API 链接到 API Reference，持久化字段链接到 Schema Reference
...
- **How it works 段塞 API 表** —— 字段表归文中 API 介绍；本节讲机制不讲字段；要交叉引用时 API 表「描述」列写"详见 § 技术原理"指过去
