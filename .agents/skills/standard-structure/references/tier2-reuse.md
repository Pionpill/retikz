...
- 先读 `standard-structure`，再按实际改动读取 [schema](schema.md)、[contract](contract.md)、[providers](providers.md)、[pipeline-compile](pipeline-compile.md) 和 `test-contract`。本文件只负责跨层的完整复用判定与闭环门槛，不替代对应层级 reference
- 固定 `Tier 2 → Standard/Core` 的依赖方向。领域字段留在 Plot、Table 等 owner；去掉领域词汇后仍成立、且有多个消费者的绘图语义才下沉到 Standard/Core
- 以每个 lower target owner 的公开契约、canonical lowering/pipeline、compile、Scene 及 manifest/artifact 行为为 source of truth；Scope 只是最常见的审计案例，不是本文件的范围边界
