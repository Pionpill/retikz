...
- `layoutXxx` 可在依赖就绪时调度领域 resolve，但不自行复制其默认、lookup 或 fallback 逻辑。
- `emitXxx` 输出 Scene primitive / resource；`collectXxx` 只向显式传入的 collector 追加所声明的对象，不隐藏编译阶段推进。
- `computeXxx` 是纯计算，不查 registry、不产生 warning、不修改 context。

## Pipeline / Compile 文件结构

- `compile.ts` 只放 `CompileOptions`、`compileToScene` 和顶层编排；domain 细节下沉到对应目录。
- 目录级 `index.ts` 只做稳定入口导出，不承载业务逻辑；默认 `export *`，公共 facade 或需要避免冲突时才精选导出。
