...
```tsx
<Draw
  way={['a', { label: { text: 'anchors', side: 'above', textColor: 'gray', font: { size: 12 } } }, 'b']}
  arrow="->"
/>
```
...
const Demo: FC = () => (
  <Layout width={660} height={150} style={{ maxWidth: '100%', height: 'auto' }}>
    {/* 顶行：数据管线 */}
    <Node id="a" position={[-300, 0]} stroke="none">
      A
    </Node>
    <Node id="b" position={[-100, 0]} stroke="none">
      B
    </Node>
    <Node id="c" position={[100, 0]} stroke="none">
      C
    </Node>
    ...
    {/* 底行：模块来源 */}
    <Node id="b_src" position={[-100, 90]} stroke="none">
      src/b/*
    </Node>
    <Node id="c_src" position={[100, 90]} stroke="none">
      src/c/*
    </Node>
...
const Demo: FC = () => (
  <Layout width={520} height={210} style={{ maxWidth: '100%', height: 'auto' }}>
    {/* 左列输入 */}
    <Node id="in1" position={[-160, -48]} stroke="none">
      Input 1
    </Node>
    <Node id="in2" position={[-160, -16]} stroke="none">
      Input 2
    </Node>
    <Node id="in3" position={[-160, 16]} stroke="none">
      Input 3
    </Node>
    <Node id="in4" position={[-160, 48]} stroke="none">
      Input 4
    </Node>

    {/* 中心 */}
    <Node id="hub" position={[-30, 0]} stroke="none">
      Hub
    </Node>
    <Node id="next" position={[60, 0]} stroke="none">
      Next
    </Node>

    {/* 右列输出 */}
    <Node id="out1" position={[200, -48]} stroke="none">
      Out 1
    </Node>
    <Node id="out2" position={[200, -16]} stroke="none">
      Out 2
    </Node>
    <Node id="out3" position={[200, 16]} stroke="none">
      Out 3
    </Node>
    <Node id="out4" position={[200, 48]} stroke="none">
      Out 4
    </Node>
...
关系 / 概念图的常用编码（一套直接套用，全页一致）：

| 角色                                  | 编码                                                           |
| ------------------------------------- | -------------------------------------------------------------- |
| 参考目标 / 连接边界（boundary、bbox） | `stroke="gray" dashPattern={[4, 3]}`——框是概念的一部分，要可见 |
| 纯文字锚点 / 来源点                   | `stroke="none"`，只留文字                                      |
| 成组 / 派生边界（cluster bbox 等）    | `lightgray` 虚线，比单体边界更淡                               |
| 强调的落点 / 当前态                   | `darkorange`                                                   |
...
- **shape 专属 anchor 用字符串 shorthand**——`'id.tip-0'` / `'id.apex'` 会被 parser 拒掉（只认标准方位名），运行时 throw（tsc 查不出，类型是 string）；star 的 `tip-N`、sector 的 `apex` / `outer-arc-mid` 等必须写对象形态 `{ id, anchor: 'tip-0' }`
- **size 档贪大**——叙述图默认优先 `xs` / `sm`，够说清就行；要塞进更小档靠压纵向高宽比，不是硬撑 `md+`
