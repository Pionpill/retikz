...
**绝不 push、绝不切回 main、绝不 merge、绝不调 alpha-feature-wrapup**。
...
| 一条龙（单条 ADR） | alpha-feature-dev（本 SKILL）单条模式 |
| 多 ADR 离线一次跑完 | alpha-feature-dev（本 SKILL）批量 worktree 模式 |
...
收尾时机：所有 worktree 都人工 review 通过 + 合并进 main 之后，整 milestone 跑一次 alpha-feature-wrapup（不每条 ADR 单独 wrapup）。
