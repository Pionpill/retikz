...
- alpha-feature-test 留下的 INFO 列表（实现稳健性素材）
- alpha-feature-document 完成的 mdx / demo
...
Agent({
  ...
  model: 'opus',
  description: 'Contract Auditor for ADR-NNNN',
...
- 你不必抓"代码 bug"——那是 alpha-feature-test 第一关 Bug Hunter 的事。
...
git commit -m ":books: alpha.X changelog：ADR-NNNN <一句话>"
```
...
| 情景 | 处理 |
|---|---|
| Contract Auditor 报告"实现与 ADR 严重不符" | halt → 主 AI 呈给人工，可能要回 alpha-feature-implement 重做或修 ADR |
...
- **上游**：alpha-feature-document（文档已落地）
- **下游**：可能的 [`package-publish`](../package-publish/SKILL.md)（如要发版）；否则结束、等下条 ADR 走 alpha-feature-dev
...
- 人工显式说"完成"或调用 alpha-feature-dev 进下一条 ADR
