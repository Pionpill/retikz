---
name: cross-test
description: retikz 交叉测试 / 缺陷挖掘技能。用于基于 packages 下现有实现和已有测试补充边缘场景测试，以找出现有实现缺陷为目标，不能凭空编造需求；同时从真实使用者角度评估 API、错误信息、文档契约、可诊断性与可改进点。适用于 alpha/beta 任意阶段的独立质量审计、回归测试补强、adversarial 测试、用户视角体验评估，以及把发现沉淀为测试、问题报告或 notes/plans TODO。
---
...
- 现有实现：`packages/core/src/**`、`packages/react/src/**`。
- 现有测试：`packages/*/tests/**`。
- 可选上下文：`notes/plans/**`、`notes/adr/**`、`apps/docs/**`。
...
- schema 与核心类型：`packages/core/src/ir/**`、`primitive/**`
- 编译链路：`packages/core/src/compile/**`
- React 链路：`packages/react/src/kernel/**`、`sugar/**`、`render/**`
- 测试分布：`packages/*/tests/**`
...
| 顺序敏感 | path 首段不是 move、cycle 后继续画、多个 sub-path + arrow、多个 `<Tikz>` 同页 |
...
- core parser：`packages/core/tests/parsers/*.test.ts`
- core compile：`packages/core/tests/compile/*.test.ts`
- core geometry：`packages/core/tests/geometry/*.test.ts`
- React builder/unbuilder：`packages/react/tests/kernel/*.test.tsx`
- React render：`packages/react/tests/render/*.test.tsx`
- Sugar：`packages/react/tests/sugar/*.test.tsx`
...
- beta 非破坏优化：追加到 `notes/plans/v0/v0.1-beta.1.md` 或当前 beta plan。
