...
3. **测试要能被留下**
   ...
   - 对不确定行为：只在目标 workspace 的 `tests/_scratch/` 写临时 exploratory test，并用 `pnpm temp:test -- --workspace <workspace-directory> --file <tests/_scratch/*.test.ts>` 运行；默认验证后自动删除，只有确认真实 bug、稳定公开契约或高风险边界时才转为正式测试或改成计划 TODO。
   - 不要删除或弱化已有测试断言。
...
| 疑似 bug     | 先在 `tests/_scratch/` 写最小 exploratory test 或用现有 helper 复现；用 `pnpm temp:test` 验证后自动删除，只有证实真实风险才正式化 |
...
- beta 非破坏优化：登记到当前中版本的 ignored 实施 plan；roadmap 只汇总能力 / 质量目标，不按 beta.N 新建目录或任务清单。
