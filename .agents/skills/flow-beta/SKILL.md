...
| 2 | 多 LLM 评估 | 本 skill 调度 | BLOCKING 清空，WARNING 经人工裁决 |
...
beta 的核心风险是重构回归和 breaking 漏迁移。至少准备以下评估材料：
...
优先跨模型 / 跨线程 / 外部窗口；主 AI 不写死模型名。用户也可手动跑评估并把结论贴回。主 AI 合并为 BLOCKING / WARNING / INFO：
...
- 实现和必要测试已落。
- 受影响模块验证通过。
- 多 LLM 评估合并，BLOCKING 清空，WARNING 已裁决。
- roadmap TODO 标完成。
