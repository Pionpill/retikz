---
name: flow-beta
description: retikz beta 期非破坏性优化编排——按 plan TODO 一条一条做，不开 ADR、不动 IR schema 字段名 / 语义 / 公开 API surface。3 阶段：实现 → 多 LLM 评估 → 收尾，每阶段硬关卡。**多 LLM 评估**是 beta 的核心阶段（与 alpha 的 adversarial 不同——beta 评估的是"refactor 是否真的非破坏 + 是否真的更好"而不是"找 bug"）。beta 期专用——若发现需要破坏性改动 → halt → 推到下次 alpha 窗口。
---

# beta 优化主流程

retikz beta 期 plan TODO 端到端执行的**编排器**。beta 的核心特征与 alpha 截然不同：

| 维度 | alpha（`flow-alpha`） | beta（本 SKILL） |
|---|---|---|
| 改动性质 | 加新功能 / 改 schema / 扩 API | 重构 / bug fix / 性能 / 注释 / 测试加固 / 文档完善 |
| 输入文档 | ADR | plan TODO 条目 |
| 破坏性 | 允许（alpha 是破坏性窗口） | **禁止**（schema 字段名 / 语义 / 公开 API surface 冻结） |
| 主要风险 | 设计选错路径 | 重构引入回归 / 偷偷破坏 API |
| 多 LLM 用法 | 设计阶段评估 ADR | 实现完成后评估"是否真非破坏 + 是否真更好" |
| 阶段数 | 5（设计 / 实现 / 测试 / 文档 / 收尾） | 3（实现 / 多 LLM 评估 / 收尾） |
...
| 2 | 多 LLM 评估 | （本 SKILL 内联，见下） | AI 调度多 LLM | 全部 LLM 都判"非破坏 + 改动确有收益"才进 3 |
| 3 | 收尾 | [`develop-wrapup`](../develop-wrapup/SKILL.md)（简化路径） | AI 起草 / **人工**最终 ack | 用户可见行为变化时才写 changelog；commit 必须经人工授权 |
...
beta 改动不动 IR schema / public API，自动排除 red。判级仅区分"内部重构 / 外部可见"：

| Level | 触发条件 |
|---|---|
| **internal** | 仅 `packages/{core,react}/src/**` 内部目录搬迁 / 注释 / 私有函数重命名 / 文件拆分 / 类型层重构（不导致 public API surface 变化） |
| **visible** | TODO 触及任何 `apps/docs/**` / `packages/*/src/index.ts`（公开 export 变化）/ 用户可见行为（错误信息 / 默认值 / 性能可观察变化） |

判级影响 stage 2 多 LLM 评估的深度（visible 需更严格审核 + stage 3 必写 changelog）。判级在 develop-implement / 本 SKILL stage 2 各跑一次。

**红线**：若发现 TODO 实际需要改 IR schema 字段名 / 语义 / 公开 export 名 → 立刻 halt，呈给人工——这条 TODO 不是 beta 范围，应推到下次 alpha 窗口。
...
- **不允许改既有测试断言**（除非测试本身在测错行为 / 即将变化的实现细节）——既有测试是"非破坏"的最直接守门
- **新加测试覆盖**视 TODO 性质：
...
beta 阶段最大风险是**偷偷破坏 API**——同 model 自己 review 改动会漏。本阶段**派两个不同 LLM 独立评估**，全部通过才进 stage 3。
...
**Prompt 角度**：审计视角——"这份 diff 是否真的非破坏？"
...
### 评估者 2：收益审计（**visible 必跑，internal 可选**）
...
| wrapup 子步骤 | beta 是否做 |
|---|---|
| changelog 草稿（zh + en） | **visible 必写**；internal 不写（用户拿到 npm 包零感知） |
...
- changelog 仅 visible 写；internal 改动只在 commit message 体现
...
- **等价性审计 LLM**：跨 vendor（用户提供）——审计 diff 是否真非破坏
...
| 触发 | 动作 |
|---|---|
| 实际改动需要改 IR schema 字段名 / 语义 / 公开 export 名 | halt → 这条 TODO 推到下次 alpha 窗口、从 plan 移除 |
...
- visible 改动有 changelog 段（zh + en）
- 多 LLM 评估意见已合并、BLOCKING 全清
- 人工显式说"完成"或调用本 SKILL 进下一条 TODO

## 何时换回 flow-alpha

发现需要破坏性改动（动 schema / public API / IR 字段名等）时——立刻 halt 本 SKILL，把该需求登记为 alpha 候选（roadmap / 下次 alpha 窗口），用 [`flow-alpha`](../flow-alpha/SKILL.md) 走 ADR 流程。**beta 不开 ADR、不动 schema** 是硬约束。
