...
- **文件夹导出边界**：文件夹下需要被外部使用的内容，都必须通过同级 `index.ts` 导出；不允许让外部调用方绕过目录入口去依赖内部文件。
- **同级模块访问边界**：同级模块之间只允许通过 `./module` 获取内容，不允许深入 `./module/subModule`、`./module/file` 获取；如果确实需要该内容，先由被访问模块的 `index.ts` 导出，再从 `./module` 引入。
- **barrel 归属边界**：`index.ts` 只能导出本模块拥有的内容，不允许把非本模块内容转手 export 出去。例如在 `resolve` 模块里 `import type { ParsedFieldValue } from './coerce'; export type { ParsedFieldValue } from './coerce';` 属于错误边界，应该由 `coerce` 自己的模块入口导出，调用方从 `coerce` 获取。
