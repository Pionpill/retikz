...
- 文件 scope 命中 `packages/react/**` 改 authoring 面时，检查是否需要 `packages/vanilla/**` 对应改；反之亦然。
...
实现 Agent 可在用户授权范围内自己跑 commit；未获得本轮流程级 commit 授权时，改为 "git stage 改动 + 报告改了什么"，由主 AI 呈给用户审阅后再提交。
