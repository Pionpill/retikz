...
> **文档不是可选项（用户习惯铁律）**：stage 4 **不因自治 / 离线 / "先收口" / 上下文预算而跳过或延后**——用户 review 先看站点 demo 再看代码，**没补 demo 的功能不算 flow 走完**。宁可少接一个功能，接了的必走到文档。完工汇报**先呈文档页 + 访问路由**，再讲代码。详 [`develop-document` §评审优先级](../develop-document/SKILL.md)。
...
需要独立视角时，优先使用当前环境可用的子代理 / 新线程 / 外部模型窗口；没有就由主 AI 执行并如实说明退化。不要写死模型名，同模型盲区一致风险通过 **prompt 角度故意错开**缓解：

- Spec Writer：写规格 + 测试（建设视角）
- Bug Hunter：构造让实现挂的输入（破坏视角）
- Contract Auditor：对账 ADR / changelog / docs / 行为四方一致（审计视角）

**用户可以介入用其它模型 / 线程并行跑同一阶段**（例：把同一份 ADR + diff 也喂给另一个模型跑 adversarial bug hunting）。主 AI 接到多份意见后**取并集 + 去重**，写进同一个 BLOCKING 列表。
...
4. 人工 ack 后：
   - 互不相干（无文件交叉、无符号依赖）→ 并发调度，每条 ADR 一个独立任务 + 一个 worktree
   - 有依赖 → 按链堆叠，串行 commit
   - 混合 → 主链堆叠 + 独立支线并发
...
- **无交叉** → 全部从确认后的集成基线拉，平行 worktree（可并发跑）
- **有交叉** → 找"地基"ADR（其它 ADR 需要先合并它的产物），按依赖链堆叠

示例（v0.1-alpha.5）：

```
<base: next-core / next-plot / next，由人工确认>
 ├─ codex/adr/alpha.5/01-scene-primitive       worktree: ../<repo>-adr-01
 │   └─ codex/adr/alpha.5/04-position-offset   worktree: ../<repo>-adr-04
 │       └─ codex/adr/alpha.5/02-step-label-t  worktree: ../<repo>-adr-02
 │           └─ codex/adr/alpha.5/03-arrow     worktree: ../<repo>-adr-03
 └─ codex/adr/alpha.5/todo-4-rename            worktree: ../<repo>-todo-4   ← 独立基线
```
...
```bash
# 先由人工确认 base（通常 core → next-core，plot → next-plot，跨域 → next）
git worktree add -b codex/adr/alpha.5/01-scene-primitive   ../retikz-adr-01 <base>
git worktree add -b codex/adr/alpha.5/04-position-offset   ../retikz-adr-04 codex/adr/alpha.5/01-scene-primitive
git worktree add -b codex/adr/alpha.5/02-step-label-t      ../retikz-adr-02 codex/adr/alpha.5/04-position-offset
git worktree add -b codex/adr/alpha.5/03-arrow             ../retikz-adr-03 codex/adr/alpha.5/02-step-label-t
git worktree add -b codex/adr/alpha.5/todo-4-rename        ../retikz-todo-4 <base>
```

pnpm hardlink store 下每个 worktree 的 node_modules 占用极小。创建 / 删除 worktree 前仍需确认确实需要分支操作。
...
**绝不 push、绝不切回集成基线、绝不 merge、绝不调 develop-wrapup**。
...
> push / merge / 切回集成基线这三条是**真正的硬红线**，批量模式不放宽。
...
- **互不相干** → **默认并发**：主 AI 先用当前工具发现可用的 multi-agent / thread 能力，再一次派 N 个独立任务；每个任务 cwd = 一个 worktree，互不可见上下文
- **有依赖（堆叠）** → 串行：后面的 base 在前面之上，AI 必须等前一条 commit 完才能开下一条；不能用并发派单
- **混合** → 堆叠主链 1 路（串行）+ 独立基线 N 路（并发）= 并发数 N+1

派独立任务时 prompt 必须含：

- 当前 worktree 路径（`cwd`）
- 该 ADR 的完整路径
- "你只动这个 worktree、跑完 5 阶段后写 REVIEW.md、不 push / 不 merge / 不切回集成基线 / 不调 wrapup"
...
cd ../<repo>
git switch <base>
git merge --no-ff codex/adr/<milestone>/XX-...   # 或 ff-only 看偏好

# 下一条 rebase 到新 base
cd ../<repo>-adr-<next>
git rebase <base>                          # 大概率干净，冲突就解
```
...
任何阶段任意独立任务单步连续 3 轮没收敛 → halt，主 AI 把当前状态呈给用户，由用户决定升级人工还是改 ADR / 改 spec。

具体阈值由各子 SKILL 落实：

- 实现任务让 spec test 跑过：3 轮失败 halt
...
- [ ] **没** push、**没** 切回集成基线、**没** merge、**没** 调 wrapup
- [ ] 工作区只动了自己分支应改的内容（无跨 worktree 串话）

收尾时机：所有 worktree 都人工 review 通过 + 合并进集成基线之后，整 milestone 跑一次 develop-wrapup（不每条 ADR 单独 wrapup）。
