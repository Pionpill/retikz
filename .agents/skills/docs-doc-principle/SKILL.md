...
- 正文和图中文字以白话为主，只保留少量必要的通用名词；先说对象发生了什么，再按需括注正式术语或 API。不要用学术化定义、内部黑话或层层命名把简单概念复杂化，进阶与原理页也不例外；简化表达不能牺牲准确性
...
解释原理 / 机制时，图文结合是讲解主线：先提出具体问题，用实例图或 demo 展示效果，再用白话指着图中的对象、刻度、箭头或前后状态逐步解释，最后对应实现流程与术语。正文和图复用同一例子与命名；不先堆长段抽象解释，也不以“有图”代替带读者看懂图。

中高难度文档讲解多阶段处理、变换或状态演进时，除 Flow 流程图外，必须就近补充具体实例的连续状态图：用同一个对象展示初始、中间与最终结果，并用箭头连接实际操作。Flow 解释处理关系，实例图解释对象如何变化；不得只靠文字或另一张抽象流程图补充。画法见 [docs-figure-logic](../docs-figure-logic/SKILL.md#具体实例的连续状态图)。
...
- 读者评审任务必须明确要求检查白话程度：即使术语已有定义，也要指出不必要的专业化、术语堆叠和把简单概念讲复杂的段落或图标签，并给出更直接、准确的替代表达
