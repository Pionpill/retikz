...
- 常规节点按表达需要统一采用有框或无框模式，边框须承担分类、分组、容器或流程边界语义。Secondary 可用无描边中性底色与有框主节点共存，表达真实辅助层级，不为装饰混搭边界。
...
颜色只标记角色类别：同色节点必须属于同一稳定类别，无关角色使用中性色或不同语义色。单行同级节点的当前重点可用 `font={{ weight: 'bold' }}` 标出，不借复用强调色表达；双行节点以常规字重标题和灰色次要说明区分层级。

线条、边界和文字不得使用在任一明暗主题中接近背景的固定浅色或深色（如 `lightgray`、`dimgray`）。中性元素使用 `gray` 或随主题适配的 `currentColor`，以对应透明度 `0.6–1` 调整层级；默认 `1`，优先用 dotted / dashed 区分辅助线与边界。不得对整个分组降透明度而连带弱化正文。低透明浅填充只作底色，不承担可辨识的边界或文字职责。
...
- 第一行是角色标题，默认 `14px`、常规字重，不自动加粗。
- 第二行是职责、输入输出或实现锚点，默认 `12–13px`、常规灰色。
- Flow Entity 文本较多时，`text` 使用 TextBlock 数组：第一项写主内容，第二项写次要说明并设置 `fill: 'gray'`（可配 `font: { size: 12 }`）；不要用字符串 `\n` 代替分层，也不要用实体级 `style.textColor` 将两行一起变灰。
- 文件路径、目录名和实现位置默认放第二行；它们不是下游阶段，不单独连接箭头。
- 同一图的双行节点统一字号、行距和最小高度；空间不足时先缩短说明或调整布局，不单独缩小某一项。
- 双行节点通过字号与第二行灰色区分主次；需要强调当前角色时，优先依靠主链位置和正文指认。
...
- 同一纵向 `FlowLayout` 中直接并列的 Entity 属于一个视觉分组时，设置 `itemWidth="match-largest"`，使其采用该组最长实体的统一宽度；不要用逐项固定宽度模拟这一规则，也不要让该策略跨越嵌套 Layout。
...
每次完成 retikz 自绘后，记录绘制过程中出现的能力缺口：期望存在却缺失的能力、为完成图示采取的绕行、效果未达预期的图形，以及自检或用户二次修改暴露的问题。图示任务中发现的其它 Retikz 能力缺陷也可自行反馈。逐项判断根因是否在 IR / 数据结构、公开属性设计或属性 `describe` 的误导；任务结束时将有证据的发现与影响一并反馈。没有发现时不虚构问题。
...
- 同级常规节点边界一致，Secondary 的无描边用于区分真实辅助内容；有框常规节点均为 4px 小圆角。
