...
description: retikz 文档站叙述性插图总规则。Use when drawing or reviewing architecture, flow, concept, schema, API, or implementation-explanation figures in apps/docs with retikz itself; defines dogfood, ComponentPreview hideCode, file shape, visual vocabulary, responsive layout, node/edge conventions, and validation. Specialized figure skills such as docs-figure-logic must read this first.
---
...
- 节点默认是文字锚点，不是 UI 卡片。除非“框”本身是概念，否则 `Node` 默认 `stroke="none"`。
...
| `darkorange`   | 第一强调对象，当前重点、被解释的关键节点           | P2 强调色   |
| `dodgerblue`   | 第二强调对象，与第一重点并列或对比                 | P2 强调色   |
| `darkviolet`   | 第三强调对象，仅当同图必须区分三类重点时使用       | P2 强调色   |
| `red`          | 错误、失败、拒绝、危险路径                         | P5 语义专用 |
| `green`        | 成功、通过、完成路径                               | P5 语义专用 |

自检：灰度打印后结构仍一眼可读；彩色只落在真正需要强调的元素上。
...
- 新增 / 修改 `.demo.tsx` 插图：`pnpm --filter @retikz/docs exec tsc --noEmit`，并打开页面做视觉检查。
