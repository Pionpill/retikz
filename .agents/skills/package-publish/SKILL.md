...
description: 发布 retikz 的 8 个 publishable npm 包：core 组 5 包与 plot 组 3 包分别 lockstep、版本线独立。覆盖版本号、结构化 changelog、模块徽章、roadmap 跟踪、验证、tag/publish 授权边界；发布后按当前 roadmap 预 bump 下一开发版本。retikz 专用。
---
...
| **路线文档** | 按当前发布组对应 roadmap 的既有格式更新里程碑跟踪（通常是勾选版本 checkbox；core 参考 `packages/kernel/_notes/decisions/v0/v0.3/roadmap.md`，plot 参考 `packages/graph/_notes/decisions/v0/...`） |
...
**8 个可发布包，分两组、各自 lockstep，同组同版本同发；两组版本线独立。当前版本以各 `package.json` 为准。**
...
参考当前发布组的 roadmap（core 通常在 `packages/kernel/_notes/decisions/v0/v0.3/roadmap.md`，plot 在 `packages/graph/_notes/decisions/v0/...`）。pre-stable（v1.0 之前）走 `-alpha.N` / `-beta.N` / `-rc.N`：
...
2. **本次发布要包含的组**（core 组 5 包 / plot 组 3 包；默认按本次改动涉及的组整组发，用户可指定）
...
直接改本组各 `packages/<group>/<pkg>/package.json` 的 `"version"` 字段。**同组所有包同版本号**（core 组 5 包，或 plot 组 3 包）。

这是发版流程的第一性动作：目标版本（例如 `0.3.0-rc.1`）必须在 working tree 中显式出现，经过验证、commit，然后再创建 tag 与 npm publish。不要在 publish 命令或临时脚本里动态覆盖版本号。

如果想用 CLI（发 core 组示例，5 包同版本）：

```bash
for p in math core render react vanilla; do pnpm --filter @retikz/$p version 0.3.0-alpha.4; done
...
- `Release.minor` 用 `v0.3` 这种中版本 key（plot 组用独立 key 如 `plot-v0.1`，避免与 core 版本线串台）；`PackageBlock.pkg` 只写本次实际影响的包（8 个发布包之一 / 必要时 `docs`）。`PACKAGE_IDS` + `PACKAGE_LABEL` 在 `changelog.types.ts`，新包要先在那里登记才能进 changelog 与筛选 chips。
...
里程碑跟踪段通常在当前发布组的 roadmap 里：core 看 `packages/kernel/_notes/decisions/v0/v0.3/roadmap.md` 或当前活跃版本目录，plot 看 `packages/graph/_notes/decisions/v0/...`。按 roadmap 既有格式更新本次发布对应版本；若是 checkbox，就把 `[ ]` 改成 `[x]`：
...
| 类型 | `for p in math core render react vanilla plot plot-react plot-vanilla; do pnpm --filter @retikz/$p exec tsc --noEmit; done` + `pnpm --filter @retikz/docs exec tsc --noEmit` |
| 测试 | `pnpm test` |
| build | core：`math core render vanilla react`；plot：`plot plot-vanilla plot-react`，按依赖序 `pnpm --filter @retikz/$p build` |
...
- 验证结果：`pnpm lint`、8 个发布包 + docs 的 `tsc --noEmit`、`pnpm test`、build、dry-run。
...
for p in math core render vanilla react; do
...
目标目录示例：发 `0.3.0-alpha.3` → `packages/kernel/_notes/decisions/v0/v0.3/alpha.3/*.md`；plot 同理在 `packages/graph/_notes/decisions/...`。
...
1. 查路线文档的版本跟踪/进度段——Tier 1 看 [`v0.3/roadmap.md`](../../../packages/kernel/_notes/decisions/v0/v0.3/roadmap.md)，plot 看 [`plot/v0/v0.1/roadmap.md`](../../../packages/graph/_notes/decisions/v0/v0.1/roadmap.md)。
...
| 任务 | 改动 |
| --- | --- |
| 发 alpha.N+1 | 本组各包版本（core 组 ×5 / plot 组 ×3）+ `apps/docs/src/data/changelog.ts` + roadmap 勾选 |
...
- **ADR 封板压缩**（阶段 6.1）：发版后、bump 前把本里程碑 Accepted ADR 从「施工蓝图」压成「决策记录」（删 🔻 待决策点 / 实现契约两段、折成指针）。规则与生命周期定义见 `packages/kernel/_notes/decisions/_template.md` header「ADR 生命周期」段。
