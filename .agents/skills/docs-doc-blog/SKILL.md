...
- 单篇接近或超过 20 分钟时复核主题、读者任务与叙事主线；只有其中一部分能独立成文时才拆系列
...
| 语气                          | 中立工具说明                      | 第一人称、主观判断                               |
| 第三方外链                    | 禁                                | 允许                                             |
| 必填结构                      | 按阅读任务选择主 skill 与固定难度 | 无固定段落结构；文末「引用」节按正文有无引用选填 |
...
- **超过 20 分钟却不复核主题内聚性与跳读结构**——先检查是否存在可独立成文的主线；需要拆系列时按 alpha-0 / alpha-1 命名
