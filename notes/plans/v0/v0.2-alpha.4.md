...
- **B：emit 层增强** —— 开工前另起（同目录，编号续 01 之后）：
  ...
  - zIndex 字段语义（int / 缺省 0 / 稳定排序 / per-group 局部）+ **Node / Path / Scope 都加、Coordinate 不加** + scope-整体 vs scope-内部两层独立 + **zIndex 不进 every-X 默认通道** + transformed-scope path（及其 zIndex）hoist 限制 + 与 A 部分 `InternalScenePrimitive` / 占位的边界（占位不入 `zIndexOf`、排序在 sealSink 后）。
