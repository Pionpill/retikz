...
- **(B-zIndex)** Node / Path 显式声明 `zIndex` 时，Scene `primitives`（及每个 scope 的 `GroupPrim.children`）按 `zIndex ?? 0` 升序、同值严格保持 IR 顺序；**zIndex 子功能未使用时排序恒等**（稳定排序 + 全 0 键 = 不改顺序）。
...
> Coordinate **不加** zIndex——它不发 primitive，zIndex 无意义；Scope **不加** zIndex——本段排序单位是"会发 primitive 的 IR child"，scope 的 GroupPrim 在父层按默认 0 参与排序即可（见 §已拍板取舍）。
...
>
> 嵌套 scope 自然递归：内层 scope 先排自己的 `innerSink` → 被包成 GroupPrim 进外层 `innerSink` → 外层再排（该 group 以默认 0 参与）。**跨 group 不比较**——与 SVG `<g>` 局部 stacking context 一致。
...
> **node round-trip 自动**：`nodePropsFromIR`（`unbuilder.ts:14`）/ `buildNodeFromProps`（`builder.ts:103`）走 `pickDefined(_, NODE_FIELDS)`——加进 NODE_FIELDS 即两端同步。**label rotate 自动**：`label` 整体透传（`builder.ts:111` / `unbuilder.ts:18`），`rotate` / `keepUpright` 随 `IRNodeLabel` 进出，React 端零改动。
...
2. **B schema 扩字段**：`ir/node.ts`（NodeSchema.zIndex + NodeLabelSchema.rotate/keepUpright）、`ir/path/path.ts`（PathSchema.zIndex）。
3. **B React 字段表 + props + unbuilder**：`_fields.ts` 两表加 `'zIndex'`（互锁立即逼着改）、`Node.tsx` / `Path.tsx` props、`unbuilder.ts` path 分支补 `zIndex`。`cd packages/react && npm run test:run` 确认 round-trip 与互锁过。
...
6. **B zIndex tag + 排序**：`zIndexOf` Map + node 分支打标 + `PendingPath.zIndex` + `resolvePendingPaths` 复制到 real prim + `stableSortByZIndex` + 两落点（sealSink 之后）；先写 `z-index.test.ts`（red）。建在步骤 1 的占位/sealSink 之上。
...
- `kernel` round-trip 测试加：Node `zIndex` / Path `zIndex` / label `rotate` + `keepUpright` 经 `buildIR` → `convertIRToReactNode` → `buildIR` 字段保持（重点验 path zIndex 不被 unbuilder 手写分支漏掉）。
...
0. **【硬前置，B-3】rotated Node 上 label 的坐标空间**：`labelCenter`（`compile/node.ts:162` → `anchorOf` → `rect.anchor` 走 `localToWorld`）对 `rect.rotate !== 0` 返回**已旋转世界坐标**；当前 emit 把 label 放进 `inner`、`inner` 再被 Node 外层 rotate group 包一层 → label 位置**被绕 node center 旋转两次**（latent bug）。**必须在 B-3 emit 改动前 ADR 拍板**，二选一：
   - **(推荐) `labelCenter` 改用 axis-aligned rect（局部坐标）**：label 位置 + 自旋都在 `inner` 局部空间，由外层 Node rotate group 统一旋转一次——顺带修双重旋转、label 自旋自然与 Node 旋转叠加。代价：**改变现有 rotated-Node label 位置**（属修复），需 snapshot review + ADR 显式声明这是行为修正。
   - **(备选) label rotate group 移到外层已应用 transform 之后的空间**：保留 `labelCenter` 世界坐标，但 label 不进 `inner`、单独挂在 Node group 之外——破坏"label 跟 Node 一起旋转"的既有语义，且与 zIndex"Node 整体作 stacking 单位"冲突，不推荐。

   不旋转 Node（`rect.rotate === 0`）两方案等价、无差异——本待定只卡 rotated Node。
...
6. **Coordinate / Scope 不加 zIndex**：只有"会发 primitive 的 IR child"才有栈序意义。
7. **带文本才包 `<g>`**（非无条件全包）：纯几何 Node 维持极简平铺；带文本 = 语义化节点，给 DOM 边界 + stacking 单位。`layout.lines` 为判据。
8. **label rotate 中心 = label 自身中心 `[lx, ly]`**：位置仍由 `position` / `distance` 决定，`rotate` 只改朝向。**注意**：旋转 Node 上 `[lx, ly]` 取局部还是世界坐标由 §待定 0 决定，未拍板前不实施 B-3 emit。
...
- **B：emit 层增强** —— 开工前另起（同目录，编号续 01 之后）：
  - **（硬前置）rotated Node 上 label 坐标空间**（§待定 0）：选定局部 vs 世界，声明是否为行为修正 + snapshot 影响范围。
  - zIndex 字段语义（int / 缺省 0 / 稳定排序 / per-group 局部）+ Coordinate / Scope 不加的理由 + transformed-scope path 顶层排序限制 + 与 A 部分 `InternalScenePrimitive` / 占位的边界（占位不入 `zIndexOf`、排序在 sealSink 后）。
  - 带文本 Node 包 `<g>` 判据（`layout.lines`）+ 无旋转 group 无 transform + 不加 `data-node-id`（meta 留未来）。
  - label `rotate` 枚举值 + radial / tangent 在屏幕 y-down 的角度定义 + keepUpright 翻转阈值 + 与 Node 自身 rotate 的组合顺序。
  - 测试覆盖矩阵（zIndex 排序 / 稳定性 / scope 局部；group 包裹三态；label rotate 四模式 + keepUpright + **rotated Node 不漂移不变量**；React round-trip path zIndex 守卫）。
