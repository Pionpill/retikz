...
> 写于 2026-05-16。alpha.1 计划与 ADR 在 next 分支起草；v0.1.0 出关后再开实施代码。完工即删（git 历史还在）。
>
...
| # | 标题 | 状态 | 工作量 | 优先级 |
|---|---|---|---|---|
| 1 | [Scope IR schema + compile 下沉到 GroupPrim](../../adr/v0/v0.2-alpha.1/01-scope-ir-and-compile.md)（含 `scope.id` 字段 + PolarTranslate 第 4 transform 变体） | 草案 | 大 | P0 |
| 2 | [nodeIndex / anchor 跨 scope 解析](../../adr/v0/v0.2-alpha.1/02-node-index-anchor-resolution.md)（id 冲突共享命名空间 + 前向引用） | 草案 | 中 | P0 |
| 3 | [scope.id 注册 synthetic bbox layout](../../adr/v0/v0.2-alpha.1/03-scope-id-bounding-box.md)（scope 作为整体引用目标，对应 TikZ `local bounding box`） | 草案 | 中 | P0 |
| 4 | [scope 下相对定位语义](../../adr/v0/v0.2-alpha.1/04-relative-position-in-scope.md) | 草案 | 中 | P0 |
