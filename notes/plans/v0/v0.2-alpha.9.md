...
- **partway 端点 = 自包含 `AbsoluteTarget`**（Cartesian / Polar / NodeTarget / **AbsoluteOffsetPosition** / BetweenPosition），排除 `{ relative }` / `{ relativeAccumulate }`（避免 `Position`↔`Target` 递归，P1#1）；offset 端点用 `AbsoluteOffsetPositionSchema`（`of` 仅对象，不复用含 `z.string()` 的 legacy `OffsetPositionSchema`，P1#3）。
