...
| 3 | core 注释 / `.describe()` 去 SVG-imposing 语言 | ⏳ 待做 | 中 | P1 |
| 4 | `StepProps` 拆 10 个命名子类型 | ⏳ 待做 | 中 | P1 |
...
| 8 | geometry 4 shape `localToWorld`/`worldToLocal` 抽 `_transform.ts` + 死 `*Anchor` 类型清理 | ⏳ 待 review | 小 | P1 |
| 9 | 公开 type `TextLine` / `PathCommand` / `Transform` 字段 JSDoc 补 + 拆 named type | ⏳ 待 review | 小-中 | P1 |
| 10 | unbuilder round-trip 补 alpha.5 新增形态（OffsetPosition / AtPosition / `arrowDetail` / 7 keyword `position`） | ⏳ 待 review | 中 | P1 |
| 11 | `compile/path.ts` 拆目录 + `findPrev` O(n²)→O(n) + `THICKNESS_TO_WIDTH` 与 enum 互锁 | ⏳ 待 review | 中-大 | P2 |
| 12 | `_builder.ts` ↔ `_unbuilder.ts` 镜像字段表化（`NODE_FIELDS as const` + `AssertEqual` 互锁） | ⏳ 待 review | 中 | P2 |
| 13 | `_builder.ts` 60 处 `as` cast 收敛到顶层 | ⏳ 待 review | 中 | P2 |
| 14 | `CompileOptions.onWarn` 收集器：路径解析 silent fail → 显式 warning | ⏳ 待 review | 中 | P0 |
| 15 | 边界测试补：`parseWay` 形状算子错误路径 / `view-box` NaN-Infinity / `fallbackMeasurer` 极端值 / `buildPathD`/`buildTransform` throw message 契约 / 多 `<Tikz>` 实例 marker id 隔离 e2e | ⏳ 待 review | 中 | P2 |
| 16 | 改名 + 命名清理（`NodeTextSchema`→`TextBlockSchema`、`_builder` / `_unbuilder` 去 `_` 前缀、`renderPrim.tsx` 公开签名 `ctx`→`context`） | ⏳ 待 review | 小-中 | P2，**有破坏性嫌疑** |
| 17 | arrow marker `stableSpecKey` 字段表与 `ArrowEndSpec` 类型互锁 | ⏳ 待 review | 小 | P1 |
| 18 | core 测试 helper 去除 renderer mirror 漂移风险 | ⏳ 待 review | 中（74 处 `pathCommandsToD` / 8 文件） | P2 |
