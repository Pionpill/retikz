...
| 11 | `compile/path.ts` 拆目录 + `findPrev` O(n²)→O(n) + `THICKNESS_TO_WIDTH` 与 enum 互锁 | 📋 推 beta.2（red 大改动，需 e2e snapshot 守门） | 中-大 | P2 |
| 12 | `_builder.ts` ↔ `_unbuilder.ts` 镜像字段表化（`NODE_FIELDS as const` + `AssertEqual` 互锁） | 📋 推 beta.2（与 #13 cast 收敛同动 builder 合并实施） | 中 | P2 |
| 13 | `_builder.ts` 60 处 `as` cast 收敛到顶层 | 📋 推 beta.2（与 #12 合并） | 中 | P2 |
...
| 18 | core 测试 helper 去除 renderer mirror 漂移风险 | 📋 推 beta.2（74 处迁移量大、与 #11 ADR-05 配合） | 中（74 处 `pathCommandsToD` / 8 文件） | P2 |
