# v0.2 Path-level shape sugar 提案

> 写于 2026-05-17。v0.2 主线之一：补一组面向"画几何形"的 sugar 组件，让用户和 AI 都能用一行写出 TikZ 习语级的图元，不必下到 IR 层手拼 `<Path><Step>` 长串。
>
> 关联：[`v0.1-rc.2.md`](./v0.1-rc.2.md)（最初讨论起点，已撤回 TODO-5）· [`roadmap.md`](./roadmap.md) v0.2 段
...
- v0.2 同时落 **`zIndex`** + **Node group 包装**——sugar 派发出的 Path 也能享受这些 emit 层改造
...
所有 sugar 都共享 Path 视觉 prop（fill / fillOpacity / stroke / strokeOpacity / strokeWidth / dashed / dotted / dashPattern / strokeLinecap / strokeLinejoin / opacity / fillRule）。下表只列**形状专属 prop**。
...
派发：`<Path>` + `<Step kind="move" to={center}>` + `<Step kind="line" to={arcStart}>` + `<Step kind="arc" ...>` + `<Step kind="cycle">`。
...
**可选**：`borderRadius?: number`（单值，四角同半径；缺省直角）。
...
- **`Position` 字段类型**：所有点位接受 `[x, y]` 笛卡尔 + 极坐标 + `{ direction, of }` 相对——与现有 `<Coordinate>` / Node `position` 一致；sugar 内不解析（resolvePosition 是 compile 期工作），原样塞进底层 `<Path>` 的 `<Step>` `to`/`from`
...
5. **AST 解析器扩白名单**：`apps/docs/src/lib/jsx-to-ir/parser.ts` 当前 8 个组件 → 加 6 个（共 14）
6. **测试**：
...
7. **`composeSystem` system prompt 同步**：组件白名单更新到 14 个 + 每个新 sugar 配最小示例，提示 AI 优先用 sugar 而非 IR-level 拼装
...
- **`<Rectangle>` borderRadius 单值 vs 四角独立**：先做单值；四角独立（`borderRadiusTopLeft` 等）若有强需求再加
...
v0.2 开工前另起 ADR（位置 `notes/adr/v0/v0.2-alpha.1/0X-path-shape-sugar.md`，编号到时定），固化：

- 6 个 sugar 的最终 prop 形态清单 + 等价派发 IR
- IR 改动字段清单（arc 椭圆弧 / circlePath-ellipsePath 部分裁剪 / 新增 rectangle step）
- 命名口径（`Rectangle` / `Circle` 与 Node shape 同名风险的取舍 / `corner1-corner2` vs 其他命名）
- `<Grid>` 边界规则（严格按 step 步进、不补尾）
- `closed` 模式枚举值 + 各 sugar 默认值
- 多形态互斥 + 错误信息标准
- AST 白名单 14 个组件清单
- system prompt "Diagram output protocol" 更新内容
