...
v0.2 主线是 Scope + 样式继承（`<Scope>` / `<Group>` / `every node/.style` 默认值 / 局部 transform），同时**打开 NodeShape 的闭合枚举**——把"第三方自定义 shape 接入 IR"留作 v1 之前最后一步底层改造，并补两个 emit 层小改造：**显式 z-index 覆盖** + **带文本 Node 输出始终包 `<g>`**，加一组 **Path-level shape sugar**（6 个组件 + 配套 IR 扩张）。详细 ADR 在 v0.2 开工前另起，各项独立 ADR：[`v0.2-alpha.6.md`](./v0.2-alpha.6.md) 已先行铺好 sugar 提案。
