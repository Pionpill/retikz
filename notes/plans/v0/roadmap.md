...
| v0.2 | Scope + 样式 + Shape 扩展 | `<Scope>` / `<Group>`、`every node/.style` 默认值、局部 transform、`NodeShape` 闭合枚举打开 + ShapeRegistry 注入第三方 shape、结构化 Target / Anchor（对象 IR + 字符串 sugar 兼容）、显式 `zIndex` 覆盖、带文本 Node 输出始终包 `<g>`、Path-level shape sugar（Circle / Ellipse / Arc / Rectangle / Grid / Sector + IR 椭圆弧 / 圆角矩形 / 部分裁剪）、StepLabel 自定义样式（`textColor` / `opacity` / `font`） |
...
v0.2 主线是 Scope + 样式继承（`<Scope>` / `<Group>` / `every node/.style` 默认值 / 局部 transform），同时**打开 NodeShape 的闭合枚举**——把"第三方自定义 shape 接入 IR"留作 v1 之前最后一步底层改造；补一组定位表达改造：**结构化 Target / Anchor**（对象 IR + 字符串 sugar 兼容）；并补两个 emit 层小改造：**显式 z-index 覆盖** + **带文本 Node 输出始终包 `<g>`**，加一组 **Path-level shape sugar**（6 个组件 + 配套 IR 扩张）。详细 ADR 在 v0.2 开工前另起，各项独立 ADR：[`v0.2-path-shape-sugar.md`](./v0.2-path-shape-sugar.md) 已先行铺好 sugar 提案。
