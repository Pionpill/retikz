# v0.2.0-alpha.6 实施待办：Path-level shape sugar

> 写于 2026-05-17 作为独立提案；2026-05-18 并入 alpha.6 节奏（原文件 `v0.2-path-shape-sugar.md`）。v0.2 主线之一：补一组面向"画几何形"的 sugar 组件，让用户和 AI 都能用一行写出 TikZ 习语级的图元，不必下到 IR 层手拼 `<Path><Step>` 长串。
>
...
v0.2 开工前另起 ADR（位置 `notes/adr/v0/v0.2-alpha.6/`，编号到时定），固化：
