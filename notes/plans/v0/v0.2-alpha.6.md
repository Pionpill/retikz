...
export const TikZ: FC<TikZProps> = props => {
  // dev-only guard：沿用 core 既有约定（compile.ts:127 的 `typeof process` 守卫），
  // 不用 import.meta.env.DEV——后者是 Vite 专属，在 webpack/rollup/Next 同样可能缺失，更不可移植。
  // 守卫式 process.env 由打包器静态替换，裸 ESM 下 process 未定义则降级为静默（可接受）。
  if (
    typeof process !== 'undefined' &&
    process.env.NODE_ENV !== 'production' &&
    !tikzDeprecationWarned
  ) {
...
- system prompt（`context.ts`）：**面向 LLM / 文档的主契约仍是 17 个组件——只列 `Layout`，`TikZ` 作 deprecated alias 不计入**（中英双语把 `TikZ` 换成 `Layout` 并加一句 alias 注脚）；IR `to` 字段速查（65-67 / 171-173）由 `[x, y] | string` 改为主推对象 `{ id, anchor?, offset? }`（`[x, y]` / 字符串 shorthand 作便捷写法标注）。
...
- **dev-warning 守卫**：`<TikZ>` alias 用 `typeof process !== 'undefined' && process.env.NODE_ENV !== 'production'`（沿用 core `compile.ts:127` 既有约定），不用 `import.meta.env.DEV`（Vite 专属、跨打包器不可移植）。
