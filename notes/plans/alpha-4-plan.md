...
- 选项 A：保留在 IR（`Node.at?: { direction, of, distance? }`），编译期 resolvePosition 折成笛卡尔（沿用 alpha.3 polar 的"原始意图入 IR、编译期解析"模式）
- 选项 B：仅 React DSL prop，builder 同步算成 `position`，不进 IR
- 倾向 A：保留意图利于 codec / 编辑器 / AI 生成时的可读性；解析复杂度集中在编译期一处

**待决策点**：
- `at.direction` 集合：4 方向（above / below / left / right）→ 8 方向（再加 above-left / above-right / below-left / below-right）；TikZ `positioning` 是 8 方向
...
**Step 拆分**：
- step.1：IR schema —— `Node` 的 `position` 改成 union `Position | PolarPosition | { at: { direction, of, distance? } }`；写 zod + 派生类型
- step.2：compile —— `resolvePosition` 增加 `at` 分支：找 `nodeIndex.get(of)` 的中心 + direction 单位向量 × distance；前向引用要求 of 节点先出现（与 polar `origin` 一致）
...
**待决策点**：
- `position` 表达：方向枚举 `'above' | 'below' | 'left' | 'right' | 'above-left' | ...`（与 N1 direction 对齐）+ 数字角度 `number`（TikZ `label=30:foo` 表示中心 30° 方向）—— 用 union
...
- step.2：compile —— 在 `compileNode` 末尾按 label 列表追加 TextPrim；位置 = node 边界外 direction 方向 × (outerSep + distance)；继承 font 从 Node
...
- **N1 与现有 polar `origin`**：polar 已有 `origin?: string | Position | PolarPosition`，跟 `at: { of, direction, distance }` 表达力部分重叠（极坐标可以 `{ angle, radius, origin: 'A' }` 模拟）。决策：保留两套——polar 是任意角度 + 半径的极坐标；`at` 是 8 方向枚举的语义糖，AI 生成 / 用户写时各有取舍
