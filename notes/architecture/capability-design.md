...
| Diagram Graph Complete      | `@retikz/graph` | 提供可独立绘制并可被关系模型复用的图式语义元素                  | Graph schema、Core children       | Core IR、局部 artifact / identity           | `standard`、`core`、graph adapters、未来 Flow / Editor |
...
- `packages/diagram/_notes/architecture/diagram-graph-complete.md`（随首个 Graph ADR 建立）
...
`@retikz/graph` 是 Diagram 领域的基础语义元素层。它拥有图式中的节点、容器与局部连接语义，但不拥有 GraphModel、全局拓扑、算法布局或编辑状态。Core Sugar 直接输出 Core IR；需要局部布局、target 或 artifact 的元素通过公开 Layout / Standard / Core capability lowering。未来 Flow 或 Graph editor adapter 可以单向依赖 Graph，Graph 不反向依赖 Flow 或 Editor。
