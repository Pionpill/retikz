...
原子 schema、type、contract、Source 默认片段 与上层组合的长期规则见[`原子契约与组合设计`](./atomic-contract-design.md)。
...
`@retikz/graph` 是 Schematic 领域的通用关系与呈现基础。它长期拥有 JSON-safe 的 Entity / Relation 语义、可选 `graphDefaults` 与独立 `graphRules` 作者上下文、领域 Definition / resolve，以及直接下沉到 Core Node / Path / Scope 的 composite。Graph Source 组合完整 Core Scope public surface，保持 Core Theme、默认样式、placement、namespace 与失败语义；Graph Theme definition 生成与 Source 同形的稀疏默认，作者通过 graphDefaults 显式覆盖；语义规则独立按 role、kind、predicate、status、direction 等条件选择目标，并复用受限的 Source style / layout 字段。单例精确覆盖继续使用 Core-compatible 字段，不建立平行 Variant 轴。standalone React Graph 复用 Layout 建立 Scene，embedded Graph 只贡献 Scope。Entity 与 Relation 可独立放入任意 Core 内容树；Graph 不维护成员数据库、私有 identity 索引或 Graph-only child grammar。Relation endpoint 复用 Core NodeTarget 与 namespace，因此可以引用 Core 已公开寻址的 Node、Coordinate、resolved Scope，以及把公开 id 下沉到这些 target 的 Plot / Table 等上层 composite。位置、路径、尺寸和内容等实例输入复用对应 Core lower target 的字段与语义，不建立平行 geometry、appearance 或 reference 模型。当前 v0.1 的 Source IR 与 lower surface 正由 ADR-07～09 重新设计。未来 `@retikz/diagram` 单向依赖 Graph，拥有 LLM-first 的 Flow 高层 Source、结构化关系图的布局意图、与 Flow Source 同形的 flowDefaults 及其 Theme 生成来源、独立 routing 配置、单项 style / layout、Graph 语义投影、约束确定化、provider 编排、自动 routing 与布局调度；Flow 配置只投影 Graph element 已开放字段，不反向放宽或绕过 Graph 屏蔽项。Graph 不反向依赖 Diagram，也不拥有实例字段来源的裁决协议。`flow` 是 Diagram 的具体 drawing core，不再作为独立 package owner。Core Sugar 直接输出 Core IR，需要局部布局、target 或 artifact 的元素继续通过公开 Layout / Standard / Core capability lowering；Graph 与 Diagram 都不拥有 Editor 交互状态或 renderer。
