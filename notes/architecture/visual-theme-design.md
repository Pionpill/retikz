...
> **状态：长期架构方向已确认。** 本文定义 renderer-neutral Theme 的稀疏 Source 片段、视觉人格、覆盖规则与跨包边界。当前公开 API 以各 owner 的 Accepted ADR、代码与用户文档为准；架构方向不代表所有领域已完成迁移。
>
...
Theme 为已有语义对象提供视觉和表现布局默认。领域 Theme 按目标类型组织稀疏片段，片段内部复用对应 Source 的字段路径、值域与覆盖语义。环境选择、主题默认、实例输入与解析后的视觉结果各有独立职责。

长期架构维护模型、包边界、结构不变量与视觉人格。版本 ADR 冻结公开字段、行为、失败语义与兼容性；文件范围、测试 case、实施步骤与评审记录进入 ignored 镜像 plan。领域 Theme 的精确字段由领域 owner 决定，不构建全仓巨型 Theme schema。
...
目标是让作者在 Theme 与实例中使用同一套正式字段，同时保留内置风格、稀疏覆盖、JSON 持久化、可解释解析和 renderer 等价性。结构化本身不是目标；只有具有稳定语义的对象才分组，不按字段数量机械嵌套。
...
| owner-local Theme               | 对该领域的目标类型提供稀疏默认片段                               |
...
Theme 外层按 entity、relation、axis、legend、cell 等实际消费目标选择默认片段，不复制 entities、relations、guides 等实例数组。一个目标片段里的每条路径都必须在对应正式 Source 或命名配置契约中存在，并保持同名、同层级、同值域与同语义。

例如 Entity Source 的 `style.font` 与 `layout.align` 分别对应 Theme 的 `entity.style.font` 与 `entity.layout.align`。目标类型是额外的默认作用域；它不声称与整个 Source 根对象逐路径相同。
...
- Source 缺少可主题化片段时，由原 owner 先建立同名正式契约，再派生 Theme；不能从最终 appearance 反推平行 Source

presentation 的文本内容与区域格式应由同一正式区域对象表达；Theme 只选格式字段，省略内容不会创建该区域。Flow 文本排布进入 layout，自动路由进入 routing；Theme 不承载 routing 算法选择。

### 4.3 单一真源

持久化 Theme 使用闭合嵌套对象，复用正式片段 schema。dotted path 仅用于诊断、inspection 或工具导出，不是第二套 authored token map。旧 flat token、别名和双向 projection 在对应 owner 迁移时删除，不保留双读兼容。
...
Core 先按 Scene、外到内 Scope 确定环境与 shared colors。领域用共享值建立 baseline，再应用本地 style definition 的显式片段与 owner-local Theme。shared categorical 没有领域显式覆盖时来自 Core；领域显式 palette 高于该 baseline，实例显式 range / scheme 最终胜出。
...
每个 Theme owner 维护闭合稀疏 Source schema、Neutral baseline、现有 style definition registry 与领域 resolver。内置和宿主自定义 definition 消费同一公开契约与路径；缺少当前 style 的 owner definition 时 fail-loud，不静默改用 neutral。不新增跨 owner registry、自动发现或远程主题 loader。

解析结果按需求派生为 EffectiveXxxTheme 或携带来源的 XxxResolution，不为非持久化消费态另建 Zod schema，也不要求所有字段机械 required。环境阶段可确定的必需默认应完整；依内容、尺寸或目标上下文才能确定的字段由目标 resolver 决议，不能伪造值填满对象。
...
| Chart          | shell / presentation 与精确 recipe 允许的视觉默认；转发 Plot 公开 Theme | Plot Theme 的平行词汇、复制 Plot merge     |
...
用户可把稀疏 Theme 内联到 Source、保存为 JSON 或作为数据包共享。具名风格继续由实际消费的各 owner 同名 definition 注入；数据对象本身不承担 loader 或 registry 生命周期。
...
诊断指向实际 owner 与嵌套 Source 路径，区分环境、definition、Theme、规则及实例入口。未知字段、非法值、无效 palette、缺失 definition 与补全后的真实领域不变量由相应边界 fail-loud。
...
- Theme 片段与目标 Source 路径及值契约一致；preset、definition、inline 覆盖无平行词汇
...
新增 Theme 字段先确认正式 Source 语义、默认化资格、owner、consumer 和覆盖粒度。字段移位必须同步目标 Source、Theme、rules、definition 与文档，不从当前 token 名反推新结构。
