...
- **单 `<Step>` 组件**统一所有路径动作（12 种 `kind`：move / line / fold / cycle / curve / cubic / bend / arc / circlePath / ellipsePath / rectangle / generator），通过 `kind` discriminated union 区分。理由：组件数量越少，学习成本越低、文档越短、bug 面积越小，与 TikZ "所有动作走一种 path operation grammar" 的哲学一致
...
Tier 2 高层 IR（用户写 / LLM 生成 / 持久化形态）
   { namespace: 'plot', type: 'axis', domain: [0, 100], ticks: 5 }
       │
       ▼  compileToScene 第一步：lowerComposites（core 内部）遍历 composite 节点
       │  据 (namespace, type) 在注册表查到对应 CompositeDefinition
       │  用其 schema 精确校验字段、调 expand(node) 下沉成 Kernel node / path
       │
Kernel IR（Tier 1）
...
React JSX (含 sugar)
   <Draw way={['A', '-\|', 'B']}>
       ...
       │
Kernel JSX
   <Path><Step kind="move" to="A"/><Step kind="fold" via="-\|" to="B"/></Path>
       │
       ▼  buildIR
       │
...
core 不知道 plot / graph / circuit 存在，但留两个接缝让独立包接入：

1. **`IRComposite` passthrough 节点** —— `{ namespace, type, ...domain 字段 }`，core 只校验「有 `namespace` + `type`」（passthrough 放行领域字段），与 Tier 1 节点（无 `namespace`）互斥分流；validation 阶段不会拒绝
2. **`CompileOptions.composites: Array<CompositeDefinition>`** —— 每个 `CompositeDefinition` 由 `{ schema, expand(node) }` 组成（`schema` 用 zod `.extend(CompositeBaseSchema)` 把 `namespace`/`type` 收窄为 literal）。`compileToScene` 第一步用内部 `lowerComposites` 据 `(namespace, type)` 路由到对应 definition，校验后调 `expand` 下沉成 Tier 1 IR

```ts
// 用户用法（v0.3+）
import { plotComposites, Axis, Plot } from '@retikz/plot' // plotComposites: Array<CompositeDefinition>

<Tikz compileOptions={{ composites: plotComposites }}>
  <Axis domain={[0, 100]} ticks={5} />
  <Plot data={[...]} />
</Tikz>
```

跨 adapter 自动可用——`lowerComposites` 在 Tier 1 compile 之前跑，canvas / SSR / 任何 adapter 自动获得 Tier 2 能力，只要它们消费 core 的 `compileToScene` + 用户传同样的 `composites` 注册表。
...
- **持久化**：存的是 `{ namespace: 'plot', type: 'axis', domain: [0, 100], ticks: 5 }` 这十来字段对象，而不是展开后的 30+ 个 Kernel node / path
...
```ts
function compileToScene(ir: IR, options?: CompileOptions): Scene { /* 纯函数 */ }

type Scene = {
  primitives: ScenePrimitive[];        // 渲染顺序
  layout: { x, y, width, height };     // 自动算出或被 viewBox 覆盖
  resources?: SceneResource[];         // 渐变 / 图案 / 图片等 paint 资源
  animations?: IRAnimationTrack[];     // 根级（scene viewBox）动画
};

type ScenePrimitive =
  | { type: 'rect',    x, y, width, height, rx?, ry?, fill, stroke, ... }
  | { type: 'ellipse', cx, cy, rx, ry, fill, stroke, ... }
  | { type: 'text',    x, y, text, font, color, alignment, anchor, ... }   // 已度量
  | { type: 'path',    commands, stroke, fill, arrowStart?, arrowEnd?, marks? }
  | { type: 'group',   transforms, children, clip?, opacity?, animations? };

// path 携带结构化 commands（move/line/quad/cubic/arc/ellipseArc/close），
// 而非 SVG `d` 字符串——`d` 会让 Scene 偏向 SVG，破坏渲染目标无关性
```
...
> **2026-06-05 注**：本节是早期 projection，实际推进已超前——Canvas 与 plot **未等到 v2.x**：Canvas 在 v0.3-alpha.1 落地，包名是 **`@retikz/render/canvas`**（与 `@retikz/render/svg` 同包子路径，非独立 `@retikz/canvas`）；`@retikz/plot`（Tier 2 图表层）已起步、走独立 `plot/` 版本线（v0.1）。下面的包清单与 import 示例保留原始设想，**实际包名 / 排期以 v0.3 roadmap 与各包为准**；Node `@napi-rs/canvas` 服务端导出仍排在 v0.3-beta.1。
...
`@retikz/plot` 的底层语义预研见 [`plot-design.md`](../../packages/graph/notes/architecture/plot-design.md)：plot 以坐标系为底座，chart type 只是 coordinate / scale / encoding / mark / guide 的 preset 组合，不作为核心抽象。
...
> **2026-06-12 注**：下列若干项实际在 v0.3（当前 `@retikz/core` 0.3.0-beta.1）/ plot v0.1 已提前交付，勾选状态按现状更新。

- [x] Canvas 渲染（实为 `@retikz/render/canvas`，v0.3-alpha.1；Node `@napi-rs` 服务端导出仍待 v0.3-beta.1）
- [~] `@retikz/plot`（chart domain，比 flow 重数倍）—— 已起步，独立 `plot/` 版本线 v0.1（guide / scale / mark 闭环建设中）
- [x] animation / interactivity —— 时间轴动画（`IRAnimationTrack`：12 内置属性 + keyframe + 触发器 + 15 preset）与交互水合接缝（IR 存 `id` / 事件名，回调留运行时）均已进 core IR；a11y 仍待
...
- 本文档（core-design.md）：通读，理解所有架构决策与"为什么"
- AI 协作时把本文档喂进上下文，每次开会话提醒对方读过
