...
> 状态：Schematic 分组、Graph / Diagram 与 Editor 的长期边界已确认。Graph v0.1 的无 Variant Theme 与可选 Graph context ADR 已确认，当前实现正同步收敛；Diagram 自动布局与 Editor 仍需分别通过 milestone ADR 落地
...
- Entity / Relation 的 JSON-safe 语义契约、领域 Definition、metadata 与 Theme appearance rules
...
- 可独立放入任意 Core 内容树的 Entity / Relation 图式呈现能力

Graph 不拥有成员数据库或全局 GraphModel root。`IRGraph`、`IREntity` 与 `IRRelation` 是三类独立 Source composite：Entity / Relation 分别组合自身语义和 Core Node / Path lower-facing 字段；Graph 只组合完整 Core `IRChild` 并提供局部上下文。身份、namespace frame、重复 id 与引用解析继续由 Core 统一负责

当前 v0.1 已由 ADR-07～08 完成 Entity / Relation 单 record foundation，并由 ADR-09 设计以下长期组合边界：
...
Entity 与 Relation 保留 `namespace: 'graph'` 的 JSON-safe semantic IR。它们分别下沉为 Core Node 与 Core stroke Path；role、kind、predicate 与 Graph discriminator 在 lowering 后丢弃。Graph 下沉为普通 Core Scope，原样保留完整 Core Scope properties；不自动建立 local namespace，也不自动生成 id
...
| 包                      | 拥有                                                                                                             | 不拥有                                                        |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `@retikz/graph`         | Graph / Entity / Relation IR、领域 resolve、Graph context、Definition、provider、Core-compatible 字段与 lowering | 成员数据库、geometry 模型、Diagram 自动布局、Editor、renderer |
| `@retikz/graph-react`   | 三类 Source composite 的 JSX authoring 与宿主 adapter                                                            | Graph schema、resolve、lowering、布局算法                     |
| `@retikz/graph-vanilla` | 三类 Source composite 的 builder、normalize、InputEmbed 与 Vanilla adapter                                       | Graph schema、resolve、lowering、布局算法                     |

未来 `@retikz/diagram` package family 单向依赖 Graph，拥有 Diagram 布局意图、布局 Definition / provider、Diagram resolve、布局编排、routing 与计算结果；它不复制 Graph 的 Entity / Relation、appearance 或 Theme 契约。Graph 当前没有端口契约；Relation 只复用 Core 已公开的 NodeTarget，后续新的局部连接点能力必须先由 Core 或适用 owner 的独立设计冻结。`@retikz/diagram` 是实际的上层能力包，不是聚合入口；package family 的具体组成与公开 API 必须由自身 roadmap / ADR 确认后建立
...
Graph React / Graph Vanilla / direct IR
          │
          ▼
 Graph / Entity / Relation Source composites
          │
          ▼
...
Graph 依赖 Core、Foundation、必要的 Math 与 Layout 公开能力，不依赖 adapter、renderer、Viz、Diagram 或 Editor。Diagram 可以依赖 Graph 及 Core、Math、Layout 的公开 measurement / geometry capability。Graph React / Vanilla 只把宿主写法归一到对应 Graph / Entity / Relation Source IR；不维护私有 IR、Graph-root 成员索引、默认 id、平行 schema 或 renderer 分支

Graph Theme style 与 Core、Plot、Table 使用同一个 Core `theme.style` 名称协作。Graph 发布包只维护 Neutral baseline；React 通过 `GraphThemeProvider` 为 standalone Graph 注入 Graph-owned style definitions，embedded Graph / Entity / Relation 在 Layout 的静态 InputEmbed 提取边界显式传递同一 definitions，Vanilla 也通过显式 definition options 注入，不建立 ambient 全局 registry。Docs 可以像 Viz 一样通过公开 Definition 提供 Academic、Vibrant、Clean reference styles，并复用同一个 Preview Theme selector；Preview host 负责把同一 bundle 显式交给 embedded Graph authoring。这些参考值不成为 Graph Source enum 或发布包内置白名单
...
Graph 与 Diagram 的稳定管线只有 Data、Resolve 与 Layout 三个领域概念，不复制 Plot 的 Transform、Encoding、Scale 或 Coordinate 分层。Graph Source record 直接承载 lower target 字段；是否以及何时调用布局器由消费者决定：

```text
Graph / Entity / Relation records / Diagram input
        │
        ├── author / Editor / consumer supplies Core-compatible fields
        │
        └── Diagram resolve / layout / routing computes those fields
                    │
                    ▼
...
Graph Data 由独立 Entity / Relation record、对应 Core lower target 的实例字段与各类图自己的 JSON-safe 扩展组成。Entity / Relation resolve 负责领域 Definition、Theme、metadata 与补全后不变量；Graph Theme 只按 role、kind、predicate 与 direction 等真实语义匹配，实例 appearance 字段保持最高优先级。Relation endpoint、namespace 可见性、重复 id 与 unresolved reference 交由 Core NodeTarget / namespace 编译统一处理。Graph resolve 只确定局部 Graph context 并保留有序 children，不收集成员或建立引用索引，也不计算、合并或标记 geometry 来源

Diagram 增加 layout kind、direction、rank、order、pin、spacing、routing 等布局意图。Diagram Resolve 确定布局默认、约束与 provider；Layout 结合 Kernel 提供的测量和几何能力计算节点位置、分组边界、边线路径与标签位置。计算结果如何写入 render-ready Graph Source 或直接进入其它下游，由 Diagram 自身 ADR 决定，不反向要求 Graph 建立 geometry result contract
...
本文定义长期边界；具体字段、默认值、role 词汇、关系模型、布局约束与版本迁移进入对应 milestone ADR。Graph v0.1 已确认 ADR-06～09 的无 Variant Theme、Entity / Relation 单 record、独立 composite、可选 Graph context、Core NodeTarget endpoint 与三入口 parity，当前实现正同步收敛。`@retikz/diagram` package family、布局 provider 与结果交付必须由 Diagram roadmap / ADR 确认后建立，Editor 继续独立设计和审查
