...
export const StepLabelSchema = z.object({
  ...
  opacity: z.number().min(0).max(1).optional()
    .describe('Label-only opacity 0..1; multiplied with the owning path/node opacity (element-internal axis). Scope-level opacity does NOT compound.'),
...
4. **`opacity` 语义**：label-only opacity，与**所属元素**（path / node）的 opacity **相乘**——这是**元素内**一轴（与 NodeLabel opacity "与 node opacity 相乘" 一致）；**跨 scope 不复合**（scope 间 opacity 走覆盖，随 ADR-01 决策细节 9）。
...
| `ir/path/step.ts` | 新建字段 | `StepLabelSchema.opacity` | `z.number().min(0).max(1).optional()` | undefined | 标签独立透明度 0..1；与所属 path/node opacity 相乘（元素内轴；跨 scope 不复合） |
