...
- **coordinate prop → 字符串简写 `"polar2D"` + 可选对象** `coordinate={{ type:'polar2D', innerRadius, startAngle, endAngle }}`：简单默认 + 可配；缺省 cartesian（向后兼容）。
...
<Plot data={metrics} coordinate="polar2D">
...
- `buildPlotSpec polar`：`<Plot coordinate="polar2D">` + marks → 产出含 `coordinate.type='polar2D'` 的 PlotSpec。
...
- `polar 对象配置`：`coordinate={{ type:'polar2D', innerRadius:0.3 }}` → 环图 IR。
