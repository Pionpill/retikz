...
> 4 ADR 全部 **Accepted + 已实现**（Spec-First TDD：tests-first → 实现 → adversarial bug hunt + contract audit；BLOCKING「ternary 和上溢」已修并升回归）。三包 + 文档四方一致、milestone 可交付。决策点 ①~⑦ + ①b 已在各 ADR「决策」段落定。
