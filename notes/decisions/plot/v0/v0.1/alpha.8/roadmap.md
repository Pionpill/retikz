...
> 关联：[`plot v0.1 roadmap`](../roadmap.md) · [`plot v0 roadmap`](../../roadmap.md)（阶段二 alpha.8）· [`plot-design §3.4 Scale / §3.9 Guide`](../../../../../architecture/plot-design.md) · [`_template.md`](../../../_template.md) · 上个 milestone：[`v0.1-alpha.7`](../alpha.7/roadmap.md)
...
1. **把 color 通道从「仅 categorical → ordinal」扩到连续域**：补 **连续色阶**（sequential / diverging gradient）+ **离散化 scale**（quantize / threshold / quantile），了结 alpha.7 留下的「continuous / temporal `color.field` → fail-loud」债（[alpha.7 ADR-03](../alpha.7/03-color-series.md) 明确「连续色阶留 alpha.8」）。
...
现状（alpha.1~7，见 [上轮](../alpha.7/roadmap.md) + 代码核验）：
