...
| ADR | 主题 | Level | 依赖 | 状态 |
|---|---|---|---|---|
| [01](./01-scope-id-meta.md) | scope-aware id 绑定 + meta 透传（mark → `Scope.id`、line/area series → `Path.id`、datum opt-in → `Node.id` + 各级 `meta`；`<plotId>.` plot-local 前缀；`provenance` 总开关默认关→等价 alpha.4、per-datum + datumIdField 独立 opt-in；datum index = transformed/source；抽出可复用 frame；react/vanilla 转发 option） | red | core ADR-08（已回灌）| Proposed |
| [02](./02-datum-locator.md) | datum locator 命中预演（逻辑地址 → 位置/元素的确定性正向解析纯函数，复用 01 frame，不逐点预注册；与 lowering 产物位置一致性断言）| red | ADR-01 | Proposed |
