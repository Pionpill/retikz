...
> ✅ **2026-06-07 alpha.5 封口**：ADR-01/02 全部 `Accepted`；绿灯关（`@retikz/plot` 362 测试 + `plot-react` 59 + `plot-vanilla` 10 + 三包 tsc + eslint + docs build）全过；Bug Hunter（修 datumProvenance 蕴含 bug + 3 回归）与 Contract Auditor（四方对账无 BLOCKER）两关已留痕；changelog（三包，双语）已入库。暂不发版。
>
