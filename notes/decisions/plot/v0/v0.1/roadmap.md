...
| v0.1-alpha.9  | **Coordinates 坐标系统**        | **linear1D**（单轴；rug / timeline / histogram 底座）+ **ternary2D**（a+b+c=1 重心投影，取 continuous 字段自归一化；proportion 已并入 continuous）；1D 轴 / 三角轴 guide；guide dimension 按坐标系校验（拒非法维度）。**地图坐标不进 plot**（§2 独立 domain）；**3D gating 于 core 三维坐标**       | [`v0.1-alpha.9/`](./v0.1-alpha.9/roadmap.md)（roadmap 落定，4 ADR 待起草） |
