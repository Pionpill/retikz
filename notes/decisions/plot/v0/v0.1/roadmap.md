...
| v0.1-alpha.11 | **Geometry 基础**               | mark 补 **rect**（heatmap 格）+ **rule**（参考 / 阈值线）+ **text**（datum label）+ **ribbon**（sankey / alluvial 流量、跨 scope connector）。并落地**区间类几何的坐标系无关下沉**（interval / rect / sector 共用，含曲线 / 自定义坐标系）——契约与决策见表后「区间几何下沉」备注。**依赖 core 补 `contour` shape**（ADR `ADR-core-contour-shape.md`，已送 core 分支） | 待建                                                                   |
...
- **sector 间隔**（见 alpha.4 [ADR-02](./alpha.4/02-sector-geometry.md) 讨论）— *【alpha.13】*：`padAngle`(+ `padRadius`) 角向间隔——放 sector / interval mark 层（rose 复用 band `paddingInner`），含小扇形 clamp；per-datum explode / pull 单片高亮（用户「内部小圆位移圆心」想法的归宿）
