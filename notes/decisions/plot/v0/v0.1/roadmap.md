...
- **legend.scale 指向错类型 scale 不 fail-loud**（alpha.8 ADR-03 adversarial WARNING）— *【需求驱动 / legend 加固】*：`<Legend channel="color" scale="x" />` 把 color 图例绑到位置 linear scale（类型不符）时，`resolveColorLegend` 不做类型守卫、静默落 ordinal 退化（不崩、产合法 swatch），与 ADR-03 决策⑥「fail-loud」略松。补 legend.scale 与 channel 的类型一致性校验。落点 `lower/guide.ts` 的 legend 解析
