...
| v0.1-alpha.7 | **Aesthetics 上 + Scales 家族** | 通道×scale 通用抽象（推广出 position）；**size** 通道（仅 PointMark，radius scale）+ size scale；**color 从分组转真 scale 通道** + **series 一等化**（了结 color→series 重构债、修单系列 color field 静默丢弃）；scale 家族 log / pow / sqrt（仅 point/line） | [`v0.1-alpha.7/`](./v0.1-alpha.7/roadmap.md)（决策已拍板，ADR 待起草） |
