...
```ts
// packages/plot/plot/src/lower/lowerPlots.ts
...
`packages/plot/plot/tests/lower/lowerPlots.test.ts`(新建)端到端 + 单元覆盖：lower 产出 core IR 结构 / `compileToScene` 出 Scene / domain 推断 / 常量通道 / 路径顺序 / ref 缺失报错。具体见「实现契约 § 测试象限」。

## 影响

- **`packages/plot/plot/src/lower/*`**(全新)：`lowerPlots` + 投影 / scale / 字段解析 helper。
...
`red`（动 `packages/plot/plot/src/lower/**` + `src/index.ts`）。
...
- `packages/plot/plot/src/lower/field.ts`（新建：数据解析——字段路径 / 通道取值 / 排序）
- `packages/plot/plot/src/lower/scale.ts`（新建：归一化——线性映射 + domain/range 求解）
- `packages/plot/plot/src/lower/project.ts`（新建：投影——坐标系把通道值 → [x,y]）
- `packages/plot/plot/src/lower/mark.ts`（新建：mark 几何——point→Node / line→Path）
- `packages/plot/plot/src/lower/lowerPlots.ts`（新建：编排 + 入口 + defineComposite）
- `packages/plot/plot/src/lower/index.ts`（新建：barrel）
- `packages/plot/plot/src/index.ts`（修改：转出 lower）
- `packages/plot/plot/tests/lower/lowerPlots.test.ts`（新建）
