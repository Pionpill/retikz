...
DSL / API 两套表面（IR `{value}`/`{field}` 互斥；React `<RuleMark>` 扁平 props，数字→value、字符串→field）的完整示例见文档站 `apps/docs/src/contents/plot/components/mark/rule/`（zh / en）。
...
> 🔖 本文件压缩前完整施工蓝图 = `git show 6902289a:notes/decisions/plot/v0/v0.1/alpha.11/03-rule-mark.md`（封板全文）。
