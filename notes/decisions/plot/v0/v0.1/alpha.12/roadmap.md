...
| [10](./10-channel-registry.md) | **visual channel registry（内部收敛）**：补齐 ADR-07 留下的「channel 实绘 = color」缺口——size/opacity/shape/strokeWidth 仍 hardcode 在 `channel.ts` 的 `makeXxxResolver`、绕过 registry。新增 `VisualChannelDefinition` + `defineVisualChannel` + **内部** registry（照 ADR-03 内部收敛、不开放公开扩展）：scale 管 domain→归一化数学（通道无关）、visual channel 管输出空间（判别 union）+ 默认 scale + 范围 + 字段规则 + legend 形态；`ChannelScaleResolution.of` 放宽 `string→ScalarValue`、`ordinal` 调色板泛型化（color/shape 共用）；内置视觉通道降为注册项、legend 按 channel def 形态分派。对齐 Vega-Lite「channel HAS scale」。**yellow / 无 IR 改动**。**不在范围（后续 ADR，red）**：公开 `defineVisualChannel` 注入口 + 全新通道 IR encoding passthrough（encoding 闭合具名集需加 channels map）/ 彻底解耦（scale→t、跨通道复用）/ React 糖。 | Proposed（未排期；落地节点待定） |
