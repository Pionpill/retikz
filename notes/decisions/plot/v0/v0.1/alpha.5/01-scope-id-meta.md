...
- 状态：Accepted
- 决策日期：2026-06-07
- 关联：[plot v0.1-alpha.5 roadmap](./roadmap.md) · [plot-design.md §7 anchor / §8.1 id 绑定与可连接性](../../../../../architecture/plot-design.md) · **core 依赖**：[core v0.3-alpha.4 ADR-08 meta provenance](../../../../core/v0/v0.3/alpha.4/08-meta-provenance.md)（已回灌 next-plot）· **预留来源**：[plot v0.1-alpha.1 ADR-01 根节点](../alpha.1/01-plot-spec-root.md)（`id` / `meta` 字段位埋点）
...
2. **meta（不可连接的来源标签）**——core v0.3-alpha.4 [ADR-08](../../../../core/v0/v0.3/alpha.4/08-meta-provenance.md) 新增的 `Node` / `Scope` / `Path` `meta`，compile 原样 stamp 进 Scene 图元、renderer 忽略。下沉元素写 meta ⇒ 交互层命中图元即可读 `prim.meta` 反查来源。零命名空间代价（meta 不进 nodeIndex），是 v3 hit-test 的**主通道**。
