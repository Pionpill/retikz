...
import { TICK_LENGTH, LABEL_GAP } from './layout'; // 复用 ADR-03 常量
...
| guide | 几何 | 位置 |
|---|---|---|
| axis x | 轴线 Path（plot area 底边）；每 tick 一段刻度线 Path（向下 `TICK_LENGTH`）；label Node(text)：`position=(tickX, 轴底 + TICK_LENGTH + LABEL_GAP + fontSize/2)`——水平居中于 tickX **天然成立**（text 居中于 center），垂直用 `fontSize` 估高下移 | 底边 `y = plotArea.y + plotArea.height` |
| axis y | 轴线 Path（plot area 左边）；刻度线向左；label Node(text)：`position=(轴左 - TICK_LENGTH - LABEL_GAP - 估算半宽, tickY)`——垂直居中于 tickY **天然成立**，水平用「字符数 × fontSize × CHAR_WIDTH_FACTOR / 2」估半宽左移 | 左边 `x = plotArea.x` |
...
样式上提：axis 层 `pathDefault:{ stroke:'currentColor' }` + `nodeDefault:{ /* 纯文字：无 shape 描边/填充 */, fontSize }`；grid 层 `pathDefault:{ stroke:'currentColor', strokeOpacity: <浅> }`（网格淡于轴）。

```ts
// packages/plot/plot/src/lower/expand.ts（编排，z-order：grid → marks → axis）
const grids = (node.guides ?? []).filter(g => g.type === 'grid').map(g => lowerGuide(g, ctx));
const axes  = (node.guides ?? []).filter(g => g.type === 'axis').map(g => lowerGuide(g, ctx));
const markLayers = node.marks.map(m => lowerMark(m, rows, project));
const children = [...grids, ...markLayers, ...axes].filter(Boolean);
...
5. **anchor id 预留**：axis 层 scope 带 `id`（`plot.xAxis` 等），alpha.5 datum/region 命中直接用，alpha.2 只埋不解析（延续 alpha.1）。
...
- **core 纯文字 Node 表达**：label 要「只文字、无 shape 描边填充」。core `Node.shape` 可选——实现前确认省略 shape 时是否只渲文字无框（查 `packages/core/core/src/ir/node.ts` + 渲染路径）；否则用描边/填充为 none 的最小形状。不在本 ADR 拍死。
- **grid 淡化方式**：`strokeOpacity` 浅 vs 固定浅色。倾向 `strokeOpacity`（随 currentColor 主题走，不写死颜色）。具体值（如 0.15）待调。
...
- `guide_styles_hoisted`：轴线 stroke / label fontSize 在层 scope 的 pathDefault/nodeDefault，不逐图元重复
- `axis_id_to_scope_id`：`{type:'axis',dimension:'x',id:'xAxis'}` → 该层 scope.id === 'xAxis'
