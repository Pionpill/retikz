...
mark 投影从「整图 range」改为「plot area range」（改 alpha.1 [ADR-06](../v0.1-alpha.1/06-plot-lowering.md) 的 `expand.ts`）：利用 d3-scale 可分步设的特性——先按 domain 建 `scaleLinear` → `scaleTicks(scale)` 拿 labels（ticks 只依赖 domain + count、不依赖 range）→ 估算 margin → 得 plot area → `scale.range([...plot area 边])` 定 range → 投影 mark 与 guide。

**guide 的轴线/网格框取 scale 实际 range（自测补强）**：guide 的轴线、刻度、网格不直接用 margin 算出的 plotArea 边，而是用 `xScale.range()` / `yScale.range()` 的**实际输出区间**派生一个 frame。无显式 range 时二者相同；当用户给某 scale 显式 `range`（不被 plot area 覆盖）时，轴线/网格随实际绘制区走，与刻度 / mark 严格对齐，不因显式 range 而错位（见 [ADR-04](./04-guide-lowering.md) GuideContext）。
...
- **margin 之和 ≥ 尺寸（退化）**：定为**抛清晰错误**（plotArea 非正会一路出 `cx="NaN"`/负坐标坏图，与 alpha.1 [W1](../v0.1-alpha.1/roadmap.md) 的 width/height 校验同思路、同一致性）。备选静默夹到最小正值——但会出退化但「看似正常」的图，掩盖配置错误，不取。**用户 margin 还逐边校验有限非负**（自测补强）：`NaN`（`NaN <= 0` 为 false，会漏过尺寸守卫）/ 负值（让 plot area 比画布还宽、图元跑出画布）同样抛清晰错误，不静默出坏坐标。
...
- **`packages/plot/plot/src/lower/scale.ts`**（由 [ADR-02](./02-d3-scale.md) 重构为 d3-scale，本 ADR 不重复改）：本 ADR 只在 `expand.ts` 里用 d3 scale 的「先按 domain 建 `scaleLinear` → 算 plot area → `scale.range([...plot area 边])`」分步设 range。domain 推断（d3 `extent`，空集 `[0,1]`）与既有 `domain_inferred_from_data` / `explicit_domain_range_respected` / 投影向后兼容测试由 [ADR-02](./02-d3-scale.md) 守。
...
- [ADR-02 `scaleTicks`](./02-d3-scale.md)（`packages/plot/plot/src/lower/scale.ts`，基于 d3-scale）—— **消费**：估算 margin 前先 `scaleTicks(scale)` 拿 label。
