...
> 路径假设实例位于 `notes/decisions/kernel/<MAJOR>/<MAJOR>.<MINOR>/<MAJOR>.<MINOR>-channel.N/<NN>-...md`，模板里的相对链接按此位置写（在模板自身处链接会断，cp 到实例位置后才正确）
...
`packages/kernel/core/tests/<对应路径>.test.ts` 覆盖：
