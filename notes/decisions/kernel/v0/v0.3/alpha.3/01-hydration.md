...
- 决策日期：2026-06-04
- 关联：[v0.3 roadmap §水合 / §动画 / §Alpha 切分 alpha.3](../roadmap.md) · [core-design.md §4.4 IR 100% 可序列化](../../../../../architecture/core-design.md) · [alpha.1 ADR-03 vanilla runtime / 依赖图](../alpha.1/03-vanilla-runtime-and-dependency-graph.md)
...
- **`react ⊥ vanilla`**（[alpha.1 ADR-03](../alpha.1/03-vanilla-runtime-and-dependency-graph.md)）——两者唯一共同依赖是 `@retikz/render`，共享水合 runtime 只能落那里（不能落 vanilla）。
...
> - 完整施工契约（Schema 改动表 / 文件 scope / 测试象限 16 case / 依赖现有元素）：`git show 9f795047:notes/decisions/core/v0/v0.3/alpha.3/01-hydration.md`。

> 🔖 封板压缩 commit `a5a1bd80`；压缩前完整施工蓝图 = `git show a5a1bd80^:notes/decisions/core/v0/v0.3/alpha.3/01-hydration.md`。
