...
7. **全量验收 + ADR 落档**：core / react / docs 三包测试全绿；ADR 集合（`notes/decisions/kernel/v0/v0.2/alpha.7/`）固化全部待定项；pattern / image 留**顺延 ADR 占位**（不实现）。
...
开工前另起（`notes/decisions/kernel/v0/v0.2/alpha.7/`，编号到时定），固化上节待定项 + 落交付物：
