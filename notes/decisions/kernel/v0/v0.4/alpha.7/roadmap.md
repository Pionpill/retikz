...
| ADR | 状态 | 主题 | 说明 |
| --- | --- | --- | --- |
| ADR-01 | 待起草 | Provider registry contract | 统一 array 输入、Map resolve、builtin-first、duplicate throw、unknown 诊断、测试矩阵与错误消息格式 |
| ADR-02 | 待起草 | Provider key contract | 区分 string reference provider 与 operation provider；明确 `name`、schema literal、`namespace.type` 三种 key 来源 |
| ADR-03 | 待起草 | Capability migration | 迁移 shape / arrow / pattern / path generator / path kind / ribbon width profile / composite 到统一 provider 模型 |
| ADR-04 | 待起草 | Adapter surface and docs | React / Vanilla provider 透传改成数组定义；文档新增 provider authoring 总览与扩展示例 |
