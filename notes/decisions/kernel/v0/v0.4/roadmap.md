...
| alpha.6          | Ribbon 可变宽度路径（plot 桑基图底座）                   | plot→core 请求     | [roadmap](./alpha.6/roadmap.md) · [ADR-01](./alpha.6/01-ribbon.md)                                                                                         | Proposed（新增 core `ribbon` IR，compile lower 为闭合 `PathPrim`；文档落 `core/components/draw/ribbon`） |
| 暂不             | 交互行为 / 跨框架 runtime / 伪三维                        | C / G / P3D        | —                                                                                                                                                         | ⏸（P3D 详见下文「伪三维视角层」）                                                                         |

**建议实现顺序**：alpha.1 **A（math 底座）** → alpha.2 **scope 多态 bbox（消费 A）+ 可嵌入 Tier2**（已实现发布）→ alpha.3 **B（路径补强，消费 A）** → alpha.4 **F（Scene 视觉，独立、视余量）** → alpha.5 **E（tex，行内 `$...$` text+math 混排）**。（D eval 已交其他分支，不在本路线）
