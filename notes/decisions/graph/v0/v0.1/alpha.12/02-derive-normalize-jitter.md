...
- `apps/docs/src/contents/graph/.../transform`（修改：normalize / derive-interval / jitter 双语 mdx 章节——经 ADR-01 `<Transform>` / `<Plot transforms>` 编写，jitter demo 用连续数值字段，百分比堆叠不带 `<BarMark stack>`）
- `apps/docs/src/contents/graph/.../*.demo.tsx`（新建：百分比堆叠（显式 `[normalize, stack]`，无 mark auto-stack）/ 甘特区间 / jitter 连续数值散点 demo）
