...
- **docs grammar 分组落点**：interval 页是否含子页（bar / radial / pie / heatmap 各 SubPage），还是单页多 demo？倾向**单页多 demo 起步**（受 `data/graph.ts` 两层结构约束），增长后再升组。
...
- `apps/docs/src/contents/graph/grammar/**`（修改 / 新建：抽象 mark 页 + 图表示例改写，zh/en）
- `apps/docs/src/contents/graph/**` 的 `data/graph.ts` + i18n（修改：contents + data + i18n 同步）
...
- `apps/docs` grammar contents / `data/graph.ts` / i18n —— 修改：按抽象数据几何重组 + 图表示例改写
