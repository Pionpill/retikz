...
- `geometry/` 七模块：`point`（向量 + `dot`/`cross` 新增 + `shiftToward` + `lerp`，迁自 core point/edge）、`transform`（迁自 core）、`arc`（全文件迁自 core）、`intersect`（contour 私有求交提出 + `segmentSegment` 新增 + `rayArc` 转出）、`triangle`（incircle/circumcircle 新增）、`polygon.containsPoint`（新增）、`hull.convexHull`（新增）。
