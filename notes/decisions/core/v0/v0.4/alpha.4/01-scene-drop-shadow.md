...
- 决策日期：2026-06-16
- 关联：[v0.4-alpha.4 roadmap](./roadmap.md) · [v0.4 roadmap 候选 F](../roadmap.md) · [core-design.md §7 AI 一等公民](../../../../../architecture/core-design.md) · [ADR-02 blend mode（同享接线骨架）](./02-blend-mode.md)
...
react kernel `<Node shadow>` / `<Path shadow>` 与 vanilla `node()` / `draw()` config（经 `Omit<IRNode>` / `Omit<IRPath>` 自动派生 `shadow`，同 react 一份 schema）皆支持预设字符串与对象两种写法。落地用法 / demo 见文档站 `apps/docs/src/contents/core/components/effects/shadow/`（zh/en + presets / object / primary-only 三组 demo）。
...
> 🔖 本文件压缩前完整施工蓝图 = `git show 6902289a:notes/decisions/core/v0/v0.4/alpha.4/01-scene-drop-shadow.md`（封板全文）。
