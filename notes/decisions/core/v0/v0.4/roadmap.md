...
- A 的核心待决：math 包到底装什么、与 core `geometry/` 如何拆、是否并入 render 的 oklch 色彩算 / 动画插值等数值算（拆分方向已在 [alpha.1 ADR-01](./alpha.1/01-math-package-pure-geometry-sink.md) 拍板）。
...
| --- | --- | --- | --- | --- |
| alpha.1 | `@retikz/math` + core 纯几何下沉 | A | [ADR-01](./alpha.1/01-math-package-pure-geometry-sink.md) | Accepted（设计已拍板，待 plan） |
...
### A · `@retikz/math`（2026-06-12 拍板；2026-06-13 深化见 [alpha.1 ADR-01](./alpha.1/01-math-package-pure-geometry-sink.md)）

> 2026-06-13 深化：依赖方向定为 **core → math**（core 可正向依赖 math），且**首切就把 core 纯几何下沉 math**（按函数粒度切）、core 改 re-export 保公开面。下方原 2026-06-12 结论中「core geometry 留在 core 不迁移」「短期两个家」已被推翻，以 [`alpha.1/01-math-package-pure-geometry-sink.md`](./alpha.1/01-math-package-pure-geometry-sink.md) 为准。

- **定位**：新增·独立纯计算包；**零依赖、零 IR、零 zod**；一律纯函数 + 普通对象 / `Position` 数组，**不写 class**（对齐 core 几何约定）。
- **与 core 的关系**：依赖方向 **core → math**（正向，允许）；math 不反依赖 core。首切即把 core `geometry/` 的纯计算（向量运算 / 仿射变换 / lerp / 私有求交原语 / `arc.ts`）下沉 math，core 从 math re-export，「两个几何的家」消除。详见 [ADR](./alpha.1/01-math-package-pure-geometry-sink.md)。
