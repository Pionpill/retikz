...
| alpha.3          | 路径补强（圆角 B1 + 平滑曲线 B2）+ 任意轮廓 contour shape | B + plot→core 请求 | [ADR-01](./alpha.3/01-polyline-rounded-corners.md) · [ADR-02](./alpha.3/02-smooth-curve-through-points.md) · [ADR-03](./alpha.3/03-core-contour-shape.md) | Proposed / 实现契约齐 + 多 LLM 评审已合并（B 消费 A；contour 使能 plot alpha.11；下「后续讨论结论 · B」） |
