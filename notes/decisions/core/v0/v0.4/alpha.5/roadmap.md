...
| # | 子项 | 代号 | ADR | 状态 |
|---|---|---|---|---|
| E1 | `@retikz/tex` 包 + `lowerMath` 引擎 + `node.math` 内容（独立公式 A） | E | [ADR-01](./01-tex-package-and-node-math.md) | Proposed（route 2 定稿；引入 `IRNode.math` + `CompileOptions.lowerMath` + Layout 通道，统一内容模型） |
| E2 | 带框公式（B）——`node.math` + 任意 shape 容器 | E | [ADR-02](./02-node-embedded-math.md) | Proposed（依赖 E1；复用内容路径，几乎零增量，黄级） |
| E3 | 行内 text+math 混排（C）——`IRLineSpec` run 序列 + `$...$` | E | [ADR-03](./03-inline-math-runs.md) | Proposed（最重；扩 text-run 模型 + gated `$...$`，依赖 E1 注入） |
