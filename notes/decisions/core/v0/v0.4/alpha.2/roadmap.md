...
| E | 可嵌入 Tier2 in `<Layout>` | [ADR-01](./01-embeddable-tier2-in-layout.md) | 实现中（2026-06-15 人工签字 #4=组件静态属性） |
...
实现中。外部 LLM 评审已过（2026-06-13，6 findings 全采纳）；适配器注册形态 2026-06-15 人工签字 = **组件静态属性**（MVP），`<Layout embeddables>` 显式 prop 作可选逃生舱。core-react 机制（builder + collect-hydration-handlers + Layout + `EmbeddableTier2Adapter` 接口）按 ADR-01「影响」段落落地；plot 侧 `<Plot>` 嵌入态归 plot ADR-02 L2-a，不在本切片。实现计划见 `docs/superpowers/plans/`。
