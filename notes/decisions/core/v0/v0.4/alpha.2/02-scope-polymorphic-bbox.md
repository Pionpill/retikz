...
- 状态：Accepted（MVP = rectangle + circle，2026-06-15 已实现，待 changelog 封口；polygon / ellipse 缓做。详见下「实现决策」）
- 记录日期：2026-06-13
...
> 本文件原为 v0.4 候选方向设计 note；2026-06-14/15 按 alpha.2 MVP 落地为闭集枚举 `rectangle | circle`，非法值由 schema 拒绝。
...
ShapeRegistry 于 [v0.2-alpha.3](../../v0.2/alpha.3/01-shape-registry.md) 落地，但本次实现没有把 `boundingShape` 设计成开放 ShapeRegistry 引用。原因是「包络任意子树点集」需要逐形状的最小外接算法，不能复用普通 Node shape 的开放注册表。
...
## 方向：`scope.id` 的 synthetic layout 包络形状可多态，经闭集算法生成对应 shape layout

- 给 `<Scope>` 加一个**可选**包络形状入口：`boundingShape?: 'rectangle' | 'circle'`。缺省 `'rectangle'` → **逐字回退现状、向后兼容**。
...
- synthetic layout 用对应 shape 构造，外部 `name.north` / `name.30` / `boundaryPoint` 解析走与「同形状普通 Node」**完全一致**的路径——延续 ADR-03「无新 anchor 代码路径」的设计取向。
...
- **一字段一职责**：包络形状走 `boundingShape`（闭集包络算法名），不与 `clip`（裁剪区，渲染语义）混用——两者形状可不同、职责不同。
...
- `polygon` 包络：需要显式顶点凸多边形 ShapeDefinition 与凸包 boundaryPoint。
- `ellipse` 包络：需要轴对齐外接椭圆或最小面积椭圆算法。
...
- **字段形态**：`boundingShape?: 'rectangle' | 'circle'`，闭集枚举；缺省 → rectangle，逐字回退现状。**padding 本轮不做**（延后项保留）。
...
- **未支持的 boundingShape**（polygon / ellipse / 未知名）：schema 在 parse 边界直接拒绝，compile 只处理合法枚举值，不提供 compile 阶段 fallback。
...
alpha.2 MVP 已落地：`@retikz/math` 提供 `minimalEnclosingCircle`，core schema 接受 `boundingShape='rectangle' | 'circle'`，compile 用圆形 MEC 构造 synthetic ellipse(`circumscribe:'equal'`) layout，Scope 文档与 demo 已同步。
