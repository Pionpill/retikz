...
- **08 完全正交于 shape 线**：`meta` provenance 透传与形状几何无关，纯沿 [alpha.3 水合](../v0.3-alpha.3/01-hydration.md)的 `id`-stamp 通路加一个兄弟字段（Node/Scope/Path 加 `meta` → compile 同点 stamp 进 Scene 图元 → renderer 忽略）。plot 把 datum/series/layer 来源带进 Scene 供交互命中的 core 侧能力；不依赖 01-07，可独立实现 / 排期。
