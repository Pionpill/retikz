# ADR-06：连接面与视觉形状解耦——`connectAs`（node 默认）+ edge `boundary`（单边覆盖），复用 shape 注册表借边界
...
| connectAs 取值 | boundary 类（自动求交 + 数字角度 anchor） | compass（north/east…·compile 层统一） | 形状专属锚点（tip-N / outer-arc-mid） | edgePoint |
|---|---|---|---|---|
...
7. **anchor cache key 纳入连接面**：现有 `anchor-cache.ts` 缓存 key 是 `(layout, anchorName)`；boundary 类 + compass 类的解析结果随 `boundary ?? connectAs` 变，故 **surface-dependent 解析的 cache key 须加入连接面判别**（如 `${surfaceKey}|${anchorName}`，`surfaceKey` = `'shape'` / `'circle'` / 借用 shape 的稳定序列化）。surface-independent 的解析（专属命名 anchor、`edgePoint`）key 不含 surface、维持现状。防「同节点同 `north` 在 circle / shape 两条边串缓存」。

### IR 侧（`connectAs` 进 Node、`boundary` 进端点引用，均进 IR、JSON 可序列化）

```ts
// packages/core/core/src/ir/connectSurface.ts（新建）—— 复用 ShapeRefSchema，保留字编译期消解
export const ConnectSurfaceSchema = z
...
// packages/core/core/src/ir/path/target.ts —— NodeTargetSchema 加 boundary（单边覆盖 node.connectAs）
...
```ts
// packages/core/core/src/ir/connectSurface.ts
...
export type ConnectSurfaceKeyword = ValueOf<typeof ConnectSurface>;
```

### 编译期分派

边界 / compass anchor 解析处，按 `edge.boundary ?? node.connectAs ?? 'shape'` 选定连接面，再分派：

```ts
const surface = target.boundary ?? node.connectAs ?? ConnectSurface.Self;
// 选 def：
//   'shape'  → 视觉形状自身 def（boundary 类现状路径不变）
//   'circle' → 真圆：把 AABB squared 到 max 半轴（hw=hh=max）后跑 ellipse def
//   其余     → normalizeShape(surface) → lookupShape(type) → 用本节点 layout AABB（含 circumscribeOffset）当 rect

// 按 anchor 种类路由（compass 提到 compile 层、不再进各 shape.anchor）：
//   boundary 类（auto-clip toward / 数字角度 anchor） → surfaceDef.boundaryPoint(AABB, toward)
//   compass 类（9 个 rect 方位名）                    → surfaceDef.anchor(AABB, name)   ['shape' 等同 'rectangle']
//   形状专属命名 anchor（tip-N / outer-arc-mid …）    → 视觉形状 def.anchor（恒不借用）
//   edgePoint（{ side, t }）                          → 视觉形状 def.edgePoint（恒不借用）

// cache：surface-dependent（boundary 类 + compass 类）key 含 surfaceKey；
//        surface-independent（专属 anchor / edgePoint）key 不含 surface
```
...
- **保留字撞名**：用户经 `CompileOptions.shapes` 注册名为 `shape` / `circle` 的形状时，连接面语义优先保留字、不进借用。ADR 声明二者为连接面保留字，注册同名 shape 仍可作视觉 `Node.shape`（那条路径不查保留字），仅不能经 `connectAs` 借用。

## DSL 表面（react + vanilla 双示例）

```tsx
// react —— node 设默认连接面（五角星，但默认连成圆）
<Node id="star" shape={{ type: 'star', params: { points: 5, innerRadius: 20, outerRadius: 50 } }} connectAs="circle" />
// 默认（连到该节点）即走 circle；单条 edge 破例连真实尖端：
<Path from={{ id: 'star', boundary: 'shape' }} to={[120, 0]} />
// 借矩形连接面
<Node id="box-like" shape={{ type: 'polygon', params: { sides: 6 } }} connectAs="rectangle" />
```

```ts
// vanilla builder —— 同一份 IR（node + draw(way, config?)，无 path() builder）
node('star', { shape: { type: 'star', params: { points: 5, innerRadius: 20, outerRadius: 50 } }, connectAs: 'circle', position: [0, 0] });
// 默认连到该节点即走 circle：
draw(['star', [120, 0]]);
...
`packages/core/core/tests/shapes/connect-surface.test.ts`（新建）+ `tests/compile/node-shape.test.ts`（扩）+ `tests/ir/path-target.test.ts`（扩）覆盖：

- 三类取值解析（保留字 `shape` / `circle`、内置名 `rectangle` / `ellipse`、`{type, params}` 借用、自定义注册名借用）
- **`connectAs:'shape'` boundary 类 + 专属锚点 + edgePoint 等价回归**（star/sector/arc/4 内置形状的 `boundaryPoint`、数字角度、专属 anchor、`edgePoint` 全等现状）
...
- edge `boundary` 覆盖 node `connectAs`；都省略 = `'shape'`
- **boundary 字段 toward-less no-op**：`between` 端点 / `offset.of` 引用里带 `boundary` 取中心时被忽略、不报错；但同处显式 `anchor:'north'` + `boundary:'ellipse'` 仍落椭圆
- **anchor cache 不串扰**：同节点同 `north`，一条边 `connectAs:'circle'`、另一条 `boundary:'shape'`，两次解析互不污染（surface 进 cache key）
- layout-neutral：改 `connectAs` 不改 viewBox / scope bbox
- 借用形状不调其 `circumscribe`
- 保留字优先级（注册同名 `circle` shape 时 `connectAs:'circle'` 仍真圆）
- rotate / scale × 连接面交互；IR round-trip 自描述

## 影响

- **`packages/core/core/src/ir/connectSurface.ts`**（新建）：`ConnectSurfaceSchema` + `ConnectSurface` 常量 + `ConnectSurfaceKeyword`。
- **`packages/core/core/src/ir/node.ts`**（修改）：`NodeSchema` 加 `connectAs`（缺省 `'shape'`）。
- **`packages/core/core/src/ir/path/target.ts`**（修改）：`NodeTargetSchema` 加 `boundary`。
- **`packages/core/core/src/ir/index.ts`**（修改）：导出 `ConnectSurfaceSchema` / `ConnectSurface`。
- **`packages/core/core/src/compile/**`**（修改）：① boundary 类（auto-clip + 数字角度）/ compass 类解析按 `boundary ?? connectAs ?? 'shape'` 选连接面 def——借内置 `rectangle` / `ellipse` + `'circle'` squared 特例；专属锚点 / edgePoint 恒走视觉形状。② **compass 9 名上提到 compile 层统一走 AABB**（从各 shape.anchor 抽出）。③ `boundary` 在 `refPointOfTarget`（`compile/**/anchor.ts`）的 toward-less 取中心路径 = no-op。
- **`packages/core/core/src/compile/**/anchor-cache.ts`**（修改）：surface-dependent 解析 cache key 加入 surfaceKey；surface-independent 维持现状。防 per-edge 串扰。
...
- **`packages/core/core/src/index.ts`**（修改）：导出 `ConnectSurface` / `ConnectSurfaceSchema`。
- **对外 API**：`NodeSchema` additive（`connectAs` 可选）；端点引用 additive（`boundary` 可选）。boundary 类 / 专属 anchor / edgePoint 默认逐字段等价现状；**sector/arc compass 为新增行为**（之前 throw）。整体 **非 BREAKING**（无现有合法用法被破坏）。
- **render / vanilla**：emit 不变 → 渲染零改动；连接点解析在 core，runtime 透明。
- **文档**：shape reference 页补「连接面 connectAs」+ path 连接页补端点 `boundary`；`<ComponentPreview>` 演示星形 connectAs="circle" 与 edge boundary 覆盖。
...
判级：动 `packages/core/core/src/ir/**`（node / path target / 新 connectSurface schema）+ `src/compile/**`（连接面分派）→ red。

### Schema 改动

| 文件 | 操作 | 字段名 | 类型 | 默认值 | describe 中文摘要 |
|---|---|---|---|---|---|
| `src/ir/connectSurface.ts` | 新建 schema | `ConnectSurfaceSchema` | `z.union([z.string().min(1), ShapeRefSchema])` | — | 连接面：保留字 shape/circle 或借用已注册 shape，layout-neutral |
| `src/ir/connectSurface.ts` | 新建常量 | `ConnectSurface` | `{ Self:'shape', Circle:'circle' } as const` | — | 连接面保留关键字（编译期消解） |
| `src/ir/node.ts` | 加字段 | `connectAs` | `ConnectSurfaceSchema.optional()` | `'shape'`（编译期） | 节点默认连接面，edge 可覆盖 |
| `src/ir/path/target.ts` | 加字段 | `boundary` | `ConnectSurfaceSchema.optional()` | 省略 = node.connectAs | 单边覆盖连接面；仅 path 端点 clip + 该端点 compass/角度生效，toward-less 引用 no-op |

### 文件 scope

- `packages/core/core/src/ir/connectSurface.ts`（新建）
- `packages/core/core/src/ir/node.ts`（修改：connectAs）
...
- `packages/core/core/src/compile/**/anchor-cache.ts`（修改：cache key 纳入 surfaceKey）
- `packages/core/core/src/shapes/star.ts`（修改：删 compass fallback，compile 层接管）
- `packages/core/core/src/index.ts`（修改：导出 ConnectSurface / ConnectSurfaceSchema）
- `packages/core/core/tests/shapes/connect-surface.test.ts`（新建）
...
- `connect_self_default_equivalent`：省略 `connectAs` ≡ `'shape'`，boundaryPoint / compass / 专属锚点 / edgePoint 逐字段等现状（快照回归）
- `connect_as_rectangle_borrows_aabb`：星形 `connectAs:'rectangle'` → boundaryPoint 落 AABB 矩形边
- `connect_as_ellipse_borrows_aspect`：`connectAs:'ellipse'` → 按 AABB 宽高比椭圆求交
- `connect_as_circle_true_circle`：`connectAs:'circle'` → r = max 半轴真圆
- `sector_compass_additive_aabb`：sector `north`（现状 throw）→ compile 层统一后返回 AABB 方位点（additive 新增行为）

**边界（≥ 2）**：

- `edge_boundary_overrides_node`：node `connectAs:'circle'` + edge `boundary:'shape'` → 该边连真实形状、其余边连圆
- `both_omitted_is_self`：node / edge 都省略 → `'shape'`
- `circle_keyword_beats_registered_circle`：注册同名 `circle` shape 时 `connectAs:'circle'` 仍真圆（保留字优先）
- `boundary_noop_in_between`：`between` / `offset.of` 端点带 `boundary` 取中心 → 忽略不报错；同处 `anchor:'north'`+`boundary:'ellipse'` → 落椭圆

**错误路径（≥ 2）**：

- `connect_as_unregistered_throws`：`connectAs:'nope'`（非保留字、未注册）→ 编译期 throw
- `connect_as_bad_params_rejected`：`connectAs:{type:'sector', params:{innerRadius:'a'}}` → 借用形状 paramsSchema reject

**交互（≥ 2）**：

- `specific_anchor_ignores_surface`：`connectAs:'circle'` + `anchor:'tip-0'` → 仍解析到星形尖端（专属锚点不受连接面）
- `edge_point_ignores_surface`：`connectAs:'circle'` + `anchor:{side,t}` → edgePoint 走视觉形状
- `compass_follows_surface`：`connectAs:'ellipse'` 的 `north` 落椭圆、`'shape'`/`'rectangle'` 的 `north` 落 AABB、`'circle'` 落真圆
- `angle_anchor_follows_surface`：`anchor:30` 属 boundary 类——`connectAs:'circle'` 落真圆、`'shape'` 落真实轮廓
- `star_compass_equals_today`：star `north` 在 compile 层统一后 = 迁移前 AABB 值（回归）
- `cache_no_cross_pollination`：同 layout 同 `north`，`connectAs:'circle'` 与 `boundary:'shape'` 两路解析不串（surface 进 key）
- `layout_neutral`：改 `connectAs` 前后 viewBox / scope bbox 不变
- `surface_with_rotate_scale`：连接面 × node rotate / scale 协同正确
- `roundtrip_self_describing`：含 connectAs / boundary 的 IR → JSON → parse → 等价
...
- `resolveAnchor` / `angleBoundaryOf` / `anchorOf` / cache（`src/compile/**/anchor-cache.ts` + `compile/node`）—— **修改**：cache key 纳入 surfaceKey；数字角度 anchor（boundary 类）按连接面选 def；防 per-edge 串扰。
