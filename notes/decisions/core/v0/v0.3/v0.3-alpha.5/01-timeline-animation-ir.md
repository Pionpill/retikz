...
|---|---|---|---|
| `src/ir/animation.ts` | 新建 | `AnimationProperty`（内置通道，含 `scaleX`/`scaleY` 非均匀缩放）/ `AnimationEasing` / `AnimationDirection` / `AnimationFill` 常量 + `BuiltinAnimationProperty` / `AnimationPropertyRef`（开放）；`EasingSchema`（具名∪bezier∪自定义名）/ `OriginSchema`（命名 anchor ∪ `[x,y]` 局部点）/ `KeyframeSchema` / `TriggerSchema` / `AnimationTrackSchema`（`property` 开放 string + `iterations` + `origin`（transform 支点）+ `.refine` keyframes 升序 + `.superRefine` 内置 property↔value 类型）+ `IRAnimationTrack` / `IRKeyframe` / `IRAnimationTrigger` / `IRAnimationOrigin` | 时间轴动画 track（renderer 无关、JSON、无函数、property/easing 可扩展） |
...
**非均匀缩放 + origin（≥2）**：`scaleY` + `origin:'south'`（柱子从基线长出）track → 图元原样透传含 `origin`；`scaleX/scaleY` value 须 number、`origin` 接受命名 anchor 或 `[x,y]`、拒空串 / 长度≠2。
