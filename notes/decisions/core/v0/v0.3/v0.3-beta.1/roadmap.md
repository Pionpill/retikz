...
| 7 | `outerSep` 对齐 TikZ outer sep（parity，⚠️ breaking on `margin>0`） | visible | 中 | 📝 草案 | border 类 anchor（compass / 角度 / 自动连线）统一外扩 margin 并计入布局占位；缺省 0 故零 margin 用法不变。red 级。多 LLM 评审已合并、子决策已落定；待人工板定后进实现。 |
...
| [07](./07-outer-sep-tikz-parity.md) | TODO-7 | `outerSep` 对齐 TikZ outer sep（外推 border anchor + 计入占位） | Proposed |
