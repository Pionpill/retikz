...
- \`\`\`retikz-tsx\`\`\`：JSX，仅允许 18 个 retikz 组件 — Kernel \`Layout\` / \`Node\` / \`Path\` / \`Step\` / \`Text\` / \`Coordinate\` / \`Scope\`，Sugar \`Draw\` / \`EdgeLabel\` + 形状 \`Circle\` / \`Ellipse\` / \`Arc\` / \`Sector\` / \`Rectangle\` / \`RegularPolygon\` / \`Star\`，以及 Standard Tier 2 \`Grid\` / \`Axes\`；props 只能是字面量（字符串 / 数字 / 布尔 / null / 数组字面量 / 对象字面量）；**禁止** 变量引用、表达式、\`.map()\`、hooks、模板插值、spread。
...
\`<Circle center={[0,0]} radius={20} />\` · \`<Ellipse center={[0,0]} radius={{ x: 30, y: 15 }} />\` · \`<Arc center={[0,0]} radius={20} startAngle={0} endAngle={90} />\` · \`<Sector center={[0,0]} radius={20} startAngle={0} endAngle={60} fill="#eee" />\` · \`<Rectangle corner1={[0,0]} corner2={[40,20]} cornerRadius={4} />\` · \`<Grid bounds={{ min: [0,0], max: [60,40] }} spacing={10} />\` · \`<Axes bounds={{ x: { min: -60, max: 60 }, y: { min: -40, max: 40 } }} ticks={{ x: 10, y: 10 }} />\` · \`<RegularPolygon center={[0,0]} radius={25} sides={6} />\` · \`<Star center={[0,0]} outerRadius={25} points={5} fill="#fbbf24" />\`。Circle/Ellipse 带 \`startAngle\`+\`endAngle\` 画部分弧；视觉 prop（fill/stroke/...）与 Path 一致。**需算坐标的形态**（Circle from/to、bbox corner、Rectangle center+宽高、Sector、Grid、Axes、RegularPolygon、Star）点位只接 literal \`[x, y]\`。
...
- \`\`\`retikz-tsx\`\`\`: JSX, only the 18 retikz components allowed — Kernel \`Layout\` / \`Node\` / \`Path\` / \`Step\` / \`Text\` / \`Coordinate\` / \`Scope\`, Sugar \`Draw\` / \`EdgeLabel\` + shapes \`Circle\` / \`Ellipse\` / \`Arc\` / \`Sector\` / \`Rectangle\` / \`RegularPolygon\` / \`Star\`, plus Standard Tier 2 \`Grid\` / \`Axes\`. Props must be literals (string / number / boolean / null / array literal / object literal). **No** variable references, expressions, \`.map()\`, hooks, template interpolation, or spread.
...
\`<Circle center={[0,0]} radius={20} />\` · \`<Ellipse center={[0,0]} radius={{ x: 30, y: 15 }} />\` · \`<Arc center={[0,0]} radius={20} startAngle={0} endAngle={90} />\` · \`<Sector center={[0,0]} radius={20} startAngle={0} endAngle={60} fill="#eee" />\` · \`<Rectangle corner1={[0,0]} corner2={[40,20]} cornerRadius={4} />\` · \`<Grid bounds={{ min: [0,0], max: [60,40] }} spacing={10} />\` · \`<Axes bounds={{ x: { min: -60, max: 60 }, y: { min: -40, max: 40 } }} ticks={{ x: 10, y: 10 }} />\` · \`<RegularPolygon center={[0,0]} radius={25} sides={6} />\` · \`<Star center={[0,0]} outerRadius={25} points={5} fill="#fbbf24" />\`. Circle/Ellipse with \`startAngle\`+\`endAngle\` draw a partial arc; visual props (fill/stroke/...) match Path. **Forms that compute coordinates** (Circle from/to, bbox corners, Rectangle center+size, Sector, Grid, Axes, RegularPolygon, Star) only accept literal \`[x, y]\` points.
