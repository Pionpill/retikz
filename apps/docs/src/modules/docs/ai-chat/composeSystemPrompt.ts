...
- \`\`\`retikz-tsx\`\`\`：JSX，仅允许 19 个 retikz 组件 — Kernel \`Layout\` / \`Node\` / \`Path\` / \`Step\` / \`Text\` / \`Coordinate\` / \`Scope\`，Sugar \`Draw\` / \`EdgeLabel\` + 形状 \`Circle\` / \`Ellipse\` / \`Arc\` / \`Sector\` / \`Rectangle\` / \`RegularPolygon\` / \`Star\`，以及 Standard Tier 2 \`Grid\` / \`Axes\` / \`Frame\`；props 只能是字面量（字符串 / 数字 / 布尔 / null / 数组字面量 / 对象字面量）；**禁止** 变量引用、表达式、\`.map()\`、hooks、模板插值、spread。
...
Step = {
  ...
  to?: [x, y] | { id, anchor?, offset? },  // 节点/Coordinate 引用用对象；anchor = 'top' | 30 | { side, fraction }；写 JSX 时可用 'A.top' 字符串简写（react 层转对象）
...
\`<Circle center={[0,0]} radius={20} />\` · \`<Ellipse center={[0,0]} radius={{ x: 30, y: 15 }} />\` · \`<Arc center={[0,0]} radius={20} startAngle={0} endAngle={90} />\` · \`<Sector center={[0,0]} radius={20} startAngle={0} endAngle={60} fill="#eee" />\` · \`<Rectangle corner1={[0,0]} corner2={[40,20]} cornerRadius={4} />\` · \`<Grid bounds={{ min: [0,0], max: [60,40] }} spacing={10} />\` · \`<Axes origin={[80,60]} extent={{ x: 60, y: 40 }} x={{ ticks: { source: { kind: 'spacing', spacing: 10 } } }} />\` · \`<Frame id="group" label="Group"><Node position={[0,0]} text="A" /></Frame>\` · \`<RegularPolygon center={[0,0]} radius={25} sides={6} />\` · \`<Star center={[0,0]} outerRadius={25} points={5} fill="#fbbf24" />\`。Frame 只接受直接 Node children。Axes 的 y 正值只在自身 lowering 中视觉向上，不改变其它 Core Position 仍为 y-down。Circle/Ellipse 带 \`startAngle\`+\`endAngle\` 画部分弧；视觉 prop（fill/stroke/...）与 Path 一致。**需算坐标的形态**（Circle from/to、bbox corner、Rectangle center+宽高、Sector、Grid、Axes、RegularPolygon、Star）点位只接 literal \`[x, y]\`。
...
- \`\`\`retikz-tsx\`\`\`: JSX, only the 19 retikz components allowed — Kernel \`Layout\` / \`Node\` / \`Path\` / \`Step\` / \`Text\` / \`Coordinate\` / \`Scope\`, Sugar \`Draw\` / \`EdgeLabel\` + shapes \`Circle\` / \`Ellipse\` / \`Arc\` / \`Sector\` / \`Rectangle\` / \`RegularPolygon\` / \`Star\`, plus Standard Tier 2 \`Grid\` / \`Axes\` / \`Frame\`. Props must be literals (string / number / boolean / null / array literal / object literal). **No** variable references, expressions, \`.map()\`, hooks, template interpolation, or spread.
...
Step = {
  type: 'step',
  kind: 'move' | 'line' | 'arc' | 'circlePath' | 'quad' | 'cubic' | 'ellipseArc' | 'cycle',
  to?: [x, y] | { id, anchor?, offset? },  // node/Coordinate references use the object form; anchor = 'top' | 30 | { side, fraction }; in JSX the 'A.top' string shorthand works (react converts to object)
...
\`<Circle center={[0,0]} radius={20} />\` · \`<Ellipse center={[0,0]} radius={{ x: 30, y: 15 }} />\` · \`<Arc center={[0,0]} radius={20} startAngle={0} endAngle={90} />\` · \`<Sector center={[0,0]} radius={20} startAngle={0} endAngle={60} fill="#eee" />\` · \`<Rectangle corner1={[0,0]} corner2={[40,20]} cornerRadius={4} />\` · \`<Grid bounds={{ min: [0,0], max: [60,40] }} spacing={10} />\` · \`<Axes origin={[80,60]} extent={{ x: 60, y: 40 }} x={{ ticks: { source: { kind: 'spacing', spacing: 10 } } }} />\` · \`<Frame id="group" label="Group"><Node position={[0,0]} text="A" /></Frame>\` · \`<RegularPolygon center={[0,0]} radius={25} sides={6} />\` · \`<Star center={[0,0]} outerRadius={25} points={5} fill="#fbbf24" />\`. Frame only accepts direct Node children. Positive y values point visually up only inside Axes lowering; other Core Positions remain y-down. Circle/Ellipse with \`startAngle\`+\`endAngle\` draw a partial arc; visual props (fill/stroke/...) match Path. **Forms that compute coordinates** (Circle from/to, bbox corners, Rectangle center+size, Sector, Grid, Axes, RegularPolygon, Star) only accept literal \`[x, y]\` points.
