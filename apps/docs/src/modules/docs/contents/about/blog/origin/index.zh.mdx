...
这是 [karl-circle 示例](/kernel/examples/karl-circle)的最终图。示例页把它拆成 7 步演进，从一个圆开始一步步搭出来。
...
任何 demo 卡片右上角的 **Ask AI** 都把当前图喂给站内 AI——把 [karl-circle 示例](/kernel/examples/karl-circle) 的 prompt 加一句「画齐六个三角函数：把 sec / csc / cot 也按几何定义画出来」，模型在原图基础上加一条切线（同时承担 sec / csc 两段）+ 一条顶部水平切线（cot），右侧信息框扩到 6 个公式：

<ComponentPreview files="unit-circle-six" hideCode size="xxl" />

[AI 专家路线图示例](/kernel/examples/learning-path) 则是另一个综合例——按坐标 1:1 复刻 [AMAI-GmbH/AI-Expert-Roadmap](https://github.com/AMAI-GmbH/AI-Expert-Roadmap) 的 intro.svg 封面，演示 RoadmapNode 抽象 + AtPosition 紧贴堆叠 + Draw way 的 line / curve / dashPattern / arrow / label 一套组合：
...
- [karl-circle 示例](/kernel/examples/karl-circle)：v0.1 标志性例子的 7 步演进
