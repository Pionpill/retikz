...
这是 [karl-circle 示例](/kernel/galleries/karl-circle)的最终图。示例页把它拆成 7 步演进，从一个圆开始一步步搭出来。
...
| 包             | 职责                                                             | 状态                                 |
| -------------- | ---------------------------------------------------------------- | ------------------------------------ |
| `@retikz/core` | 通用图元（Node / Path / Step / Anchor）<br />+ IR + Scene 编译器 | v0.1 稳定版<br />（API 遵循 SemVer） |
| `@retikz/flow` | 流程图 / UML 等 DAG 类高层组件<br />+ 布局算法（dagre / elkjs）  | 规划中                               |
| `@retikz/plot` | 数据驱动的图表能力（chart）                                      | 规划中                               |

在 v0.1 起步时，底层之外还只露出一层 framework adapter，把 IR 翻成具体框架的元素——当时先发布 `@retikz/react`，Canvas / SSR 与无框架入口还在路线图里。到 v0.3，渲染层已收敛到 `@retikz/render`，并补出 `@retikz/vanilla`；flow / plot 等 domain 包仍按独立方向演进。
...
完整的架构原则（IR 居中 / Sugar 不引入新能力 / Scene 渲染目标无关 / domain 包扩展点等）见 [retikz 核心理念](/about/blog/core-philosophy)。
...
任何 demo 卡片右上角的 **Ask AI** 都把当前图喂给站内 AI——把 [karl-circle 示例](/kernel/galleries/karl-circle) 的 prompt 加一句「画齐六个三角函数：把 sec / csc / cot 也按几何定义画出来」，模型在原图基础上加一条切线（同时承担 sec / csc 两段）+ 一条顶部水平切线（cot），右侧信息框扩到 6 个公式：

<ComponentPreview files="unit-circle-six" hideCode size="xxl" />

[AI 专家路线图示例](/kernel/galleries/learning-path) 则是另一个综合例——按坐标 1:1 复刻 [AMAI-GmbH/AI-Expert-Roadmap](https://github.com/AMAI-GmbH/AI-Expert-Roadmap) 的 intro.svg 封面，演示 RoadmapNode 抽象 + AtPosition 紧贴堆叠 + Draw way 的 line / curve / dashPattern / arrow / label 一套组合：
...
每个版本各有命题——v0.2 加 Scope 与 Shape Registry，v0.3 推进 renderer/runtime、Canvas 与动画运行时，v0.4 再评估增量渲染、Progressive IR 与更复杂的 decorations，1.x 之后继续扩展 domain 包。每一步把 AI 适配做扎实。
...
- [karl-circle 示例](/kernel/galleries/karl-circle)：v0.1 标志性例子的 7 步演进
...
- [retikz 核心理念](/about/blog/core-philosophy)：完整的架构原则（待发布）
