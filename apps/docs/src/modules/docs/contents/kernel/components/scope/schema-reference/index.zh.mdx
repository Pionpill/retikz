...
`ScopeSchema` 是 Tier 1 的容器节点。它通过可选 `frame` 绘制独立包络装饰，并给一组 children 建立局部变换、样式默认值、裁剪、层级与引用边界；Scope 本身仍可通过 `id` 被引用。
...
外框只记录装饰间距与独立样式；不接受 id、事件或独立动画。

<ZodSchema
  name="ScopeFrameSchema"
  shallow
