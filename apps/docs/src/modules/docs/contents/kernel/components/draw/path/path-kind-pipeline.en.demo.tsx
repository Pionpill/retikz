import type { FC } from 'react';

import { Draw, Layout, Node } from '@retikz/react';

/** Path kind provider 从组件输入分派到 Scene 输出的逻辑图 */
const Demo: FC = () => (
  <Layout>
    <Node
      id="path-input"
      position={[-300, 0]}
      cornerRadius={4}
      style={{ stroke: 'dimgray', fill: 'lightgray', fillOpacity: 0.16, font: { size: 14 } }}
    >
      Path + Step
    </Node>
    <Node
      id="kind-registry"
      position={[-135, 0]}
      cornerRadius={4}
      style={{ stroke: 'dodgerblue', fill: 'dodgerblue', fillOpacity: 0.08, font: { size: 14, weight: 'bold' } }}
    >
      kind registry
    </Node>
    <Node
      id="stroke-provider"
      position={[65, -78]}
      cornerRadius={4}
      style={{ stroke: 'darkorange', fill: 'darkorange', fillOpacity: 0.1, font: { size: 14 } }}
    >
      stroke: commands + decor
    </Node>
    <Node
      id="ribbon-provider"
      position={[65, 0]}
      cornerRadius={4}
      style={{ stroke: 'darkorange', fill: 'darkorange', fillOpacity: 0.1, font: { size: 14 } }}
    >
      optional ribbon: width → outline
    </Node>
    <Node
      id="custom-provider"
      position={[65, 78]}
      cornerRadius={4}
      style={{ stroke: 'darkviolet', fill: 'darkviolet', fillOpacity: 0.08, font: { size: 14 } }}
    >
      custom: own output
    </Node>
    <Node
      id="scene-output"
      position={[300, 0]}
      cornerRadius={4}
      style={{ stroke: 'dimgray', fill: 'lightgray', fillOpacity: 0.16, font: { size: 14 } }}
    >
      Scene primitives
    </Node>

    <Draw
      way={[
        'path-input',
        { label: { text: 'kind', side: 'top', sloped: true, textColor: 'gray', font: { size: 12 } } },
        'kind-registry',
      ]}
      arrow="->"
      style={{ stroke: 'gray' }}
    />
    <Draw way={['kind-registry', 'stroke-provider']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['kind-registry', 'ribbon-provider']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['kind-registry', 'custom-provider']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['stroke-provider', 'scene-output']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['ribbon-provider', 'scene-output']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['custom-provider', 'scene-output']} arrow="->" style={{ stroke: 'gray' }} />
  </Layout>
);

export default Demo;
