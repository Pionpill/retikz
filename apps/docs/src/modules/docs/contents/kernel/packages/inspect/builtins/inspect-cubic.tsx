...
const preview = defineControlledBuiltinInspectPreview(
  ...
  values => {
    return (
      <InspectLayout registry={registry} viewBox={{ x: -120, y: -115, width: 240, height: 230 }}>
        <Scope transforms={[{ kind: 'translate', x: values.position[0], y: values.position[1] }]}>
          ...
          >
            ...
            <Step kind="cubic" control1={values.control1} control2={values.control2} to={values.end} />
          </InspectPath>
        </Scope>
      </InspectLayout>
    );
