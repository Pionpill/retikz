...
import { integrationI18n } from './blend-integration.i18n';
...
const IntegrationDemo: FC<IntegrationDemoProps> = props => {
  ...
  return (
    <Layout viewBox={{ x: -65, y: -65, width: 330, height: 155 }}>
      ...
      <Scope
        defaults={{
          node: { style: { blendMode: 'multiply', fill: 'dodgerblue', fillOpacity: 0.65 } },
          path: { style: { blendMode: 'multiply', fill: 'darkorange', fillOpacity: 0.65 } },
        }}
      >
        ...
        <Path>
          <Step kind="move" to={[-10, 35]} />
          <Step kind="line" to={[35, -40]} />
          <Step kind="line" to={[75, 35]} />
          <Step kind="cycle" />
        </Path>
        <Node position={[200, 0]} layout={{ minimumSize: 75 }} style={{ blendMode: 'normal', fill: 'darkorange' }}>
          {text.overridden}
        </Node>
      </Scope>
    </Layout>
