/** Vega barley 数据中的明尼苏达试验地点 */
export type StripVegaBarleySite = 'University Farm' | 'Waseca' | 'Morris' | 'Crookston' | 'Grand Rapids' | 'Duluth';

/** Strip Chart 进阶示例中的大麦产量观测 */
export type StripVegaBarleyDatum = {
  /** 大麦产量 */
  yield: number;
  /** 大麦品种 */
  variety: string;
  ...
  year: 1931 | 1932;
  /** 明尼苏达试验地点 */
  site: StripVegaBarleySite;
};

/**
 * Vega Datasets barley 的 120 行静态快照
 *
 * @remarks 数据来自 Minnesota Agricultural Experiment Station 在六个地点开展的
 * 大麦品种试验，每个地点包含 10 个品种在 1931 与 1932 年的 20 条观测。
 * 快照取自 vega-datasets main，原数据见 USDA Technical Bulletin No. 735
 */
export const stripVegaBarleyData: Array<StripVegaBarleyDatum> = [
