import type { FC } from 'react';

import { Draw, Layout, Node, Text } from '@retikz/react';

/**
 * 通道绑定与处理规则汇入统一解析器的英文插图
 * @description IRPlot 的位置绑定进入坐标投影，其余绑定与内置 / 自定义通道规则在解析器处汇合
 */
const Demo: FC = () => (
  <Layout>
    <Node
      id="binding"
      position={[-145, -105]}
      cornerRadius={4}
      style={{ stroke: 'darkorange', fill: 'darkorange', fillOpacity: 0.08 }}
      layout={{ minimumSize: { width: 118, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>IRPlot binding</Text>
      <Text fill="gray" font={{ size: 12 }}>
        field / constant
      </Text>
    </Node>
    <Node
      id="builtin"
      position={[0, -105]}
      cornerRadius={4}
      style={{ stroke: 'dodgerblue', fill: 'dodgerblue', fillOpacity: 0.08 }}
      layout={{ minimumSize: { width: 118, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Built-in channels</Text>
      <Text fill="gray" font={{ size: 12 }}>
        built-in rules
      </Text>
    </Node>
    <Node
      id="custom"
      position={[145, -105]}
      cornerRadius={4}
      style={{ stroke: 'dodgerblue', fill: 'dodgerblue', fillOpacity: 0.08 }}
      layout={{ minimumSize: { width: 118, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Custom channels</Text>
      <Text fill="gray" font={{ size: 12 }}>
        channelDefinitions
      </Text>
    </Node>
    <Node
      id="registry"
      position={[72, -25]}
      cornerRadius={4}
      style={{ stroke: 'dodgerblue', fill: 'dodgerblue', fillOpacity: 0.08 }}
      layout={{ minimumSize: { width: 132, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Channel registry</Text>
      <Text fill="gray" font={{ size: 12 }}>
        merge by channel name
      </Text>
    </Node>
    <Node
      id="resolver"
      position={[72, 55]}
      cornerRadius={4}
      style={{ stroke: 'darkviolet', fill: 'darkviolet', fillOpacity: 0.07 }}
      layout={{ minimumSize: { width: 132, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Resolve binding</Text>
      <Text fill="gray" font={{ size: 11 }}>
        dispatch by channel type
      </Text>
    </Node>
    <Node
      id="position"
      position={[-145, 55]}
      cornerRadius={4}
      style={{ stroke: 'gray', fill: 'gray', fillOpacity: 0.06 }}
      layout={{ minimumSize: { width: 148, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Coordinate projection</Text>
      <Text fill="gray" font={{ size: 12 }}>
        x / y / z → screen
      </Text>
    </Node>
    <Node
      id="delivery"
      position={[0, 145]}
      cornerRadius={4}
      style={{ stroke: 'gray', fill: 'gray', fillOpacity: 0.06 }}
      layout={{ minimumSize: { width: 118, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Visible properties</Text>
      <Text fill="gray" font={{ size: 12 }}>
        mark / container / node / path
      </Text>
    </Node>
    <Node
      id="descriptor"
      position={[145, 145]}
      cornerRadius={4}
      style={{ stroke: 'gray', fill: 'gray', fillOpacity: 0.06 }}
      layout={{ minimumSize: { width: 138, height: 54 }, align: 'middle', lineHeight: 16 }}
    >
      <Text font={{ size: 14, weight: 'bold' }}>Legend information</Text>
      <Text fill="gray" font={{ size: 12 }}>
        legend input
      </Text>
    </Node>

    <Draw way={['builtin', 'registry']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['custom', 'registry']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['binding', 'position']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['binding', 'resolver']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['registry', 'resolver']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['resolver', 'delivery']} arrow="->" style={{ stroke: 'gray' }} />
    <Draw way={['resolver', 'descriptor']} arrow="->" style={{ stroke: 'gray' }} />
  </Layout>
);

export default Demo;
