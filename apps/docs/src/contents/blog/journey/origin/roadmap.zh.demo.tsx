import type { FC } from 'react';
import { Draw, Layout, Node } from '@retikz/react';


const Demo: FC = () => (
  <Layout width={640} height={160} style={{ maxWidth: '100%', height: 'auto' }}>
    <Node id="v01" position={[-250, -45]} stroke="none">v0.1（当前）</Node>
    <Node id="v02" position={[-150, -45]} stroke="none">v0.2</Node>
    <Node id="v03" position={[-50, -45]} stroke="none">v0.3</Node>
    <Node id="v04" position={[50, -45]} stroke="none">v0.4</Node>
    <Node id="v10" position={[150, -45]} stroke="none">v1.0</Node>
    <Node id="future" position={[250, -45]} stroke="none">未来</Node>

    <Draw way={['v01', 'v02']} arrow="->" stroke="gray" />
    <Draw way={['v02', 'v03']} arrow="->" stroke="gray" />
    <Draw way={['v03', 'v04']} arrow="->" stroke="gray" />
    <Draw way={['v04', 'v10']} arrow="->" stroke="gray" />
    <Draw way={['v10', 'future']} arrow="->" stroke="gray" />

    <Node position={[-250, 10]} stroke="none">基础图元</Node>
    <Node position={[-150, 10]} stroke="none">Scope + Shape</Node>
    <Node position={[-50, 10]} stroke="none">高级定位</Node>
    <Node position={[50, 10]} stroke="none">libraries</Node>
    <Node position={[150, 10]} stroke="none">+ canvas</Node>
    <Node position={[250, 10]} stroke="none">+ plot</Node>
  </Layout>
);

export default Demo;
