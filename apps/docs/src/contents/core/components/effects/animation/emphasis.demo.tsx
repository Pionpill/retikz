import { Layout, Node, blink, colorShift, flash, loop, pulse, slideIn, spin, wiggle } from '@retikz/react';
...
const Demo: FC = () => (
  <Layout width={460} height={210}>
    ...
    <Node id="spin" position={[-150, 55]} shape="rectangle" minimumWidth={56} minimumHeight={16} fill="#6366f1" animations={[spin()]} />
    {/* loop：把一次性 slideIn 变成往返无限循环 */}
    <Node id="loop" position={[20, 55]} fill="#0ea5e9" animations={[loop(slideIn({ axis: 'x', offset: -40 }), { direction: 'alternate' })]}>
      loop
    </Node>
    {/* colorShift：纯形状的 fill 在 oklch 空间从红渐变到蓝（末帧 = base 色） */}
    <Node id="color" position={[160, 55]} shape="rectangle" minimumWidth={48} minimumHeight={36} fill="#3b82f6" animations={[colorShift({ from: '#ef4444', to: '#3b82f6' })]} />
  </Layout>
);
