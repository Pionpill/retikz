const translations: Record<string, string> = {
  ...
  'false 关闭该 Inspector，true 使用默认 options，对象提供 sparse options':
    'false disables this Inspector, true uses default options, and an object supplies sparse options',
  '请求生效的 scene、subtree 或 self 范围': 'Scene, subtree, or self scope where the request applies',
  '封锁目标范围内的全部 Inspector，并阻止后代规则重新开启':
    'Blocks all Inspectors in the target scope and prevents descendant rules from re-enabling them',
  '只能作用于整张 scene 或 subtree 的封锁范围': 'Barrier scope restricted to an entire scene or subtree',
  '通过 JSX 声明的图形内容': 'Drawing content declared through JSX',
  'SVG 或 Canvas CSS 高度；缺省按内容尺寸计算，单轴数值尺寸按内容比例补齐另一轴，CSS 字符串尺寸由浏览器排版':
    'SVG or Canvas CSS height; defaults to content size, a single numeric dimension determines the other from the content aspect ratio, and CSS strings are laid out by the browser',
  'SVG 资源 id 前缀；省略时由 React useId 生成': 'SVG resource ID prefix; generated by React useId when omitted',
  '显式视框，使用绘图坐标；优先于 ir.viewBox，省略时沿用场景视框，场景未指定时按内容计算':
    'Explicit view box in drawing coordinates; takes precedence over ir.viewBox, otherwise inherits the scene view box or computes it from content when absent',
  'SVG 或 Canvas CSS 宽度；缺省按内容尺寸计算，单轴数值尺寸按内容比例补齐另一轴，CSS 字符串尺寸由浏览器排版':
    'SVG or Canvas CSS width; defaults to content size, a single numeric dimension determines the other from the content aspect ratio, and CSS strings are laid out by the browser',
  '直接传入 ir 时使用的事件处理函数表': 'Event handler map used when ir is supplied directly',
  编译附加产物成功提交后的通知: 'Notification after compilation artifacts are successfully committed',
  '是否播放动画；由动画模式上下文优先决定，未指定时遵循系统减少动态效果偏好':
    'Whether to play animations; animation-mode context takes priority, otherwise follows the system reduced-motion preference when unspecified',
  可动画属性注册表: 'Animatable property registry',
  动画缓动函数注册表: 'Animation easing function registry',
  '动画采样时刻，单位为毫秒；指定后定格在该时刻，不播放动画，优先于 animate':
    'Animation sample time in milliseconds; freezes at that time without playback and takes precedence over animate',
  '选择 retained 增量更新或 static 完整编译模式；省略时使用 retained':
    'Selects retained incremental updates or static full compilation; defaults to retained',
  请求生成的编译附加产物: 'Compilation artifacts requested for generation',

  '使用默认错误码创建 Inspect 错误': 'Creates an Inspect error with the default error code',
  '使用结构化参数创建 Inspect 错误': 'Creates an Inspect error from structured options',
  '使用显式错误码创建 Inspect 错误': 'Creates an Inspect error with an explicit error code',

  选择整张主图: 'Selects the whole primary scene',
  选择指定来源的子树: 'Selects the subtree at the given source',
  '目标 Scope 在 Source IR 中的路径': 'Path of the target Scope in Source IR',
  仅选择来源对应的最终实例: 'Selects only final occurrences from the source',
  '按来源路径定位实例；occurrenceIndex 省略时选择该来源与所属者下的全部实例':
    'Locates occurrences by source path; omitting occurrenceIndex selects all occurrences for that source and owner',
  选择规则准入或匹配阶段: 'Selection admission or matching stage',
  '失败规则在 rules 中的零基索引': 'Zero-based index of the failing rule in rules',
  '已识别的目标；目标无效或不可恢复时为 null': 'Recognized target; null when invalid or unavailable',
  '被观察对象解析或 Inspector 回调阶段': 'Observed-subject parsing or Inspector callback stage',
  回调输出准入或辅助片段编译阶段: 'Callback-output admission or auxiliary-fragment compilation stage',
  '产生当前诊断的 Inspector 键': 'Inspector key associated with the diagnostic',
  被观察实例的所属者: 'Owner of the observed occurrence',
  产生诊断的最终实例定位信息: 'Final occurrence locator associated with the diagnostic',
  回调输出中的零基序号: 'Zero-based index in the callback output',
  辅助观测结果汇总阶段: 'Inspection-result completion stage',

  '按注册顺序提供的 Inspector 定义；空数组创建空注册表':
    'Inspector definitions in registration order; an empty array creates an empty registry',
  '独立的只读注册表，定义在注册时补齐缺省选项并冻结':
    'An independent readonly registry whose definitions receive missing option defaults and are frozen during registration',
  '定义的 namespace、type 或所属者标识为空白，或存在重复 namespace/type 时抛出 RetikzInspectError':
    'Throws RetikzInspectError for blank definition namespace, type, or owner identifiers, or duplicate namespace/type keys',
  '按合并顺序传入的注册表；不传参数时创建空注册表':
    'Registries in merge order; omitting all arguments creates an empty registry',
  '包含各输入注册表定义的新只读注册表，不修改输入':
    'A new readonly registry containing the input definitions without modifying the inputs',
  '不同注册表包含相同 namespace/type 时抛出 RetikzInspectError，不覆盖先前定义':
    'Throws RetikzInspectError when registries contain the same namespace/type; earlier definitions are not overwritten',
  '追加在五类内置 Inspector 之后的自定义定义，省略时仅注册内置定义':
    'Custom definitions appended after the five built-in Inspectors; omission registers only the built-ins',
  包含内置与自定义定义的新只读注册表: 'A new readonly registry containing built-in and custom definitions',
  '自定义定义无效或与内置、自定义定义的 namespace/type 重复时抛出 RetikzInspectError':
    'Throws RetikzInspectError for invalid custom definitions or namespace/type keys duplicated among built-in or custom definitions',
  '待校验的 Inspector 定义；省略选项 schema 时仅接受空对象，省略 resolver 时直接使用解析结果':
    'Inspector definition to validate; an omitted options schema accepts only an empty object, and an omitted resolver uses the parsed result directly',
  '补齐缺省选项并浅冻结的定义；已由本包处理的定义保持原引用':
    'The definition with missing defaults filled and shallowly frozen; definitions already processed by this package retain their reference',
  'namespace、type 或所属者要求的标识为空白时抛出 RetikzInspectError':
    'Throws RetikzInspectError when namespace, type, or required owner identifiers are blank',
  '要编译并观测的 Source Scene IR，不修改输入': 'Source Scene IR to compile and observe; the input is not modified',
  'Inspector 注册表、选择规则与可选 Core 编译配置':
    'Inspector registry, selection rules, and optional Core compilation settings',
  '同次编译的主图、辅助平面与诊断；无辅助输出时 inspection 为 null':
    'The primary result, inspection plane, and diagnostics from the same compilation; inspection is null when there is no auxiliary output',
  '选择、对象解析、Inspector 回调或辅助输出无效时编译失败，不返回部分结果；Core 编译错误沿调用链传播':
    'Compilation fails without a partial result for invalid selection, subject parsing, Inspector callbacks, or auxiliary output; Core compilation errors propagate through the call chain',
  '单项或多项检查请求；false 封锁整个 Scene 或 Scope，不能用于单个实体':
    'One or more inspection requests; false blocks a whole Scene or Scope and cannot be used on an individual entity',
  '写入 Vanilla 输入 authoring 字段的冻结标记；其请求在驱动编译时解析':
    'A frozen marker for the Vanilla input authoring field; its requests are resolved during driver compilation',
  '固定注册表与可选选择规则、提交通知；selection 省略时仅使用 authoring 请求':
    'Fixed registry with optional selection and commit callbacks; omitting selection uses only authoring requests',
  '供 Vanilla 宿主接入的编译驱动；诊断通知先于 onCommit，通知失败不回滚已提交结果':
    'A compilation driver for Vanilla hosts; diagnostics are delivered before onCommit, and callback failure does not roll back committed results',
  '面向调用方的错误消息，错误码为 INSPECTION_ERROR': 'Error message for callers; the error code is INSPECTION_ERROR',
  '错误码、消息与可选详情、原始原因；省略 details 时使用包含 code 的只读对象':
    'Error code, message, optional details and original cause; omitted details use a readonly object containing code',
  用于调用方分支处理的稳定错误码: 'Stable error code for caller-side branching',
  面向调用方的错误消息: 'Error message for callers',
  '可选详情与原始原因；省略 details 时使用包含 code 的只读对象':
    'Optional details and original cause; omitted details use a readonly object containing code',
