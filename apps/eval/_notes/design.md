...
- core 已升级 **zod v4**（见 [`v0.3 beta.2`](../../../packages/kernel/_notes/decisions/v0/v0.3/roadmap.md)），内置 `z.toJSONSchema` 可**零额外依赖**产出 JSON Schema；`SceneSchema` / `PlotSchema` 的导出能力已有测试锁定 → 导出做进 core（见 D2）只剩封装工作。
