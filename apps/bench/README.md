...
固定环境声明在 `bench-environment.json`。完整 fingerprint覆盖expected Playwright/采样/viewport等配置、实际Node与browser环境。SVG与Canvas在真实Chromium中执行，Canvas oracle来自完整像素摘要；dispose预算还通过真实listener、handler、hit-test与image handler探针冻结`liveHandles=0`。timing candidate只收录ADR-05的12个tracked 5000场景，100/1000与direct full仅留在ignored report。median / p95使用tracked baseline `1.20×`护栏，update另与同次、同Runtime生命周期及同renderer factory的full p95比较；max超`2.00×`时只完整重跑一次，第二次fingerprint漂移会skip并以非零退出，连续两次unstable才阻断。
