...
建立可发布的 Notation foundation，并把当前由 Standard 暂时拥有的逻辑图式元素迁入正确领域 owner。迁移保持元素名称和输入语义，canonical namespace 从 Standard 改为 Notation；Layout 提供通用排版，Standard 保留通用绘图拓展。
