...
建立可发布的 Notation foundation，并把当前由 Standard 暂时拥有的逻辑图式元素迁入正确领域 owner。迁移保持元素名称和输入语义，canonical namespace 从 Standard 改为 Notation；Layout 提供通用排版，Standard 保留通用绘图拓展。四个基础逻辑单元同时接入 Core 的四种 Theme Style，并保留显式外观的最高优先级。
...
| [02](./02-logic-unit-theme-styles.md) | 基础逻辑单元接入四种 Theme Style       | Core Theme / Node color；Notation semantic IR；Core color atom         | Accepted |
...
- 逻辑单元只从 authored `color` 或模式黑白默认值取得主要色，不读取 Core categorical palette
