...
这里放 Diagram 分组的内部协作文档。Graph 三包作为首个 foundation package family；Graph、Flow 与 Graph editor adapter 只在各自 roadmap / ADR 确认后建包。

## 目录

- [`architecture/`](./architecture)：Diagram Graph 的长期边界与完备性准入
...
- [`architecture/diagram-graph-complete.md`](./architecture/diagram-graph-complete.md)：可复用图式元素、Standard / Core 复用与 Graph 排除边界
