...
import { LogicLayoutItemArtifactSchema, LogicOuterArtifactSchema } from '../../src';
...
describe('logic artifact schemas', () => {
  beforeEach(() => {
    expect(LogicOuterArtifactSchema).toBeDefined();
    expect(LogicLayoutItemArtifactSchema).toBeDefined();
  });
  ...
  it('parses strict outer and content placement artifacts and survives JSON round-trip', () => {
    ...
    expect(LogicOuterArtifactSchema.parse(outer)).toEqual(outer);
    expect(LogicLayoutItemArtifactSchema.parse(item)).toEqual(item);
    expect(LogicOuterArtifactSchema.parse(JSON.parse(JSON.stringify(outer)))).toEqual(outer);
    expect(LogicLayoutItemArtifactSchema.parse(JSON.parse(JSON.stringify(item)))).toEqual(item);
  });
  ...
  it('accepts canonical zero geometry and null visibility without inventing positive bounds', () => {
    ...
    expect(LogicOuterArtifactSchema.parse(outer)).toEqual(outer);
    expect(LogicLayoutItemArtifactSchema.parse(zeroItem)).toEqual(zeroItem);
  });

  it('keeps authored identity outside the generic item placement artifact', () => {
    expect(() => LogicLayoutItemArtifactSchema.parse({ ...item, key: 'content' })).toThrow();
    expect(() => LogicLayoutItemArtifactSchema.parse({ ...item, sourceIndex: 0 })).toThrow();
  });

  it('rejects unknown artifact fields instead of silently widening the JSON contract', () => {
    expect(() =>
      LogicOuterArtifactSchema.parse({
    ...
    expect(() => LogicLayoutItemArtifactSchema.parse({ ...item, extra: true })).toThrow();
    expect(() =>
      LogicLayoutItemArtifactSchema.parse({ ...item, overflow: { ...item.overflow, extra: true } }),
    ).toThrow();
  });
  ...
  it('rejects negative or non-finite rectangles and translations at their authored geometry fields', () => {
    ...
    expect(() =>
      LogicLayoutItemArtifactSchema.parse({ ...item, translation: { x: Number.POSITIVE_INFINITY, y: 0 } }),
    ).toThrow();
    expect(() =>
      LogicLayoutItemArtifactSchema.parse({ ...item, visualBounds: rect(0, 0, 10, Number.NEGATIVE_INFINITY) }),
    ).toThrow();
  });
  ...
  it('rejects null allocation or visual geometry while retaining null only for optional visibility', () => {
    ...
    expect(() => LogicLayoutItemArtifactSchema.parse({ ...zeroItem, allocationBounds: null })).toThrow();
  });
});
