...
const blockDefinitionOf = (): AnyCompositeDefinition => Notation.LogicFrameDefinition;

const blockArtifactOf = (value: unknown): LogicFrameArtifact => {
  return Notation.LogicFrameArtifactSchema.parse(value);
};
...
const block = (input: Parameters<typeof Notation.createLogicFrame>[0]) => Notation.createLogicFrame(input);

const section = (
  key: string,
  id: string,
  options: Parameters<typeof child>[1] = 24,
): Notation.LogicFrameSectionInput => ({
  key,
...
describe('LogicFrame layout and artifact contract', () => {
  beforeAll(() => {
    ...
    expect(Notation.LogicFrameArtifactSchema, 'production mutation required: LogicFrameArtifactSchema').toBeDefined();
  });
  ...
  it('keeps header optional, preserves authored section order, and rejects empty or duplicate sections', () => {
    ...
    expect(() => Notation.LogicFrameSchema.parse({ namespace: 'notation', type: 'logicFrame', id: 'empty' })).toThrow();
    expect(() =>
      Notation.LogicFrameSchema.parse({
        namespace: 'notation',
        type: 'logicFrame',
        id: 'duplicate',
  ...
  it('supports nested LogicFrame children through the same public definition registry', () => {
    ...
    const artifacts = output.artifacts
      .filter(value => value.kind === 'composite' && value.namespace === 'notation' && value.type === 'logicFrame')
  ...
  it('reuses the canonical Flex compiler without requiring FlexLayoutDefinition', () => {
    ...
    expect(blockArtifactOf(compositeArtifact(output, 'logicFrame').value).id).toBe('direct-flex-reuse');
  ...
  it('exposes strict header, section, container, outer, and divider artifact geometry', () => {
    ...
    const schema = Notation.LogicFrameArtifactSchema;
