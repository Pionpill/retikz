...
type CalloutArtifact = Readonly<{
  kind: 'callout';
  id: string;
  target: Notation.LogicDiagramTarget;
  placement: Notation.CalloutPlacement;
  outer: Notation.LogicOuterArtifact;
  container: Readonly<{
  ...
  content: Notation.LogicLayoutItemArtifact;
...
const callout = (input: Parameters<typeof Notation.createCallout>[0]): IRChild => Notation.createCallout(input);
...
const artifactOf = (output: ReturnType<typeof compileToScene>): CalloutArtifact => {
  const artifact = output.artifacts.find(
    value => value.kind === 'composite' && value.namespace === 'notation' && value.type === 'callout',
  );
...
describe('Callout placement and leader contract', () => {
  ...
  it('keeps default and empty leaders arrowless, preserves custom marks, and supports leader false', () => {
    const compile = (id: string, leader?: false | Notation.ConnectorAppearanceInput) =>
      compileCallout([
        node(`${id}-target`),
        callout({
          id,
