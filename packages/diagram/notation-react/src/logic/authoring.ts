...
export const collectSemanticNodeInput = <TInput extends Readonly<Record<string, unknown>>>(
  ...
  if (childrenInput === undefined || childrenInput.length !== 1) {
    throw new Error('Notation semantic unit must contain exactly one Core Node authoring input.');
  }
...
/** 收集 Connector children 为等待 Vanilla 归一化的 Core Path 输入 */
export const collectConnectorPath = (
  children: ReactNode,
  embedIdPrefix: string,
): Readonly<{
  child: InputPath;
  ...
  if (childrenInput === undefined || childrenInput.length !== 1) {
    throw new Error('Connector children must contain one Core Path authoring input.');
  }
