...
import { BarMark, PointMark, TextMark } from '../../src/components/marks';

/**
 * ADR-04（alpha.11）：plot-react 装配契约。
 * priority-1：位置 mark 的扁平 label* props → IR mark.label（MarkLabelSchema），resolveLabel 收进旁路、不落 IR；
 * priority-2：<TextMark> 扁平 props → IR text mark（type:'text'、encoding.text、x/y/color、dx/dy 落顶层）。
 */

describe('priority-1 宿主 mark label 扁平 props → IR mark.label', () => {
  it('react-mark-label-assembly：BarMark label* → interval mark.label，与手写 IR 等价', () => {
    const spec = buildPlotSpec(<BarMark x="month" y="revenue" label="revenue" labelPosition="above" labelDistance={6} labelFormat=",.0f" />, '__plot');
  ...
  it('无 label prop → mark 无 label 字段', () => {
    const spec = buildPlotSpec(<BarMark x="m" y="r" />, '__plot');
    expect(spec.marks[0]).not.toHaveProperty('label');
  });

  it('resolveLabel prop（配 id）→ 收进旁路、不落 IR', () => {
    const fn = (row: { revenue?: number }) => `$${row.revenue}`;
    const spec = buildPlotSpec(<BarMark id="bars" x="month" y="revenue" resolveLabel={fn} />, '__plot');
  ...
  it('resolveLabel prop 无 id → fail-loud', () => {
    expect(() => buildPlotSpec(<BarMark x="month" y="revenue" resolveLabel={() => 'x'} />, '__plot')).toThrow(/mark id/i);
  });
});

describe('priority-2 独立 TextMark 扁平 props → IR text mark', () => {
  it('react-textmark-encoding-assembly：TextMark → type text、encoding.text、x/y/color、dx/dy 落顶层', () => {
    const spec = buildPlotSpec(<TextMark x="month" y="revenue" text="revenue" color="cat" dy={-8} />, '__plot');
  ...
  it('TextMark format → encoding.text.format', () => {
    const spec = buildPlotSpec(<TextMark x="x" y="y" text="v" format=",.1f" />, '__plot');
    const mark = spec.marks[0] as { encoding: { text?: unknown } };
    expect(mark.encoding.text).toEqual({ field: 'v', format: ',.1f' });
  });

  it('TextMark resolveLabel（配 id）→ 旁路收集、不落 IR', () => {
    const fn = (row: { label?: string }) => `<${row.label}>`;
    const spec = buildPlotSpec(<TextMark id="t1" x="x" y="y" text="label" resolveLabel={fn} />, '__plot');
    expect(JSON.stringify(spec)).not.toContain('resolveLabel');
