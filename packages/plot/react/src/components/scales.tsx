...
/** 位置 scale 可配置的坐标维度 */
export type ScaleDimension = 'x' | 'y' | 'angle' | 'radius';

/** React DSL 里当前暴露的位置 scale 类型 */
export type PositionScaleType = 'linear' | 'time' | 'point' | 'log' | 'sqrt';

/** <Scale> props：声明某个坐标维度使用的 scale 类型 */
export type ScaleProps = {
  /** 绑定哪个定位维度；polar 下也可用 x / y 作为 angle / radius 的别名 */
  dimension: ScaleDimension;
  /** scale 类型；更细的 domain / range / base 等参数走 spec 入口 */
