...
import { makeSizeResolver } from './channel';
...
const expandPlot = (node: PlotSpec, datasets: ExternalDatasets, options: LowerPlotsOptions): IRChild => {
  ...
  const markLayers: Array<IRChild> = node.marks
    .map((mark, markIndex) => lowerMark(mark, rows, frame, resolveColor(mark), resolveSize(mark), provenance ? { context: provenance, markIndex, registerDatumId } : undefined))
