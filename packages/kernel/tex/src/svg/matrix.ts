...
export const parseTransform = (value: string | undefined): AffineMatrix => {
  ...
  while ((hit = re.exec(source)) !== null) {
    if (source.slice(cursor, hit.index).trim().length > 0) {
      throw new RetikzSvgTransformError('malformed', `Malformed SVG transform syntax: ${source}`);
    }
    const fn = hit[1];
    const args = hit[2]
      .split(/[\s,]+/)
    ...
    if (args.some(arg => !Number.isFinite(arg))) {
      throw new RetikzSvgTransformError('malformed', `Invalid SVG transform argument: ${hit[0]}`);
    }
    let local: AffineMatrix;
    if (fn === 'translate') {
      if (args.length < 1 || args.length > 2)
        throw new RetikzSvgTransformError('malformed', `Invalid translate transform: ${hit[0]}`);
      local = [1, 0, 0, 1, args[0] ?? 0, args[1] ?? 0];
    } else if (fn === 'scale') {
      if (args.length < 1 || args.length > 2)
        throw new RetikzSvgTransformError('malformed', `Invalid scale transform: ${hit[0]}`);
      const sx = args[0] ?? 1;
      local = [sx, 0, 0, args[1] ?? sx, 0, 0];
    } else if (fn === 'matrix') {
      if (args.length !== 6) throw new RetikzSvgTransformError('malformed', `Invalid matrix transform: ${hit[0]}`);
      local = [args[0], args[1], args[2], args[3], args[4], args[5]];
    } else {
      throw new RetikzSvgTransformError('unsupported', `Unsupported SVG transform: ${fn}`);
    }
