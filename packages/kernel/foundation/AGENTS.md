...
- **解决的问题**：为 Kernel、Standard、Viz 与其它跨领域包提供无领域的原子类型工具、基础 Zod 校验、确定性标量值计算、typed non-empty string 不变量、JSON 数据快照、结构只读集合快照和可分类错误骨架
- **拥有的契约**：`ValueOf`、`AssertEqual`、`OpenString`、`WithRequiredProperties`、`NonEmptyReadonlyArray` 等无领域类型工具，既有断言与错误契约，六个无领域标量 schema、`createOpenStringSchema(values)`、静态 CSS 颜色解析与不透明 source-over 预合成、plain-data container 断言、`cloneAndFreezeJson` 深冻结快照与 `createReadonlyMap` 浅快照
- **不拥有的能力**：IR、对象或领域 schema、IR / DSL parser、Theme mode 或主色链、geometry、Definition / registry、Diagnostic、renderer、host state、领域错误码或领域恢复语义
- **输入与输出**：基础 schema 把 unknown 校验为不变形的 string / number；颜色原子把静态 CSS 颜色解析为归一化 sRGB 或按归一化权重与不透明底色预合成为不透明十六进制颜色；JSON helper 输出脱离原输入的深冻结副本；集合契约把 entries 复制为不暴露写方法的浅快照；其余契约接收已收窄字符串或结构化错误基础字段，输出 `void` 或 Foundation 错误 class hierarchy；不创建图形数据
...
- `schema.ts` 只承载无领域 string / number 原子，以及唯一受限的 `createOpenStringSchema(values)`；禁止对象、数组、coercion、transform、default、catch、参数化 range factory、颜色 schema、几何或领域 refinement
- `createOpenStringSchema(values)` 只接受 const object enum，并组合已知 enum branch 与非空白开放字符串 branch；不得查询 registry、注入领域默认或收窄为闭合集合
- `color.ts` 只承载无需宿主环境即可确定的静态 CSS 颜色解析、归一化 sRGB 数据与不透明 source-over 预合成；不得读取 Theme、IR、renderer、DOM、CSSOM、系统色或领域主色链
- 静态颜色原子必须固定支持子集、sRGB 合成、通道舍入和小写不透明十六进制输出；`currentColor`、CSS variable、系统色及其它动态宿主颜色不得猜测或 fallback
- 颜色权重必须是 `[0, 1]` 有限数，预合成底色必须不透明；Foundation 只报告原子输入失败，调用方拥有字段路径和领域诊断
- 颜色原子失败统一使用 `RetikzFoundationErrorCode.Color`，details 标识失败输入角色和原始值；不得加入 Theme、IR 或领域路径
