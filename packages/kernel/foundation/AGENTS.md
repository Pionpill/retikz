...
- **拥有的契约**：`ValueOf`、`AssertEqual`、`OpenString`、`WithRequiredProperties`、`NonEmptyReadonlyArray` 等无领域类型工具，既有断言与错误契约，六个无领域标量 schema、`createOpenStringSchema(values)`、plain-data container 断言、`cloneAndFreezeJson` 深冻结快照与 `createReadonlyMap` 浅快照
