...
- **拥有的契约**：`ValueOf`、`AssertEqual`、`OpenString`、`WithRequiredProperties` 等无领域类型工具，既有断言与错误契约，六个无领域标量 schema、从 const object enum 建立开放非空字符串 schema 的 `createOpenStringSchema(values)`，以及 JSON-safe plain data 快照 helper
- **不拥有的能力**：IR、对象或领域 schema、parser、geometry、Definition / registry、Diagnostic、renderer、host state、领域错误码或领域恢复语义
- **输入与输出**：基础 schema 把 unknown 校验为不变形的 string / number；JSON helper 接收待校验的 plain data 并输出脱离原输入的深冻结副本；其余契约接收已收窄字符串或结构化错误基础字段，输出 `void` 或 Foundation 错误 class hierarchy；不创建图形数据
...
- `src/` 固定为 `types.ts`、`schema.ts`、`assert.ts`、`error.ts`、`json.ts`、`index.ts`；不得新增 `shared`、`utils`、`helpers` 或领域目录
