...
- **解决的问题**：为 Kernel、Standard、Viz 与其它跨领域包提供无领域的原子类型工具、基础 Zod 校验、typed non-empty string 不变量、结构只读集合快照和可分类错误骨架
- **拥有的契约**：`NonEmptyReadonlyArray` 等无领域类型工具、`createReadonlyMap` 浅快照、既有断言与错误契约，以及 `NonBlankStringSchema`、`PositiveNumberSchema`、`NonNegativeNumberSchema`、`PositiveIntegerSchema`、`NonNegativeIntegerSchema` 与 `NormalizedFractionSchema`
- **不拥有的能力**：IR、对象或领域 schema、parser、geometry、Definition / registry、Diagnostic、renderer、host state、领域错误码或领域恢复语义
- **输入与输出**：基础 schema 把 unknown 校验为不变形的 string / number；集合契约把 entries 复制为不暴露写方法的浅快照；其余契约接收已收窄字符串或结构化错误基础字段，输出 `void` 或 Foundation 错误 class hierarchy；不创建图形数据
...
- `src/` 固定为 `types.ts`、`schema.ts`、`assert.ts`、`collections.ts`、`error.ts`、`index.ts`；不得新增 `shared`、`utils`、`helpers` 或领域目录
- `index.ts` 只用 `export *` 聚合五个 owner 文件，不写业务逻辑、包装或重命名
