...
- **解决的问题**：为 Kernel、Standard、Viz 与其它跨领域包提供无领域的原子类型工具、基础 Zod 校验、通用 JSON 契约、确定性标量值计算、typed non-empty string 不变量、JSON 数据快照、结构只读集合快照和可分类错误骨架
- **拥有的契约**：`ValueOf`、`AssertEqual`、`OpenString`、`WithRequiredProperties`、`NonEmptyReadonlyArray`、`JsonValue`、`JsonObject` 等无领域类型工具，既有断言与错误契约，六个无领域标量 schema、通用 JSON value / object schema、`createOpenStringSchema(values)`、静态 CSS 颜色解析与不透明 source-over 预合成、plain-data container 断言、`cloneAndFreezeJson` 深冻结快照与 `createReadonlyMap` 浅快照
- **不拥有的能力**：IR、领域对象 schema、IR / DSL parser、Theme mode 或主色链、geometry、Definition / registry、Diagnostic、renderer、host state、领域错误码或领域恢复语义
- **输入与输出**：基础 schema 把 unknown 校验为不变形的 string / number 或递归 JSON；颜色原子把静态 CSS 颜色解析为归一化 sRGB 或按归一化权重与不透明底色预合成为不透明十六进制颜色；JSON helper 输出脱离原输入的深冻结副本；集合契约把 entries 复制为不暴露写方法的浅快照；其余契约接收已收窄字符串或结构化错误基础字段，输出 `void` 或 Foundation 错误 class hierarchy；不创建图形数据
