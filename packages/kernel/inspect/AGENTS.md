...
本包拥有宿主无关的 Inspector Definition、registry、selection、辅助编译、diagnostics、plane 与内置 Path Inspector。根入口只依赖 `@retikz/core` 和直接使用的 `zod`；Render、React、Vanilla 集成必须位于独立子入口，且只作为 optional peer。
