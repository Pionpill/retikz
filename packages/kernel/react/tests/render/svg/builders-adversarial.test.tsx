...
describe('buildPathD 对抗：浮点 / 360° / 退化', () => {
  ...
  it('两个 sub-path 都被 close 收尾（多段 cycle 形状）', () => {
    ...
    expect(buildPathD(commands)).toBe('M 0 0 L 10 0 L 10 10 Z M 20 0 L 30 0 L 30 10 Z');
  });
  ...
  it('超精度 round 传入：保留 4 位小数', () => {
    ...
    const commands: Array<PathCommand> = [{ kind: 'move', to: [1.234567, 2.345678] }];
    expect(buildPathD(commands, r)).toBe('M 1.2346 2.3457');
  });

  it('arc 第一段时自动 emit M 起点（adapter 单独调用 builder 不需要消费者先 move）', () => {
    const commands: Array<PathCommand> = [{ kind: 'arc', center: [0, 0], radius: 10, startAngle: 0, endAngle: 90 }];
...
describe('buildTransform 对抗', () => {
  it('rotate cx 给 0 cy 给 undefined 仍写 3 参形态（防止 0 被当 undefined）', () => {
    const transforms: Array<Transform> = [{ kind: 'rotate', degrees: 90, cx: 0 }];
    expect(buildTransform(transforms)).toBe('rotate(90 0 0)');
  });

  it('rotate cx undefined cy 给 0 同理', () => {
    const transforms: Array<Transform> = [{ kind: 'rotate', degrees: 90, cy: 0 }];
    expect(buildTransform(transforms)).toBe('rotate(90 0 0)');
  });

  it('rotate 两个都 undefined 走 2 参短形', () => {
    const transforms: Array<Transform> = [{ kind: 'rotate', degrees: 90 }];
    expect(buildTransform(transforms)).toBe('rotate(90)');
  });

  it('scale y=0 仍写出（合法压扁变换，不被 ?? 替换为 x）', () => {
    const transforms: Array<Transform> = [{ kind: 'scale', x: 2, y: 0 }];
  ...
  it('round 函数透传到所有数值字段', () => {
    ...
    expect(buildTransform(transforms, r)).toBe('translate(2 2) rotate(31 6 10) scale(3 3)');
  });
});
