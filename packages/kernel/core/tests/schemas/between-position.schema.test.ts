...
describe('between 结构完整性', () => {
  it('between 仅含 1 个端点拒绝', () => {
    expect(() => BetweenPositionSchema.parse({ between: [[0, 0]], fraction: 0.5 })).toThrow();
  });
  ...
  it('between 字段缺失拒绝', () => {
    expect(() => BetweenPositionSchema.parse({ fraction: 0.5 })).toThrow();
  });
});
