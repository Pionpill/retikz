...
import type { MarkerFill, MarkerPrimitive, PathPrim, ResolvedArrowEndSpec, ScenePrimitive } from '../../src/contract/scene';
  ...
  it("顶层 scale=2 + start.shape='diamond' → start.scale 进 markerWidth（继承非 shape 字段）", () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('<->', { scale: 2, color: 'blue', start: { shape: 'diamond' } }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
  ...
  it('end 完全空对象 + 顶层全填 → end 拿到全部顶层字段（不丢失）', () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('->', { shape: 'stealth', scale: 1.5, color: 'red', end: {} }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
  ...
  it('end.shape 显式 override 顶层 shape（不被吞）', () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('<->', { shape: 'stealth', end: { shape: 'circle' } }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
  ...
  it("空心 shape + end.fill='red'（end 子对象上写）也被丢", () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('->', { shape: 'open', end: { fill: 'red' } }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
  ...
  it("start.shape='normal' / end.shape='stealth' → 2 个 spec shape 不同", () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('<->', { start: { shape: 'normal' }, end: { shape: 'stealth' } }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
  ...
  it("arrow='->' + arrowDetail.start={...} → start spec 被丢（方向不含 start）", () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('->', { start: { shape: 'diamond', color: 'red' } }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
  ...
  it("arrow='none' + arrowDetail={全配置} → 两端都不挂", () => {
    const ir: IR = {
      ...
      children: [
        {
          type: 'path',
          marks: arrowMarks('none', { shape: 'stealth', color: 'red', scale: 2 }),
          children: [
            { type: 'step', kind: 'move', to: [0, 0] },
            { type: 'step', kind: 'line', to: [10, 0] },
          ],
