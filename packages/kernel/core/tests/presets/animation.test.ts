...
describe('Sugar=Kernel 等价（默认参数）', () => {
  ...
  it('growUp / slideIn / colorShift', () => {
    ...
    expect(colorShift({ from: '#ff0000', to: '#0000ff' })).toEqual({
      property: 'fill',
      keyframes: [
        { at: 0, value: '#ff0000' },
        { at: 1, value: '#0000ff' },
...
describe('opts 覆盖', () => {
  it('base opts（duration/delay/easing/trigger）透传', () => {
    expect(fadeIn({ duration: 1000, delay: 100, easing: 'linear', trigger: 'visible' })).toEqual({
      property: 'opacity',
      keyframes: [
        { at: 0, value: 0 },
        { at: 1, value: 1 },
  ...
  it('scaleIn from/origin、slideIn axis/offset', () => {
    expect(scaleIn({ from: 0.5, origin: [2, 3] })).toEqual({
      property: 'scale',
      keyframes: [
        { at: 0, value: 0.5 },
        { at: 1, value: 1 },
    ...
    expect(slideIn({ axis: 'y', offset: 50 })).toEqual({
      property: 'translateY',
      keyframes: [
        { at: 0, value: 50 },
        { at: 1, value: 0 },
...
describe('loop / stagger', () => {
  it('loop 叠加 iterations:infinite（+ direction）', () => {
    ...
    expect(loop(fadeIn(), { iterations: 3, direction: 'alternate' })).toEqual({
      ...fadeIn(),
      iterations: 3,
      direction: 'alternate',
    });
