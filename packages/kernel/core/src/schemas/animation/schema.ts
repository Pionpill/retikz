...
export const OriginSchema = union([
  ...
  tuple([number(), number()]).describe('Explicit transform pivot in element-local coordinates [x, y].'),
]).describe(
...
export const AnimationTrackSchema = object({
  ...
  keyframes: array(KeyframeSchema)
    .min(1)
