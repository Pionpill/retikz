...
- 是否可由现有能力组合：
- math / core / render / adapter 责任切分：
- 是否需要新 IR / contract / registry；不采用 registry 时的理由：
- Scene / manifest / renderer / diagnostics 如何闭环：
- provenance / locator / Interaction Readiness 是否适用：
...
<只写需要 schema、compile、adapter parity、renderer、docs 等哪些证据层及关键不变量，不写 case、路径、命令或数量；详细矩阵进入镜像 plan 的 TEST_CONTRACT.md>
