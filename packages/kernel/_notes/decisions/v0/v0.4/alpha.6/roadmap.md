...
| [ADR-04](./04-node-label-inside-placement.md) | 已接受 | Node label 内侧 placement | `Node.label` 增加 `placement: "outside" \| "inside"` 与 `{ boundary, t }` position，支持边界上的自定义标签位置 |
...
- [x] `Node.label.position={ boundary, t }` 能表达矩形 / box-like 节点某条边界上的自定义比例位置。
