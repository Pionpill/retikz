...
> 本文件是 v0.4 的总计划与执行索引：记录已拍板方向、alpha 里程碑、取舍和后续候选。alpha.1-alpha.7 已按各自 roadmap / ADR 落地并同步 changelog；alpha.8 原定为 v0.4 改动收口，当前追加 plot 牵引的 `dashOffset` 描边相位补齐，作为 beta 前最后一项 core 通用描边能力收敛。
>
...
| D    | eval（LLM 生成评测）      | 🔀 **交其他分支处理**，本 core 路线不跟踪    | eval 工程已转交独立分支推进，不在本 core v0.4 路线内排期；设计参考 [`apps/eval/_notes/design.md`](../../../../../../apps/eval/_notes/design.md)                                                                                                                                                                                                                                                     |
| E    | 数学公式（LaTeX）         | ✅ alpha.5 落地                              | 守 renderer-agnostic：MathJax SVG-path → Scene `PathPrim` 的 lowering；独立包 **`@retikz/tex`**，MathJax 走 optional peer。落地为文本里 `$...$` / `$$...$$` / `{ runs }` 行内 text+math 混排（详见下「后续讨论结论 · E」与 [alpha.5 roadmap](./alpha.5/roadmap.md)）                                                                                                                                |
...
| --------- | --------------------------------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| alpha.1   | `@retikz/math` + core 纯几何下沉                          | A                                 | [ADR-01/02/03](./alpha.1/roadmap.md)                                                                                                                      | ✅ Accepted（实现 + 文档 + changelog 完成；math 加入 kernel lockstep，core 纯几何下沉并 re-export）                                                                                                                                                               |
| alpha.2   | 可嵌入 Tier2 in `<Layout>` ∥ scope 多态 bounding shape    | Tier2 机制 / H                    | [ADR-01](./alpha.2/01-embeddable-tier2-in-layout.md) · [ADR-02](./alpha.2/02-scope-polymorphic-bbox.md)                                                   | ✅ Accepted（MVP 已实现；`<Layout>` 可嵌入 Tier2，`Scope.boundingShape` 支持 rectangle / circle）                                                                                                                                                                 |
...
| alpha.4   | Scene 视觉（shadow + blend；z-index 已落地）              | F                                 | [ADR-01 shadow](./alpha.4/01-scene-drop-shadow.md) · [ADR-02 blend](./alpha.4/02-blend-mode.md)                                                           | ✅ Accepted（shadow + blend 实现 + 文档 + changelog + 测试完成；2026-06-16 完工，roadmap [alpha.4](./alpha.4/roadmap.md)）                                                                                                                                        |
...
| beta.1    | 编译期契约收敛                                            | beta hardening                    | [roadmap](./beta.1/roadmap.md)                                                                                                                            | ✅ Accepted（compile 文件结构范式定稿；首步将 path stroke 装饰 / 输出 / 倒角 / shrink / split 下沉到 `compile/path/stroke`，保持公开行为不变）                                                                                                                    |
| beta.2    | Adapter 作者 API、包根公共面与动画宿主策略收敛            | beta hardening                    | [roadmap](./beta.2/roadmap.md)                                                                                                                            | ✅ Accepted（Vanilla plain spec、React 包根 owner、Tier 2 IR 反向转换与动画三态 / Provider 已实现；实现 + 文档 + changelog + 对账完成，2026-07-14 收口；不新增 core IR / Scene 能力）                                                                             |
| 后续版本  | v0.5 / rc / long-term 候选拆分                            | 待定                              | —                                                                                                                                                         | 待讨论：headless interaction / Progressive IR 可进入 v0.5 设计；交互和用户操作场景需要评估 incremental compile、IR diff、bailout 与 concurrent 调度；rc 聚焦 API 与安装验收；P3D / ScenePrimitive 底层能力重评估 / scope 富化等更长期处理                         |
...
- **独立包 `@retikz/tex`**（不并进 `@retikz/extension`）：它是自带 path-d 解析 + 坐标映射 + 引擎集成的小子系统，比单个 extension 定义重；且拖一个重的 MathJax optional peer，不该和轻量几何 extension 纠缠。落地时归 **core 分组**（`packages/kernel/tex`，与 math/core/render/react/vanilla 同 lockstep），不再单独 version——详见 [alpha.5 roadmap](./alpha.5/roadmap.md) 与 [ADR-01](./alpha.5/01-tex-package-and-node-math.md)。
