...
| alpha.7          | Provider contract 收敛与功能补齐                         | contract-provider 机制 | [roadmap](./alpha.7/roadmap.md)                                                                                                                             | Accepted（2026-06-29 人工签字，待实现；统一 provider array 输入、Map resolve、duplicate throw、key contract、React / Vanilla 透传与 docs authoring surface；允许破坏性改动，不保留旧 provider 输入兼容） |
