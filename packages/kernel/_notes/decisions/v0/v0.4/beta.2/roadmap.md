...
> beta.2 聚焦 beta 阶段的公开 API、adapter 作者模型与动画宿主策略收敛。该阶段允许在 0.x 内做破坏性整理，但每项必须先落 ADR，再进入实现。
...
| [ADR-04](./04-animation-enabled-override.md) | render / react / vanilla / docs | 统一动画开关三态语义，并允许 React 宿主覆盖整棵预览子树。                                    | Accepted |
...
- 不改变 animation track、easing、trigger、snapshot 或后端静态降级能力。
