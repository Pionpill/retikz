...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · [本 milestone ADR-015 sugar 设计约定](./015-sugar-conventions.md)
...
输出 `M arcStart, arc, L center, Z` —— 干净闭合 wedge，用 emitClose（Z）收口（描边接角正确）。代价：Sector 的 center 与 arcStart 都须 literal 笛卡尔（sugar 算 arcStart），与 [ADR-015](./015-sugar-conventions.md)「可计算形态」一致。
...
- circlePath / ellipsePath 部分裁剪（[ADR-013](./013-partial-circle-ellipse.md)）；矩形 step（[ADR-014](./014-rectangle-step.md)）。
