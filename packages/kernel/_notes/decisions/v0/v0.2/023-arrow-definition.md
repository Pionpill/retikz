...
- 状态：Accepted（已实现）
- 决策日期：2026-05-24
- 关联： · tikz-gap-analysis §2 Path（历史分析已删除） · [ADR-007 Shape Registry](./007-shape-registry.md)（注册面先例）· [ADR-019 Paint](./019-paint-basics.md)（`PaintValue.contextStroke` 颜色继承）· 本 milestone [ADR-024](./024-path-generator-definition.md) / [ADR-025](./025-curve-transform-marking.md)
...
- `<marker>` 是 SVG-only（ 明列 marker 为禁项），内置箭头把 SVG 写死在 react 包 = 渲染泄漏，违反「core 不假定渲染端」。
- 颜色继承依赖 v0.2 ADR-019 的 `PaintValue.contextStroke`，故 ArrowDefinition 紧跟 alpha.7、先于 PathGeneratorDefinition 实现。

## 决策：`ArrowDefinition` + `CompileOptions.arrows` 注册面，emit 产 `MarkerPrimitive` 窄子集，内置 7 降注册项

`arrowDetail.shape` 由 `z.nativeEnum(ARROW_SHAPES)` 开放为 `z.string()`（对齐 alpha.3 开放 node.shape）；未注册名编译期 throw。`CompileOptions.arrows?: Record<string, ArrowDefinition>`，有效表 = `{ ...BUILTIN_ARROWS, ...arrows }`，同名覆盖发 warn（`ARROW_OVERRIDES_BUILTIN`，仿 `SHAPE_OVERRIDES_BUILTIN`，不 error）。内置 7 迁进 （仿 `shapes/`），降注册项无特权。`ArrowDefinition` / `ArrowEmitContext` 为 TS type（运行时注入、不进 IR）；签名真源见 。
...
- **`MarkerPrimitive` 窄子集**（）：仅 marker-local `path` / `ellipse` / `rect` / `group`；`fill` 收窄到 `string | { kind:'contextStroke' }`（无 `resourceRef` / 外部资源）；禁 `text`、`arrowStart/End`、`clipRef`。允许 `group`（嵌套 transform 做复合箭头），但禁再套 marker / text。
...
- **PathGeneratorDefinition**→ [ADR-024](./024-path-generator-definition.md)；**out/in·self-loop / 路径变换 / marking**→ [ADR-025](./025-curve-transform-marking.md)。
- **`PaintValue` / `contextStroke` 定义**→ v0.2 ADR-019（本篇消费，不定义）。

---

## 最终实现结果

已实现本 ADR 的核心决策。兼容性：向后兼容非 breaking（`arrowDetail.shape` 从 7 枚举扩为 string，旧字面量仍合法；内置 7 行为零回归）；其余默认行为、失败语义与公开契约以正文为准。
