...
v0.2 开工前另起 ADR（位置 `packages/kernel/_notes/decisions/v0/v0.2/alpha.5/`，编号到时定），固化上节全部「待定」项的最终取舍（命名歧义 / `closed` 模式暴露 / Sector 闭合 / rect-vs-path primitive / corner 命名等），并落以下交付物：
