...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · 本 milestone [ADR-01](./01-arc-center-and-elliptical.md) / [ADR-02](./02-partial-circle-ellipse.md) / [ADR-03](./03-rectangle-step.md)

> **目标**：固化 alpha.5 一批形状 sugar（`<Circle>` / `<Ellipse>` / `<Arc>` / `<Sector>` / `<Rectangle>` / `<Grid>` / `<RegularPolygon>` / `<Star>`）共享的跨组件约定。ADR-01~03 管 IR 改动，本篇管 sugar 层契约。
