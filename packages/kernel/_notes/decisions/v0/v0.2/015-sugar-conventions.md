...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · 本 milestone [ADR-012](./012-arc-center-and-elliptical.md) / [ADR-013](./013-partial-circle-ellipse.md) / [ADR-014](./014-rectangle-step.md)

> **目标**：固化 alpha.5 一批形状 sugar（`<Circle>` / `<Ellipse>` / `<Arc>` / `<Sector>` / `<Rectangle>` / `<Grid>` / `<RegularPolygon>` / `<Star>`）共享的跨组件约定。ADR-012~014 管 IR 改动，本篇管 sugar 层契约。
...
- **可计算形态**（sugar 要算 midpoint / bbox / 宽高 / arcStart，点位**只接 literal `[x, y]`**，传 node id / 极坐标 / 相对坐标**明确抛 Error、不静默兜底**）：Circle `{ from, to }` / `{ corner1, corner2 }`、Ellipse `{ corner1, corner2 }`、Rectangle `{ center, width, height }` / `{ center, side }` / `{ corner1, width, height }`、Sector（算 arcStart 画 wedge 边，[ADR-012](./012-arc-center-and-elliptical.md)）、Grid 全部形态。
- **点位类型以 `TargetSchema` 为准**：不含 `{ direction, of }`（那是 Node / Coordinate 的 `position`，不是 path target）。
...
ADR-012~014 新增的 outline / 顶点几何**一律写成 `geometry/*.ts` 纯函数**（`arc.ts` 椭圆弧、`{circle,ellipse}.ts` 部分 outline、`rect.ts` 圆角 outline、`polygon.ts` 顶点），**不内联在 `compile/path/`**。理由：`@retikz/core` 不能 import `@retikz/react`，「Node 复用 sugar」依赖方向不成立；正确形态是 sugar 的 compile 路径与未来 node shape 的 `emit` / `boundaryPoint` / `anchor` **并列消费同一 core 几何层**。
