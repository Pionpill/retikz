...
- 状态：Accepted（已实现）
- 决策日期：2026-05-24
- 关联： · tikz-gap-analysis §3 Step（历史分析已删除） · [ADR-007 Shape Registry](./007-shape-registry.md)（注册面先例）· 本 milestone [ADR-023](./023-arrow-definition.md)（先实现）/ [ADR-025](./025-curve-transform-marking.md)
...
`{ kind:'generator', name, to?, params }` 进 `StepSchema` discriminatedUnion；`params` 类型 = `JsonObjectSchema`（递归 JSON 值，禁 function / undefined）。`CompileOptions.pathGenerators?: Record<string, PathGeneratorDefinition>`，未注册名编译期 throw。`PathGeneratorDefinition` / `PathGeneratorContext` 为 TS type（运行时注入、不进 IR）；签名真源见 。
...
- **`JsonObjectSchema`（新增 ）**：`z.record(JsonValueSchema)`，`JsonValue` = string/number/boolean/null/array/object 递归；禁 function / undefined。core 公开供外部 refine。
...
- **ArrowDefinition**→ [ADR-023](./023-arrow-definition.md)；**out/in·self-loop / 路径变换 / marking**→ [ADR-025](./025-curve-transform-marking.md)。
...
已实现本 ADR 的核心决策。兼容性：向后兼容非 breaking（纯叠加新 step kind + 新 CompileOptions，core 不内置任何曲线）；其余默认行为、失败语义与公开契约以正文为准。
