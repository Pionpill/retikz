...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · [ADR-007 Shape Registry](./007-shape-registry.md)（anchor 接口先固化）· [ADR-002 nodeIndex/anchor 解析](./002-node-index-anchor-resolution.md) · 本 milestone [ADR-017](./017-side-t-edge-point.md) / [ADR-018](./018-tikz-to-layout-rename.md)
...
- `AnchorRefSchema = union(命名 anchor 枚举 | 角度 number().finite() | { side, t })`——`{ side: 'north'|'south'|'east'|'west', t: 0..1 }` 边上比例点（几何见 ADR-017）。
...
- `{ side, t }` 的几何实现（真实边界 / `edgePoint` / `resolveEdgePoint`）→ [ADR-017](./017-side-t-edge-point.md)，本篇只纳入 schema + compile 分发。
- `<TikZ>`→`<Layout>` 改名 → [ADR-018](./018-tikz-to-layout-rename.md)；命名 / 角度 anchor 的 `resolveAnchor` 缓存沿用 alpha.3。
