...
- 状态：Accepted（已实现）
- 决策日期：2026-05-16
- 关联： · [本 milestone ADR-001](./001-scope-ir-and-compile.md) · [本 milestone ADR-002](./002-node-index-anchor-resolution.md) · [本 milestone ADR-004](./004-relative-position-in-scope.md)

## 背景 / 约束

- ADR-001 给 `<Scope>` 加了可选 `id` 字段；ADR-002 决策 scope.id 与 node.id / coordinate.id 共享命名空间、冲突走 `DUPLICATE_NODE_ID`。但 **scope.id 设值时 nodeIndex 里存什么 layout** 还未拍——本 ADR 处理。
...
- Pass 1 完成 scope 子树后**立即**算 bbox（不延迟到 Pass 2，因 Pass 2 path 引用时 bbox 须已就位）：取子树**全部** NodeLayout（含嵌套 scope 内的 + Coordinate 的 0×0 layout）的 4 角点集合（`rect.anchor` 的 north-west / north-east / south-west / south-east），求 axis-aligned bbox。每个 layout 4 角已是 scope transform 累积后的全局坐标（ADR-001 Pass 1 保证），故 bbox 自动是全局 AABB。
...
- 注册到**父 namespace frame**（NameStack 栈顶，ADR-002）——**不受当前 scope 的 `localNamespace` 影响**，scope.id 是外部句柄。
...
- **synthetic layout 不发 ScenePrimitive**：只进 NameStack（compile 内部数据），不影响渲染输出；scope 视觉表达仍是 GroupPrim（ADR-001）。

## 长期边界

- scope id 冲突检测的命名空间归属 → [ADR-002](./002-node-index-anchor-resolution.md)。
