...
- 状态：Accepted（已实现）
- 决策日期：2026-05-24
- 关联： · [ADR-019 Paint 基础](./019-paint-basics.md)（复用 `PaintValue` / `SceneResource` / `<defs>` 物化）

> **实现结果**：pattern / image 复用 Paint 资源表与物化契约；image 首版仅支持 URL，compile 不新增独立资源流程。

## 背景 / 约束

ADR-019 首批只做 gradient。pattern（斜线 / 网点 / 网格）与 image（图片填充）同样靠 `fill="url(#id)"` 引用 `<defs>`——pattern → `<pattern>`、image → `<pattern>` 套 `<image>`，复用 ADR-019 全部基建。
...
4. **零 core compile 改动**：`createPaintRegistry`（ADR-019）按 JSON 去重任意 `IRPaint`，pattern / image 自动进资源表 + 拿 `resourceRef`，仅 schema + react PaintDefs 改。
