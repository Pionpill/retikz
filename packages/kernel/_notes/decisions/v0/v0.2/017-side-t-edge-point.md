...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · 本 milestone [ADR-016](./016-structured-target-anchor.md)（schema + compile 分发）· [ADR-007 Shape Registry](./007-shape-registry.md)（`ShapeDefinition` / anchor 接口）

> **目标**：[ADR-016](./016-structured-target-anchor.md) 把 `{ side, t }` 纳入 schema 并分发到 `resolveEdgePoint`；本篇定义 `resolveEdgePoint` / `ShapeDefinition.edgePoint` 算什么几何。
...
- v0.2 ADR-007 已拍板「`{side,t}` 留作内置 shape 专属，第三方仅必须支持命名 anchor」——故 `edgePoint` 设为**可选**方法。

## 决策：真实边界 + `ShapeDefinition.edgePoint?` 可选方法，内置 4 shape 必实现

`{ side, t }` 落 shape **真实边界**，各 shape 自报几何（`edgePoint?(rect, side, t)`，签名见 ）；自定义 shape 不强制，缺省时 `resolveEdgePoint` 抛 `shape 'X' does not support side anchors`。方向约定单一真源 的 `EDGE_ENDS`：**north/south = 西→东**（t=0 在 west 端）、**east/west = 北→南**（t=0 在 north 端）。三点（t=0/0.5/1）全落真实边界、与现有 9-anchor 数值重合（保证 `{side,t}` 端点与命名 anchor 无缝）：
...
- `resolveEdgePoint` 在 ，**缓存 key `${side}:${t}`** 与命名 anchor 共用 layout Map（`'north'` vs `'north:0.5'` 不冲突）。
...
- `{ side, t }` 的 schema + compile 分发 → ADR-016；offset 叠加 → ADR-016 compile 路径。
- 第三方 shape 的 `{side,t}` generic 兜底；角度 / 命名 anchor 沿用 alpha.3。

---

## 最终实现结果

已实现本 ADR 的核心决策。兼容性：向后兼容（第三方现有 shape 不实现仍合法、仅不支持 `{side,t}`）；其余默认行为、失败语义与公开契约以正文为准。
