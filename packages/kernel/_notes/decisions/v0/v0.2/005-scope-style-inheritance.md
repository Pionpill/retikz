...
- 状态：Accepted（已实现）
- 决策日期：2026-05-21
- 关联：[v0 roadmap §v0.2](../roadmap.md) · [kernel v0.2 Roadmap](./roadmap.md) · [ADR-001 Scope 容器](./001-scope-ir-and-compile.md) · [ADR-002 inside-out lookup](./002-node-index-anchor-resolution.md)
...
retikz 现状缺第 2 套：只有 `stroke` / `fill` / `textColor` 分项、**没有主色**，所以颜色跟随一直要靠补丁（ADR-006 早稿的"段级 stroke 推导"即补丁，且不 TikZ：`draw=red` 时 TikZ label 不变色，我们却让它变）。本 ADR 把这两套机制补齐。
...
2. **一个机制解决颜色跟随**：label / arrow / 文字跟色 = 读同一已解析主色，不需 host 回退树，且修掉 ADR-006"跟 stroke"的偏离（`draw=` 不跟、`color=` 跟，与 TikZ 一致）。
3. **扁平 < 树**：四通道独立、无父子轴 + 无实例 host 轴，merge 只剩 model A 一条轴。
4. **复用 alpha.1**：嵌套 fold 与 ADR-006 inside-out 同方向；compile 解析进具体 primitive，保持 renderer-neutral。
...
- **`labelDefault` 单通道双宿主**：node-label→node.color、step-label→path.color（细节 ADR-006）。
- **未知 key**：`.strict()` 严拒，错误信息列合法字段集。

## 长期边界

- **StepLabel `textColor`/`opacity`/`font` + label 继承顺序** → [ADR-006](./006-step-label-style.md)。
