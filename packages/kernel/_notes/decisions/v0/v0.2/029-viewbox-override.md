...
- 状态：Accepted（已实现）
- 决策日期：2026-05-24
- 关联： · tikz-gap-analysis §6 Scene（历史分析已删除） · 本 milestone [ADR-027](./027-partway-absolute-target.md) / [ADR-028](./028-clip.md)
...
- **与 clip 正交**（[ADR-028](./028-clip.md)）：viewBox 决定画布范围（坐标系映射），clip 决定可见裁切；clip 不改 viewBox、viewBox 不裁元素。
...
- **partway**→ [ADR-027](./027-partway-absolute-target.md)；**clip**→ [ADR-028](./028-clip.md)。
