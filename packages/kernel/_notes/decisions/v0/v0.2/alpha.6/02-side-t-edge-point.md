...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · 本 milestone [ADR-01](./01-structured-target-anchor.md)（schema + compile 分发）· [alpha.3 ADR-01 Shape Registry](../alpha.3/01-shape-registry.md)（`ShapeDefinition` / anchor 接口）

> **目标**：[ADR-01](./01-structured-target-anchor.md) 把 `{ side, t }` 纳入 schema 并分发到 `resolveEdgePoint`；本篇定义 `resolveEdgePoint` / `ShapeDefinition.edgePoint` 算什么几何。
