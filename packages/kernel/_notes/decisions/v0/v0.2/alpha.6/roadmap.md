...
10. **全量验收 + ADR 落档**：core / react / docs 三包测试全绿；ADR 集合（`packages/kernel/_notes/decisions/v0/v0.2/alpha.6/`）固化全部待定项。
...
开工前另起（位置 `packages/kernel/_notes/decisions/v0/v0.2/alpha.6/`，编号到时定），固化上节全部"待定"项，并落以下交付物：
