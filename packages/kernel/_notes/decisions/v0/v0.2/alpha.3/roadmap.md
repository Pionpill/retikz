...
>
> 关联：[`v0.2 总计划 §六段 alpha 节奏`](../roadmap.md#六段-alpha-节奏) · [`v0 roadmap §Shape Registry 提案`](./roadmap.md#shape-registry-提案) · alpha.3 ADR（`packages/kernel/_notes/decisions/v0/v0.2/alpha.3/` 已起草：[ADR-01](./01-shape-registry.md)）· [alpha.1 ADR-02 anchor 解析](../alpha.1/02-node-index-anchor-resolution.md)（anchor lookup 同方向）
...
- `apps/docs/.../kernel/**`：新增「自定义 shape / Shape Registry」章节——`ShapeDefinition` 四方法 + factory 模式示例（`createPolygonShape`）+ `CompileOptions.shapes` 注入 + 覆盖内置 warn；配 demo
...
已起草（`packages/kernel/_notes/decisions/v0/v0.2/alpha.3/`，状态 Proposed）：
