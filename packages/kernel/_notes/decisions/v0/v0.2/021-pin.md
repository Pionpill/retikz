---
description: pin 引脚（label + 引线）
keywords: 'pin、label、boundaryPointOf、strokeWidth、position、stroke'
---

# ADR-021：pin 引脚（label + 引线）

- 状态：Accepted（已实现）
- 决策日期：2026-05-24
- 关联： · tikz-gap-analysis §1 Node（历史分析已删除） · 本 milestone [ADR-019](./019-paint-basics.md) / [ADR-020](./020-max-text-width.md) · [alpha.4 Node label rotate](./)（label 体系）· [ADR-017 edgePoint](./017-side-t-edge-point.md)（引线起点边界解析）
...
- **Paint / 资源表** → [ADR-019](./019-paint-basics.md)；**maxTextWidth** → [ADR-020](./020-max-text-width.md)。
