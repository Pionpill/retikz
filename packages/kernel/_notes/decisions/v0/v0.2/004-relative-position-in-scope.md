...
- 状态：Accepted（已实现）
- 决策日期：2026-05-16
- 关联： · [本 milestone ADR-001](./001-scope-ir-and-compile.md) · [本 milestone ADR-002](./002-node-index-anchor-resolution.md) · [本 milestone ADR-003](./003-scope-id-bounding-box.md) · [ADR-001 (Node.position direction/of)](../v0.1/001-node-at-positioning.md) · [ADR-007 (OffsetPosition)](../v0.1/007-position-offset.md)
...
- referent 坐标取全局（ADR-001 / ADR-002：NameStack 内 layout 存全局坐标，lookup 走 inside-out shadowing，命中最近 frame 的 referent）。
...
- **笛卡尔字面量 `[x, y]` 同样按 scope 投影**：v0.1 行为延续，是 ADR-001 Pass 1 累积 transform 的自然结果。
- **嵌套 polar / offset 整条链都在当前 node 的 scope 局部度量**，不切换基准 scope。
- **scope chain 传到 `resolvePosition` 时已是 Cartesian-only**：ADR-001 compile 阶段已把 polar-translate 展平为 Cartesian translate，故 `applyScopeTransform` / `inverseScopeTransform` 只需处理 translate / rotate / scale 3 变体。
...
- TikZ `cm` 任意仿射矩阵下 inverse → v0.2 不引入 cm（ADR-001 已决）。
