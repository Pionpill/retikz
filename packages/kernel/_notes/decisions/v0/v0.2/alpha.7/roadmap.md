...
>
> 关联：[`v0.2 总计划 §alpha.7 设计预想`](../roadmap.md) · `tikz-gap-analysis §1 Node`（历史分析已删除） · 后续段 [`v0.2-alpha.8.md`](../alpha.8/roadmap.md)（自定义 arrow 复用本段 Paint 颜色继承）
>
> **贯穿约束（评审 P1，本段两处命中）**：① ScenePrimitive 渲染无关（`packages/kernel/core/src/primitive/scene.ts:8`：不允许 SVG-only 特性）——Paint 由 core 产 renderer-agnostic 资源表 + primitive 挂 `paintRef`，`<defs>` 物化只在 React SVG adapter；② IR 100% JSON 可序列化——`IRPaint` 字段只用 JSON 值，禁 function / `z.any`。
...
node 文本现状：`text` 是单行 string 或多行 `IRLine[]`（`ir/text.ts`），**只能手动分行**。文字度量已有注入式接口 `TextMeasurer`（`compile/text-metrics.ts:29`，react 用 canvas measureText、ssr 用 fontkit），node 布局用它算每行宽高、定外接框。
...
| 改动                              | 文件                                                    | 形态                                                                                                                                              |
| --------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 新增 `IRPaint`（gradient 基础） | 新 `ir/paint.ts`                                        | `{ type: 'linearGradient' \| 'radialGradient', stops, ... }`（纯 JSON）                                                                           |
| `fill` 升 union                   | `ir/node.ts` / `ir/path/path.ts` / `ir/scope.ts`        | `z.union([z.string(), PaintSchema])`；`string` 仍纯色                                                                                         |
...
| compile 收集 / 去重 / 派 id       | 新 `compile/paint.ts`                                   | 扫场景把 `IRPaint` 收进资源表，primitive 写 `fillRef`                                                                                           |
| adapter 物化                      | `react/src/render/`（新 `defs.tsx` + `renderPrim.tsx`） | 资源表 → `<defs>`，`fillRef` → `fill="url(#id)"`                                                                                                  |

### `IRPaint` schema（草案，ADR 固化）
...
export type SceneResource = { kind: 'paint'; id: string; spec: IRPaint };
...
- **去重 + 稳定 id 在 core**（确定性）：`compile/paint.ts` 把相同 `IRPaint`（结构化深比较）合并为一个 `{ kind:'paint' }` 资源，id 用内容 hash / 递增序号（ADR 定）——保证同输入同 id（快照稳定、SSR / CSR 一致）。
...
1. node / path / scope 的 `fill` 若是 `IRPaint` → 进 `compile/paint.ts` 收集器，换回 primitive 的 `fillRef`；若是 `string` → 原样写 `fill`。
2. scope `fill` 级联（alpha.2 样式继承）：`IRPaint` 作默认值同样能级联——继承链解析后再进资源收集（避免每个继承点重复收集，去重兜底）。
...
- 折行结果并入现有多行布局（与 `IRLine` / `align` / `lineHeight` 同管线）：折出的物理行继承该逻辑行的 `IRLine` 样式。
- 外接框宽度：**`maxTextWidth` 折行阈值语义（已拍，评审 P2#2）**——盒宽 = `min(实际最长行, maxTextWidth)`，超过阈值才折行、短文本盒子**收缩到内容不留白**（贴通用图表，Mermaid / draw.io 风格）。

### 待定（ADR）

- 显式 `IRLine[]`（已手动分行）+ `maxTextWidth` 同时给：是各逻辑行再各自折行（推荐），还是冲突报错。
...
1. **Paint schema + 类型**（`ir/paint.ts` + `fill` union 三处）：`PaintSchema`（linear/radial gradient，纯 JSON）+ `NodeSchema`/`PathSchema`/`ScopeSchema` 的 `fill` 升 `union([string, IRPaint])`；导出 `IRPaint`；`src/index.ts` 公开。**测试**：schema 接受 string（纯色，向后兼容）+ 接受 gradient IRPaint + stops <2 报错 + 非 JSON（function）被 TS / zod 拒。
...
3. **compile paint 收集 + 去重**（新 `compile/paint.ts` + 接入 `compile/node.ts` / `compile/path` / scope 级联）：`IRPaint` → 资源表（结构化去重、稳定 id）+ primitive 写 `fillRef`；纯色仍 `fill`。**测试**：同 spec 多处用 → 资源表 1 条、id 一致；不同 spec → 多条；scope 级联的 IRPaint 正确收集；纯色不进资源表。
...
5. **text width 折行**（`ir/node.ts` `maxTextWidth` + `compile/node.ts` 折行器）：贪心断行（西文按词 / CJK 按字），并入多行布局。**测试**：给定 `maxTextWidth` 长文本折多行；西文 / CJK / 混排断点；未给 `maxTextWidth` 行为不变；折行行继承 `IRLine` 样式 + `align` 生效。
...
7. **全量验收 + ADR 落档**：core / react / docs 三包测试全绿；ADR 集合（`packages/kernel/_notes/decisions/v0/v0.2/alpha.7/`）固化全部待定项；pattern / image 留**顺延 ADR 占位**（不实现）。

---

## 测试

- **Paint schema**：纯色 string 仍合法（零破坏）；linear/radial gradient IRPaint 合法；stops <2 / offset 越界 / 非 JSON 被拒。
- **资源去重 / 稳定 id**：同 `IRPaint` 在多 node 复用 → `Scene.resources` 仅 1 条、id 确定（同输入同 id，快照稳定）；不同 spec 分条；纯色不进表。
- **adapter 物化**：gradient → `<defs>` + `fill="url(#id)"`；纯色 → 无 `<defs>`、走 attribute；`var(` → inline style（现有逻辑不退）。
- **text-wrap**：`maxTextWidth` 折多行；西文按词、CJK 按字、混排；超长不可断 token 行为（ADR 定后补例）；未给 `maxTextWidth` 与现行快照一致；折行 + `align`/`lineHeight`/`IRLine` 协同。
- **pin**：引线起点 = node 边界朝 label 方向点；终点锚定；样式；多 pin；prod / 旋转 node 行为。
- **回归**：alpha.1–6 全部测试通过（基线不退）；`fill` 纯色既有用法零破坏。

## 文档

- `core/concepts/paint/`（新概念页 zh+en）：纯色 vs 渐变；`IRPaint` 形态；renderer-agnostic 资源 → `<defs>` 的说明；pattern / image 标注"顺延"。
- Node / Path / Scope 组件页 `fill` 字段：补 `IRPaint` 用法 + demo（渐变填充）。
- Node 页：`maxTextWidth`（自动换行 demo，含 CJK / 西文）+ `pin`（引脚 demo）。
- IR Schema 参考：`fill` union、`IRPaint`、`maxTextWidth`、label `pin`/`leader` 字段。
...
- pattern / image **未实现**，但 `IRPaint` union 形态为其预留（顺延 ADR 占位），不破坏后续扩展。
...
- **纯色零破坏**：`fill` 仍接受 `string`；union 只是叠加 `IRPaint` 分支。
...
- **`IRPaint` 字段**：linear gradient 方向用 `angle` 还是两端点；radial 的 center/radius 坐标系（objectBoundingBox 0..1 vs userSpace）；stop 是否支持 `currentColor` / `var()`（主题反应）。
...
- **text-wrap**：显式 `IRLine[]` + `maxTextWidth` 同给的语义；长不可断 token 硬断 / 溢出；断行器 CJK 标点禁则是否处理（倾向不处理，过细）。
...
开工前另起（`packages/kernel/_notes/decisions/v0/v0.2/alpha.7/`，编号到时定），固化上节待定项 + 落交付物：

- **ADR-01 Paint 基础**：`IRPaint`（gradient）字段清单 + **`PaintValue` 词汇表**（`string` ∪ `resourceRef` ∪ `contextStroke`，alpha.8 arrow 颜色继承依赖）+ **`SceneResource` discriminated 资源表**（去重键 / 稳定 id 在 core，alpha.9 clip 复用）+ adapter 物化 `<defs>` 边界 + scope 级联交互。
- **ADR-02 text width 自动换行**：折行算法（贪心、CJK / 西文断点）+ 与 `IRLine` / `align` / `lineHeight` / 外接框的关系。
