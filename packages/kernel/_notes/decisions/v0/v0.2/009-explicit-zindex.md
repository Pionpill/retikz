...
- 状态：Accepted（已实现）
- 决策日期：2026-05-23
- 关联： · [kernel v0 Roadmap](../roadmap.md) · [本 milestone ADR-008 IR 顺序回归](./008-ir-order-regression.md)（提供 IR 顺序基线 + sealSink 对接点）
...
- 经 ADR-008 回归后渲染 z-order **严格 = IR 顺序 = JSX 顺序**——SVG / Canvas 两 renderer 都无原生 z-index，只能靠 primitive 排列顺序。
...
- ADR-008 已把 IR 顺序基线修回来，「先按 IR 顺序、再按 zIndex 覆盖」算法的前提（稳定 IR 顺序）才成立，故本 ADR 同段紧随其后。

## 决策：可选 `zIndex` + compile 末端旁路稳定排序

Node / Path / Scope schema 加可选 `zIndex`（`z.number().int().finite().optional()`）；compile 用旁路 `Map<ScenePrimitive, number>` 记录、**不写进 primitive 本体**，在 sealSink（ADR-008 的占位回填 + 收窄回 `ScenePrimitive[]`）之后对该 sink 做 decorate-sort（`a.z - b.z || a.index - b.index`，同值按原下标 = 稳定）。
...
3. **建在 ADR-008 之上、零额外管线**：复用其 sealSink 收窄点，天然避开「占位被当普通 prim 排序」。
...
- **transformed scope 内 path 的 zIndex**：这类 path 被 ADR-008 hoist 到顶层、按自身 zIndex 在**顶层**排，不跟随所属 scope 的 zIndex（给 transformed scope 设 zIndex 不会把 hoist 出去的 path 一起抬）。是 ADR-008 hoist 限制的延续，本 ADR 显式承认、不修。

## 长期边界

- **transformed scope 内 path 的完整 z-order**：ADR-008 已锁的 hoist 限制；彻底方案（path 端点改 scope 局部坐标）留未来坐标系 ADR。本 ADR zIndex 对这类 hoist path 只能在顶层覆盖。
- **`GroupPrim.meta` / `data-node-id` 钩子**：与 zIndex 无关，留未来。
- **带文本 Node 包 `<g>`** → [ADR-010](./010-text-node-group-wrap.md)；**Node label rotate** → [ADR-011](./011-node-label-rotate.md)。
