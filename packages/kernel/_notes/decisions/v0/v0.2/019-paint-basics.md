...
- 状态：Accepted（已实现）
- 决策日期：2026-05-24
- 关联： · tikz-gap-analysis §1 Node（历史分析已删除） · [alpha.2 样式继承](./)（`fill` 级联）· 下游 [v0.2 ADR-023 ArrowDefinition](./)（复用 `PaintValue.contextStroke`）· [v0.2 ADR-028 clip](./)（复用 `SceneResource` 资源表）· 本 milestone [ADR-020](./020-max-text-width.md) / [ADR-021](./021-pin.md) / [ADR-022](./022-pattern-image-deferred.md)
...
- **pattern / image** → [ADR-022](./022-pattern-image-deferred.md)（复用本 ADR 基建实现，实现期由 deferred 提升到 alpha.7 一并做）。
- **maxTextWidth** → [ADR-020](./020-max-text-width.md)；**pin** → [ADR-021](./021-pin.md)。
- **arrow 颜色继承用 `contextStroke`** → v0.2 ADR-023（本篇只定 `PaintValue` 词汇表，不产 arrow marker）。
- **clip 资源** → v0.2 ADR-028（本篇只把 `SceneResource` 定成 discriminated，clip 分支在 alpha.9 加）。
