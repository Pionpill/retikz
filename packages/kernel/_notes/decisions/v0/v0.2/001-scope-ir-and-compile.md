...
- 状态：Accepted（已实现）
- 决策日期：2026-05-16
- 关联：[v0 roadmap §v0.2 预备](../roadmap.md) · [v0.2 总计划](./roadmap.md) · [ADR-004 (Scene 结构化)](../v0.1/004-scene-primitive-structured.md)

## 背景 / 约束

- v0.1 的 IR 扁平：`Scene.children: Array<Node | Path | Coordinate>`，无分组容器；TikZ 用户用 `\begin{scope}` 做局部 transform + 样式作用域 + 结构分组，IR 层却没有对应基元。
- Scene Tier 3 已有 `GroupPrim`（带 `transforms` 数组，[Kernel v0.1 ADR-004](../v0.1/004-scene-primitive-structured.md) 结构化的产物）承担 transform 复合的实际渲染——IR Scope 有自然的下沉目标。
...
- `id` 可选：缺省 → 仅分组 / transform / 样式作用域；设值 → 注册 bbox synthetic layout 进**父 namespace frame**（不受 `localNamespace` 影响——scope.id 是 scope 的"外部句柄"，必须可被外部引用；bbox 语义详 ADR-003）。
- `localNamespace` 可选 boolean（缺省 false）：true → 创建本地 namespace 边界，子 id 不向父 frame 传播、外部不可见；最上层 `<Layout>` 才是真正全局 frame（namespace stack / lookup / duplicate 规则详 ADR-002）。
...
- nodeIndex 跨 scope ID 唯一性 / anchor 投影 / id 冲突检测 → [ADR-002](./002-node-index-anchor-resolution.md)。
- scope.id 触发的 bbox 注册 / synthetic layout 计算 / scope 作为引用整体的 anchor 语义 → [ADR-003](./003-scope-id-bounding-box.md)。
- scope 下相对定位（referent 投影规则）→ [ADR-004](./004-relative-position-in-scope.md)。
...
- scope rotate 下 bbox 是 axis-aligned vs rotation-aware → ADR-003 默认 axis-aligned 全局 bbox，rotation-aware 留后续。
