...
> 关联：[`v0.3 roadmap`](../roadmap.md) · [`v0 roadmap`](../../roadmap.md) · 范式参照：[`v0.3-alpha.2 ADR-01 Tier 2 支撑`](../alpha.2/01-tier2-support.md)（passthrough + 注册表）· 下游动机：[`plot v0.1-alpha.4`](../../../../../../viz/_notes/decisions/plot/v0/v0.1/roadmap.md)（polar 扇形需可连接 sector）
...
| [04](./04-rectangle-polygon.md)           | rectangle/polygon                 | rectangle 参数化（`roundedCorners` 从顶层迁入 params）；regular polygon（`sides` / `rotate`）；`diamond` 收为 polygon preset，并支持四边形 `aspectRatio`                                                                                                                                                                                  | ADR-01                                                                                 | Accepted |
...
- **preset 别名**：`circle`（→ ellipse equal）、`diamond`（→ polygon 4/0）作 preset 别名保留；对象形式可为四边形增加 `aspectRatio`，单一几何实现 + 旧写法兼容，与「circle 收 ellipse」同思路。
