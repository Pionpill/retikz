...
- 状态：Accepted
- 决策日期：2026-06-09
- 关联：[`v0.3-beta.1 roadmap`](./roadmap.md) TODO-2 · **直接前置（还账）**：[v0.3-ADR-020 runtime 播放控制](./020-runtime-control.md) `§实现说明 → 后续（未在本批）` ②③——本 ADR 就是兑现那两条 · **机制基座**：[v0.3-ADR-019 Canvas 播放](./019-canvas-playback.md)（`drawScene(ctx, scene, { time })` 单帧求值 + 共享 `evaluateTrack`）· [v0.3-alpha.3 水合](./008-hydration.md)（rAF / IntersectionObserver / 事件委托 / DOM 挂载 runtime 基建）· [v0.3-ADR-002 Canvas renderer](./002-canvas-renderer-and-react-canvas-mode.md)（meet-fit 坐标映射矩阵）
...
但 v0.3 ADR-020 在 `§实现说明 → 后续（未在本批）` 里**白纸黑字留了两条 Canvas 欠账**：
...
Canvas 是 **即时模式**：v0.3 ADR-019/020 确立的模型是**一条 scene 级共享 rAF 时钟 + 逐帧重绘整个 scene**，没有 per-prim DOM 节点可挂。所以非自动播触发的真问题不是「接个回调」，而是：
...
`evaluateTrack`（v0.3 ADR-019 共享引擎）零改动；`applyPrimAnimations` 不新增 trackKey 参数，继续吃 `includeNonAutoplay`。如后续确实需要同一 id 下多条 `manual` track 彼此独立启动，再单独升级 `resolvePrimAnimation` / `applyPrimAnimations` 的 per-track API，不能在本批暗改。
...
- 无 DOM / SSR / Node 截帧 → **不自动触发 `visible`，渲染 base**（不崩）。这与 v0.3 ADR-020「不要求 SSR / 无 DOM 环境自动触发 visible」一致，也与 ADR-017「三事一路」（不支持后端 / reduced-motion / `{animate:false}` 殊途同归到 base）同源。
...
3. **复用既有基建防重造**——`IdClockRegistry` / `resolvePrimAnimation`、`evaluateTrack`（ADR-019）、事件委托（alpha.3）、meet-fit（alpha.1）、scene 级时钟（ADR-020）全部复用，本 ADR 只加「事件/visible 激活接线 + 测试补齐」。
