...
- 状态：Accepted（已实现）
- 决策日期：2026-06-01
- 关联：[ADR-007 延后项](./007-composite-authoring-context-cache.md)
...
当前 expand 不接上下文，也没有 per-composite lowering cache；需要递归器、warning 或 options 时，以及实测展开成为瓶颈时，按 [ADR-007](./007-composite-authoring-context-cache.md) 的边界另行落地。
