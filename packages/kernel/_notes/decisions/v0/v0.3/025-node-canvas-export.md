...
- 状态：Accepted
- 决策日期：2026-06-09
- 关联：[`v0.3-beta.1 roadmap`](./roadmap.md) TODO-3 · **复用机制**：[v0.3-ADR-002 Canvas renderer](./002-canvas-renderer-and-react-canvas-mode.md)（`drawScene(ctx, scene)` / `renderToCanvas`、meet-fit 映射）· [v0.3-ADR-019 Canvas 播放](./019-canvas-playback.md)（`drawScene(ctx, scene, { time })` 单帧 = 截帧）
...
入口本体就是「createCanvas → getContext → drawScene → encode」这几行；动画截帧天然支持（`drawScene(ctx, scene, { time })`，v0.3 ADR-019 已定单帧 = 截帧）。
