...
- 决策日期：2026-06-02
- 关联：[ADR-006 Tier 2 支撑](./006-tier2-support.md)

## 背景与边界

ADR-006 已冻结 IR 后 lowering、注册表、schema 校验、fixpoint 和错误语义。本 ADR 只记录三个仍保持 core lowering 所有权的后续扩展，避免 adapter 各自形成平行机制。
...
当前实现维持 ADR-006 的纯 expand(node) 形态；本 ADR 没有实现变更。未来三项均应以 additive 方式演进，不改变 IR 后展开和未注册/环/深度错误语义。
