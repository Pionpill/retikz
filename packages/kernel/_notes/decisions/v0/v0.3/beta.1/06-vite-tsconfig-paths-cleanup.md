...
- `vite-tsconfig-paths: ^6.1.1` 被 **7 个 workspace** 的 `vite.config.ts` 使用：`packages/kernel/{core,render,react,vanilla}` + `packages/viz/{plot,plot-react,plot-vanilla}`。
...
- `packages/viz/{plot,plot-react,plot-vanilla}/vite.config.ts`
