...
- 状态：Accepted
- 决策日期：2026-06-09
- 关联：[`v0.3-beta.1 roadmap`](./roadmap.md) TODO-1 · **几何契约前置**：[v0.2-alpha.8 ADR-01 arrow 定义](../../v0.2/alpha.8/01-arrow-definition.md)（`ArrowDefinition` / `BUILTIN_ARROWS` / `hollow` + `lineContactX` 静态 base 范式）· [v0.1-alpha.5 ADR-03 arrow detail](../../v0.1/alpha.5/03-path-arrow-detail.md)（`fill` 在空心 shape 上 silent no-op）
...
- `apps/docs/src/contents/kernel/components/draw/arrow/*`（demo + API 表）
- `apps/docs/src/contents/kernel/reference/schema/path/*`、`extending/custom-arrow/*`（若文中硬列了 7 名清单）
