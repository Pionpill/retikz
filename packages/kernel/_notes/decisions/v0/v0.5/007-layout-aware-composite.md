...
- 状态：Accepted
- 决策日期：2026-07-23
- 关联：[ADR-015](./015-box-layout-composite-contract.md) · [ADR-017](./017-layout-proposal-probe-contract.md)
...
```ts
type ChildLayoutConstraint = { kind: 'intrinsic' } | { kind: 'constrained'; maxWidth: number };
...
type CompileResult = Readonly<{
  scene: Scene;
  artifacts: ReadonlyArray<CompositeCompileArtifact | NodeLayoutCompileArtifact>;
}>;
```
