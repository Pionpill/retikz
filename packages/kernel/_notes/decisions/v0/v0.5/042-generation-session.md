...
- 状态：Proposed（未排期）
- 决策日期：2026-07-26
- 关联：[ADR-040](./040-cooperative-concurrent-runtime.md) · [ADR-041](./041-progressive-materialization.md)
...
LLM 渐进生成需要的是多个语义合法的 draft transaction。它与 ADR-041 的 progressive materialization 正交：前者逐批改变 draft Snapshot，后者只决定同一个完整 Scene 的显示节奏。Kernel 也不应拥有模型 SDK、prompt 或聊天 UI。
