...
> 状态：`v0.5.0-alpha.1` 与 `v0.5.0-alpha.2` 已完成；alpha.3 只交付上下文颜色，alpha.4 仅登记 Headless Interaction 候选边界，alpha.5 Proposed 补充 Stroke Path 居中标签断线；Concurrent、progressive materialization 与 generation session 仍是未排期 Proposed
>
...
| Stroke Path 标签断线      | 居中标签不再被无填充描边穿过，并保持跨 renderer 的真实几何断口                                   | [alpha.5 ADR-01 Proposed](./alpha.5/01-stroke-path-label-interruption.md)      |
...
| alpha.5 | 无填充 Stroke Path 的居中标签自动断线；显式 `interrupt` 覆盖默认策略                                                                                                                      | [ADR-01](./alpha.5/01-stroke-path-label-interruption.md)                                                                                                                                                                                                                                                           |
