...
> 状态：`v0.5.0-alpha.1` 与 `v0.5.0-alpha.2` 已完成；alpha.3 只交付上下文颜色；alpha.4 的 Path 端点箭头视觉后缘重叠与 Source owner schema 单次 parse 边界统一已完成，Stroke Path 居中标签断线与 Core Source IR 浅层语义分组仍为 Proposed，Headless Interaction 仍只登记候选边界；Concurrent、progressive materialization 与 generation session 仍是未排期 Proposed
>
