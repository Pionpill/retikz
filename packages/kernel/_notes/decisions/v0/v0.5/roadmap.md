...
> 状态：`v0.5.0-alpha.1` 与 `v0.5.0-alpha.2` 已完成；alpha.3 只交付上下文颜色；当前 alpha.4 承接 Stroke Path 居中标签断线、Path 端点箭头视觉后缘重叠与 Foundation Source JSON 快照 / runtime known-key 省略统一，Headless Interaction 仍只登记候选边界；Concurrent、progressive materialization 与 generation session 仍是未排期 Proposed
>
...
| Foundation 基础契约统一   | 为跨包原子类型、typed string、结构化错误、Source JSON 快照与 runtime 字段省略建立唯一 Kernel owner | [alpha.2 ADR-14](./alpha.2/14-foundation-package.md) · [alpha.4 ADR-03](./alpha.4/03-json-undefined-field-contracts.md) |
...
| alpha.4 | 无填充 Stroke Path 的居中标签自动断线；端点箭头按归一化比例推进到视觉后缘完整进入；Foundation 以一次 JSON 快照统一 Source JSON-safe / undefined，并提供独立的 runtime known-key omission；Headless Interaction 只保留候选边界 | [ADR-01](./alpha.4/01-stroke-path-label-interruption.md) · [ADR-02](./alpha.4/02-path-endpoint-arrow-overlap.md) · [ADR-03](./alpha.4/03-json-undefined-field-contracts.md) · [交互设计](../../../../../../notes/architecture/interaction-design.md)                                                               |
