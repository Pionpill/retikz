...
| [ADR-14](./14-lightweight-theme-resolution.md)       | Proposed   | 轻量 Theme IR 与领域 Token 解析        | 冻结 selector-only Theme、Core shared colors 与 owner-local token resolution                        |
...
- ADR-13 的 namespaced Theme token bag、Definition registry 与跨 Scope token override 决策已由 ADR-14 取代；其 shared colors 与领域 owner resolver 边界由 ADR-14 继承并收窄。
- ADR-14 已完成 Proposed 设计，等待 Architecture Gate 与人工确认后才进入实现。
