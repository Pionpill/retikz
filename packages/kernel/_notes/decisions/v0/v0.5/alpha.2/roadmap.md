# v0.5.0-alpha.2 增量性能、Runtime 策略、Box Layout、Theme 与 Inspector 基建

- 状态：ADR-01～10 已完成实现、测试、双语文档与 Accepted 收口；ADR-11 Proposed，alpha.2 为 Inspector 内容抽象重新打开
- 目标版本：`0.5.0-alpha.2`
...
| [ADR-11](./11-extensible-inspector-content.md)       | Proposed | 可扩展 Inspector 内容                | 冻结 owner-attached Inspector、可编译辅助 IR、隔离 Scene 与 Path 控制点闭环                |
...
- ADR-11 已完成 Proposed 设计并获得人工方向确认与实现授权；普通 IR child、既有 Definition registry、Path kind 非 Composite 挂载、测试与双语文档已落地，正在进行完整改动评审，尚未获得人工 Accepted 收口。
...
| 10   | ADR-11 | Standard Layout Inspector 已暴露闭合 primitive 限制    | owner subject、通用 sidecar、静态辅助 Scene、诊断 provenance 与 Path 控制点闭环稳定 |
...
8. Inspector 只读取最终 settled subject；辅助内容继承 occurrence 的有效 Theme / style 并复用普通 IR / Definition / compile，但使用隔离 namespace，seal 后不保留 public id / meta / animation，且与主 Scene 的 layout、resource、identity、artifact、patch、命中和水合语义隔离。
...
- Inspector 可以返回任意已注册、可编译的 Core IR child，并在关闭递归 inspection 的隔离 channel 中生成 occurrence-local 静态 Scene；内置与第三方 owner 依附既有 Definition registry，Path kind subject/schema、Composite / Path target sidecar 和 inspection diagnostic origin 可追溯，quadratic / cubic Path 控制点在 React / Vanilla 与 SVG / Canvas 上等价，且主 Scene 保持不变。
