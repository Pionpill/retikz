# ADR-05：Theme 稀疏 Source 片段与覆盖协议

- 状态：Proposed
- 决策日期：2026-09-05
- 主责：Core 的跨包 Theme 协议；领域 Source、Theme 与规则由各领域 owner 持有
...
领域 Theme 的外层选择实际消费目标类型，目标片段内部复用对应 Source 的同名字段、分组、值域和覆盖语义。实例集合不按索引主题化；entity 片段对应 Entity Source，不对应 entities 数组的某个元素。preset、definition 输出和用户覆盖采用同一目标片段。
...
Core 协议归属不使领域包加入 Kernel 发布组。该 ADR 冻结共同协议；各领域精确 Theme 字段集合、规则作用域和与作者默认通道的优先级，由对应 owner ADR 冻结后实施。该过程不允许保留 flat / nested 双轨作为长期公共契约。
...
```ts
type NodeThemeFragment = Pick<IRNode, 'style' | 'layout'>;
type PathThemeFragment = Pick<IRPath, 'style'>;
```

领域按自身语义进一步收窄：Graph Entity 不能因复用 Node layout 而获得角色禁止的 padding；Graph Relation 不能因复用 Path style 而开放 fill。Theme 允许的字段集合不会因为底层 Source 新增字段而自动扩大。
...
对应 Entity 实例使用相同的 style、layout 子树。Theme 不包含该实例的 id、text、role、status、position 或关系引用。
...
领域的每个 Theme 片段必须具有正式 Source 或命名配置目标。缺少目标表达时，由领域先冻结 Source，再派生 Theme；不得把最终 appearance 当持久化作者输入。

presentation 区域的内容与格式应共存于正式区域对象，Theme 只选择格式字段。Theme 不创建省略的标题、Axis、Legend、Mark 或 Relation；已有对象内的 baseline、grid、tick 或 label 装饰可按 owner 明确允许的字段隐藏。

Chart 只转发 Plot 的公开 Theme 片段，不能维护同义的 Plot token vocabulary 或复制其 resolver。Flow 复用 Graph 的受限输入与语义解析，不绕过角色限制。Table 的 Cell 默认与有条件的列头覆盖分别复用其正式输入和已有规则能力。

### 规则覆盖

基础 Theme 与 selector rules 是独立契约。规则保持目标 type 与 selector，作者覆盖使用对应 Source 字段，例如：
...
规则允许字段可以窄于基础 Theme。颜色状态规则不因此获得布局覆盖能力；selector 中的 status、role、direction 只是匹配条件，不成为赋值。现有语义状态默认与有序规则能力必须保留，规则容器的精确字段和作用域由领域 ADR 定义。已确定的 appearance 继续用于 resolve / manifest / inspection，不新增同义的作者 appearance 包装。
...
Core 先解析环境。领域从 shared colors 建立 baseline，本地 style definition 的显式值覆盖共享 baseline，owner-local Theme 再覆盖对应默认；实例显式配置高于 Theme 默认。缺省 categorical 来源是 Core，显式领域 palette 可覆盖它，实例 scale range / scheme 最终胜出。
...
- 默认行为：省略 Theme 或提供空片段表示无本地覆盖，继续使用有效环境与领域 baseline；Theme 不生成省略的语义对象
...
- 兼容性：领域迁移属于公开 Source breaking change；删除旧 flat token、同义 appearance 作者入口、旧字段别名与双向 projection，不保留兼容 schema 或双读模式
...
- 可观察结果：Theme 可改变允许的样式和表现布局，但不改变数据、identity、成员拓扑或语义对象集合；来源诊断不得依赖值相等猜测
