...
- 状态：Accepted（已实现）
- 决策日期：2026-05-13
- 关联： · [ADR-004 PathPrim 结构化](./004-scene-primitive-structured.md)
...
- `THICKNESS_TO_WIDTH` 的 `Record<NonNullable<IRPath['thickness']>, number>` 与 thickness enum 字面重复——新增档位时漏写哪一边 TS 不抓（"字段表漂移"主题之一，本 ADR 只处理 THICKNESS，其余字段表互锁见 ADR-013）。
...
- builder / unbuilder 字段表化（）→ ADR-013。
- `_builder.ts` cast 收敛（）→ ADR-014。
- arrow marker `stableArrowKey` 字段表（）→ 并入 ADR-013。
