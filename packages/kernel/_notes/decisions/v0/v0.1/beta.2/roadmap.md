...
>
> 关联：[`v0 roadmap`](./roadmap.md) · [`packages/kernel/AGENTS.md`](../../../../../AGENTS.md) · [`beta.1 ADRs`](../beta.1)
...
本文件登记 beta.2 的 TODO 列表。完工后把已落地条目摘要写进 `v0/roadmap.md` 的 v0.1 跟踪段（本文件保留留档）。
...
- `_notes/` 内提到 `ViewBox` 类型的位置（不动 SVG attribute `viewBox` 字面）
...
- TikZ 使用 `dash pattern`，当前公开 `strokeDasharray` 来自 SVG/CSS。beta.2 若继续收敛公开命名，应评估 `strokeDasharray` → `dashPattern: Array<number>`，并同步 `<Path>` / `<Draw>` / docs / changelog。
- TikZ 填充规则使用 nonzero winding number rule / even odd rule，当前 `fillRule: 'nonzero' | 'evenodd'` 语义可接受，不需要调整。

Level: breaking（执行 `strokeDasharray` → `dashPattern: Array<number>` 时）。如果本轮不做公开字段 rename，需至少把 docs 中“SVG 原生语法逃生口”的表述调整为结构化 dash/gap 长度序列，避免继续强化 SVG 绑定。
