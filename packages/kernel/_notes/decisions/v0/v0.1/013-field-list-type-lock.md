...
> **目标**：把"硬编码字段表与对应 zod schema / type 的 key 集合保持完备一致"这件事确立为通用 idiom（`as const satisfies` + `AssertEqual` 双约束），并覆盖 builder/unbuilder 的 `NODE_FIELDS`、marker `stableArrowKey` 两处应用。ADR-012 的 `THICKNESS_TO_WIDTH` 是同一主题先例。
