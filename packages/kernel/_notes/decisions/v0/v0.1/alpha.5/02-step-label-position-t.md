...
> **目标**：把 edge label 的 `position` 从 3 keyword 扩成 TikZ 完整集（7 keyword + 任意数值 t∈[0,1]），并把"t 在每种 step kind 上的几何含义"写定。
...
keyword 是 t 的语义糖（一对一映射），数值 t∈[0,1] 提供任意位置。7 keyword → t：`at-start`=0 / `very-near-start`=0.125 / `near-start`=0.25 / `midway`=0.5（默认）/ `near-end`=0.75 / `very-near-end`=0.875 / `at-end`=1。
...
已实现本 ADR 的核心决策。兼容性：BREAKING——`position` 值集扩大，旧 exhaustive switch 新值落 default）；其余默认行为、失败语义与公开契约以正文为准。
