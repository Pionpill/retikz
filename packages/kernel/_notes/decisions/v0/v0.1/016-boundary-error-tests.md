...
- **NaN / Infinity 坐标：过滤 + warn**——viewBox 计算过滤非有限坐标、经 ADR-015 `onWarn` 触发 warning，不抛错（viewBox 退化优于 crash）。
...
- 性能 benchmark（如 100 step path compile 耗时）——留 ADR-012 拆 compile/path 时一并。
