...
本文件覆盖 `packages/kernel/` 下的 `math` / `runtime` / `core` / `render` / `react` / `vanilla` / `tex`。全仓通用规则见根 [`AGENTS.md`](../../AGENTS.md)，包内细则看就近 `AGENTS.md`。
...
依赖方向：`runtime` 保持零依赖；`math` 被 `core` 消费；`core` 消费 `runtime` 与 `math`；`render` 消费 `runtime` 与 `core`；`react` / `vanilla` 消费 `runtime`、`core` 与 `render`；`tex` 消费 `core`。不要让 runtime 反依赖领域包，也不要让 core 反依赖公式、React、Vanilla 或具体渲染后端。
...
七个包同属 core 组，版本号保持 lockstep：任一包发布时，其余包同步 bump 到同一版本并一并发布。包间依赖用 `workspace:*`，对外发布时由发布流程替换为固定版本。
