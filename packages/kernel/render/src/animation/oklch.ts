...
/** 解析 hex（含 `#rgba` / `#rrggbbaa`）/ rgb(a) 颜色串 → 线性 sRGB + alpha；无法解析返回 null */
const parseToLinear = (color: string): LinearRgb | null => {
  const value = color.trim();
  const hex8 = /^#([0-9a-fA-F]{4}|[0-9a-fA-F]{8})$/.exec(value);
  if (hex8) {
    let h = hex8[1];
  ...
  if (rgb) {
    ...
    if (parts.length >= 3) {
      ...
      return {
        r: gammaToLinear(channel(parts[0])),
        g: gammaToLinear(channel(parts[1])),
        b: gammaToLinear(channel(parts[2])),
...
/** 解析 `oklch(L C H[ / a])`（L 接受 0..1 或百分比；H 缺省 0；alpha 缺省 1）→ Oklch；非该形态返回 null */
...
/** 线性 sRGB → oklch（保留 alpha） */
const linearToOklch = ({ r, g, b, alpha }: LinearRgb): Oklch => {
...
/** 在 oklch 空间按 t 插值两端点（含 L/C/H 环绕 + alpha） */
const lerpOklch = (a: Oklch, b: Oklch, t: number): Oklch => ({
  L: a.L + (b.L - a.L) * t,
  C: a.C + (b.C - a.C) * t,
  H: a.H + hueDelta(a.H, b.H) * t,
