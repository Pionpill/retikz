...
describe('Happy：drawScene({time}) 应用通道', () => {
  ...
  it('自定义 property → 注册 def 的 applyCanvas 被调', () => {
    const apply = vi.fn();
    const def: AnimationPropertyDefinition = {
      interpolate: (a, b, t) => (a as number) + ((b as number) - (a as number)) * t,
      applyCanvas: apply,
    };
...
describe('trigger 过滤（auto 时钟只施加 load/缺省 track）', () => {
  ...
  it('onEvent / visible track 同样不被 auto 时钟施加', () => {
    const onEvent: IRAnimationTrack = {
      property: 'translateX',
      keyframes: [
        { at: 0, value: 0 },
        { at: 1, value: 100 },
    ...
    const visible: IRAnimationTrack = {
      property: 'translateY',
      keyframes: [
        { at: 0, value: 0 },
        { at: 1, value: 100 },
...
describe('交互', () => {
  ...
  it('camera：多条自动播 viewBox track 逐层叠加（非只取首个）', () => {
    ...
    const camB: IRAnimationTrack = {
      property: 'viewBox',
      keyframes: [
        { at: 0, value: [0, 0, 100, 100] },
        { at: 1, value: [0, 0, 50, 100] },
      ],
      duration: 400,
    };
    const ctx = createCtx();
...
describe('per-id 虚拟时钟（registry + resolvePrimAnimation）', () => {
  it('resolvePrimAnimation skip → 跳过全部 track（渲染 base，无 translate）', () => {
    ...
    const ctx = createCtx();
    drawScene(ctx, scene([rect({ id: 'a', animations: [t] })]), {
      time: 200,
  ...
  it('manual track：includeNonAutoplay=false 不施加；=true 施加', () => {
    ...
    const off = createCtx();
    drawScene(off, scene([rect({ id: 'a', animations: [t] })]), {
      time: 200,
