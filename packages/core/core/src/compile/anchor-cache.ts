 ...
 *   `anchor()` / 数字角度 `boundaryPoint(toward=旋转后单位向量)` 调用统一收编到 `resolveAnchor(layout, name, surface?)`，
...
import type { IRConnectSurface, IRPosition } from '../ir';
import { anchorOf, angleBoundaryOf } from './node';
import type { NodeLayout } from './node';
import { surfaceKey } from './connectSurface';
 ...
 * @description 数字字符串走 angleBoundaryOf；其余按 RectAnchor 走 anchorOf；surface 透传给两者
 */
const computeAnchor = (
  layout: NodeLayout,
  anchorName: string,
  ...
  if (ANGLE_RE.test(anchorName)) {
    ...
    return positionToIR(angleBoundaryOf(layout, angle, surface));
  }
  ...
  return positionToIR(anchorOf(layout, anchorName, surface));
};
 ...
 *   surface 指定连接面（默认 `'shape'`，即节点自身视觉轮廓）；不同 surface 产生独立缓存条目，互不串扰；
 *   同一 (layout, name, surface) 组合第二次起返回首调用结果的**同一引用**——上游可用 `===` 判定 cache 命中
 * @param layout 已 Pass 1 完成的 NodeLayout（rect 已是全局坐标）
 * @param anchorName 关键字或数字角度字符串
 * @param surface 连接面，缺省为 `'shape'`（视觉轮廓）
...
export const resolveAnchor = (
  ...
  const key = `${surfaceKey(surface)} ${anchorName}`;
  const cached = layoutCache.get(key);
  if (cached !== undefined) return cached;
  const result = computeAnchor(layout, anchorName, surface);
