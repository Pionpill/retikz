 ...
 * @description `'+dx,dy'` → `{rel:[dx,dy]}`（TikZ `+`，不更新 prevEnd）；`'++dx,dy'` → `{relAccumulate:[dx,dy]}`（TikZ `++`，累积）；其他形态（节点 id、笛卡尔/极坐标、已是 rel 对象）原样返回。纯函数无 nodeIndex 依赖，react adapter 与 Draw DSL 共用
 */
