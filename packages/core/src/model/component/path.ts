...
export default class PathModel {
  ...
  init = false;
  disposed = false;
  ways: Array<Array<Position>> = [];
  lineWidth = 1;
  listeners = new Set<StateListener>();

  constructor(ways: Array<Array<Position>>, lineWidth: number, init = true) {
    this.update({ ways, lineWidth, init });
  }

  update(config: { ways?: Array<Array<Position>>; lineWidth?: number; init?: boolean }) {
