...
import type { Direction } from '../../types/coordinate';
...
const PathNode = forwardRef<SVGGElement, PathNodeProps>((props, ref) => {
  ...
  const { pos, veryNearStart, veryNearEnd, start, nearStart, midway, nearEnd, sloped, ...nodeProps } = resProps;
  ...
  const posRadio = useMemo(() => {
    ...
    return 1;
  }, [pos, veryNearStart, veryNearEnd, start, nearStart, midway, nearEnd]);
  ...
  const adjustOffset = useMemo(() => {
    const model = getModel(realName);
    if (!model || !model.init) {
      return DescartesPoint.plus(anchorPosition, offset);
    }
    ...
    return DescartesPoint.plus(anchorPosition, directionPosition, offset);
  }, [anchorPosition, directionPos, getModel, offset, realName]);
