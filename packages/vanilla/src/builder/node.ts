...
/**
 * 构造一个 node IR 子节点
 * @description `node(config)` 匿名；`node(id, config)` 具名（id 提为首 positional，可选）。纯构造 IRNode
 *   （`{ type:'node', id?, ...config }`）；字段不自列、必填项（如 position）由 core schema 在 compile 期校验，
 *   故构造边界用一处断言收口（builder 故意宽松、缺字段不在此拦、交 compileToScene 报错）。
 */
