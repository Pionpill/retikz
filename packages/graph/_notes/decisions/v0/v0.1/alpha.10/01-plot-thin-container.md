...
落地于 `@retikz/plot`（`plot/src/ir/plot.ts`、`lower/*`）与 `@retikz/plot-react`（`react/src/Plot.tsx`、`components/build-plot-spec.ts`、`components/scales.tsx`），行为与 schema 覆盖见 `packages/graph/plot-react/tests/**`、`packages/graph/plot/tests/**`；文档同步见 `apps/docs/src/contents/graph/**`。
