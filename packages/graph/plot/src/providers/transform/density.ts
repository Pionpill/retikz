...
const silvermanBandwidthOf = (sortedValues: Array<number>): number => {
  if (sortedValues.length < 2) {
    throw new Error('lowerPlots: density transform with Silverman bandwidth requires at least two finite samples; pass an explicit bandwidth for single-value groups');
  }
  ...
  if (!isFiniteNumber(bandwidth) || bandwidth <= 0) {
    throw new Error('lowerPlots: density transform could not compute a positive Silverman bandwidth; values may be identical, pass an explicit bandwidth');
  }
  return bandwidth;
};
...
const sampleExtentOf = (operation: DensityTransform, sortedValues: Array<number>, bandwidth: number): [number, number] => {
...
export const densityInputFields = (operation: DensityTransform): Array<string> => [operation.field, ...(operation.groupBy ?? [])];
...
export const applyDensity = (rows: Array<ExternalRow>, operation: DensityTransform, context: TransformContext): Array<ExternalRow> =>
