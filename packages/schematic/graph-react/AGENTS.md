...
Graph v0.1 alpha.1 ADR-01～10 与 alpha.2 ADR-03 已形成 Accepted 的现行契约，alpha.2 ADR-01 已 Superseded，ADR-02 保持 Proposed。React JSX 分别归一到 Graph、Group、Block family、Entity 与 Relation Source IR；Block 接受任意 children，Header / Section / Row 独立嵌入，Row 同时支持 text content 与任意直接 children。authoring 不保存 ReactNode、不创建默认 id，也不解释布局或 endpoint。
