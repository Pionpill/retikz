...
Graph v0.1 alpha.1 已完成 ADR-01～08，ADR-09 为 Proposed。目标契约是 Graph、Entity 与 Relation 三个独立 Source composite：Vanilla 分别 normalize 对应 typed input，只展开字符串 endpoint、Relation `way` 等 authoring sugar，不创建隐式 Graph wrapper、默认 id、appearance 或 geometry wrapper。
