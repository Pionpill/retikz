...
const compileContainerWithOptions = (
  node: IRContainer,
  context: LayoutCompositeCompileContext,
  ...
  const regions = regionsOf(node).map(region => ({
    ...region,
    child: lowerGraphPresentationChildren([region.child], {
      ...options,
      theme: context.theme,
...
/** 创建使用指定 Graph registries 的 Container compile callback */
export const createCompileContainer =
  (options: ResolvedGraphDefinitionOptions) =>
  (node: IRContainer, context: LayoutCompositeCompileContext): LayoutCompositeCompileResult<ContainerArtifact> =>
    compileContainerWithOptions(node, context, options);

/** 使用内置 Graph registries 的默认 Container compile callback */
export const compileContainer = createCompileContainer(resolveGraphDefinitionOptions());
