...
- Entity 与 Relation 是可独立放入任意 Core 内容树的 semantic composite；Graph 组合完整 Core Scope surface，并额外提供 `graphTheme`，不维护成员集合、私有 identity 索引、Graph-only child grammar、Variant 视觉轴或隐式 local namespace
...
Graph v0.1 alpha.1 的 ADR-01～09 均已 Accepted，当前实现正按无 Variant Theme 与独立 composite 契约同步收敛。目标契约是 Graph、Entity 与 Relation 三个独立 Source composite：Entity / Relation 复用 Core Node / Path lower-facing surface，Graph 复用完整 Core Scope surface，children 与 Core `IRChild` 同源；Graph context 只影响其可见的 Graph 语义后代，React 与 Vanilla 只提供同一 Source IR 的 authoring sugar。
