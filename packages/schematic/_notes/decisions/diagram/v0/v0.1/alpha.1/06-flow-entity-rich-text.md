# ADR-06：Flow Element 复用 Core 富文本

- 状态：Accepted；Relation label 扩展待 Core ADR-01 接受并实施
- 决策日期：2026-09-02
...
Core ADR-01 接受并实施后，Flow Relation 的可选 `label` 也直接采用完整 Core `TextBlock`。它在 Flow resolve / materialize 后成为一个 Graph `GeometryLabel`，由 Core 作为整体路径标签测量、定位、绘制和（适用时）断线。Flow 不创建 Relation text、label line、label bbox 或 renderer primitive；多行不能拥有彼此独立的 position、side、rotation 或 relation identity。
...
`IRFlowEntity.text` 的公开类型是 Core `IRTextBlock`；Core ADR-01 落地后，可选的 `IRFlowRelation.label` 也使用同一类型。它保留简单字符串写法，也接受非空行数组；每一行可以是默认文本、带 `fill` / `opacity` / `font` 覆盖的文本，或 Core 已有的 mixed text / math line。例如：
...
Flow 的 Entity 文本仍是必填且语义非空；Relation label 可省略，Core ADR-01 落地后出现时同样必须语义非空。字符串、每个 styled line 的文本，以及 mixed line 的文本 / TeX run 全部为空白时必须被 Source schema 拒绝。空数组继续不合法；非空数组不能以空行或空白 run 规避该约束。该判断不改变 Core TextBlock 在其它 owner 中允许的表达范围
