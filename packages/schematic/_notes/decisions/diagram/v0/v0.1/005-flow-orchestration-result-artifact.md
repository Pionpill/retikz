...
ADR-004 的 id、顺序、尺寸、scope containment、Layout 固定 placement、margin collision、route、label coverage 与 capability 不变量全部在 Graph materialization 前验证。Retikz 把 provider output 深度脱离并递归冻结，拒绝非 plain-data / JSON-safe 值、Promise、非有限数值、缺失 / 多余 / 重复 id 与任何违反 Definition output contract 的记录
...
FlowDiagram 的 Definition provider 拥有 package-internal Diagram Foundation assembly，并显式依赖 Graph Entity / Group / Relation、Flex / Grid Layout 以及 Foundation 所需的 Layout / Standard providers；不存在单独的 Foundation provider 或临时 public composite。Flow 使用一个公开的 provider contribution assembly，把 `DiagramDefinitionOptions` 与同一次调用的 `GraphDefinitionOptions` 组合为 `FlowDiagramDefinitionOptions`；Flow resolver 与最终 Graph lowering 因此读取同一组 role、kind、Graph Theme、Diagram Theme、Flow Theme 和 layout Definition。Graph provider 的完整闭包可以继续包含 Graph Block，这不扩大 Flow Source 或 authoring surface
...
- provider capability不足、callback失败与output非法继续使用ADR-004的错误码。output错误必须包含layout definition、provider output path与相关authored id；不能把provider path伪装成Source path
