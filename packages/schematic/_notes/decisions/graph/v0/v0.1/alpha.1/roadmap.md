...
> 状态：进行中。ADR-01～09 均已 Accepted；当前实现按 ADR-06 无 Variant Theme breaking revision 与 ADR-09 独立 composite 契约同步收敛。关联：[Graph v0.1 roadmap](../roadmap.md) · [Schematic Graph 完备设计](../../../../../architecture/schematic-graph-complete.md) · [Schematic 制图能力域设计](../../../../../../../../notes/architecture/schematic-design.md)

## 目标

建立可发布的 Graph package family，以 Graph、Entity 与 Relation 三类 JSON-safe Source composite 表达通用图式语义。Entity 与 Relation 在各自 record 中组合 Graph 语义和 Core lower-facing 实例字段，可以独立放入任意 Core 内容树；Graph 组合完整 Core Scope surface，并提供可选 `graphTheme`。Relation 引用复用 Core NodeTarget 与 namespace；Graph 不拥有成员集合、私有引用索引、geometry 数据模型、布局调度、Variant 视觉轴或按 identity 分离的 appearance 投影
...
- 直接 IR、React 与 Vanilla 共享同一 Graph / Entity / Relation Source IR、Definition、resolve 与 lowering；JSX / builder 只提供 authoring sugar
