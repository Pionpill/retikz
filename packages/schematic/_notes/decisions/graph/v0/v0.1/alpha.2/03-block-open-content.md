...
Graph 需要一种可以作为关系节点使用、同时允许作者自由组织内部视觉内容的结构容器。代码、工程和节点图中的对象通常共享“外层边界内按稳定方向排列内容”的基本形态，但 Header、Section、Row、Cell 只是一种常见写法，并不是所有 Block 都必须保存的领域事实。若把这一写法固化为 Block Source grammar，用户必须先把任意内容改写成预设层级，官方类图等更高层封装也会与基础容器争夺同一语义真源
...
Block 接受零个或多个有序 `IRChild`。每个 child 可以是 Core Node、Path、Coordinate、Scope，现有 Graph composite、Layout、Standard composite，或其它已注册 Tier 2 composite。Block 不根据 child 类型、字段名称或声明位置推断 Header、Section、Row、Cell、port 或领域角色，也不收集成员数据库
...
Header、Section、Row 与 Cell 的 Direct IR、React、Vanilla 入口同样产生各自唯一 Source；adapter 不把这些结构重新嵌回 `IRBlock`，也不建立只有某一入口可表达的私有 child 形态
...
Header、Section、Row 与 Cell 的最小关系为：
...
type IRBlockRow = IRScopeProps &
  ...
  Readonly<{
    namespace: 'graph';
    type: 'blockRow';
    children?: ReadonlyArray<IRBlockCell>;
    gap?: number;
  }>;
```
...
- Block 不自动创建 Relation、port、Header、Section、Row、Cell 或 Tier 3 结构，也不根据空内容切换另一种 lower 根形状
- 旧 `header`、`sections` 与嵌套 rows / cells 不再属于 `IRBlock`；Header、Section、Row 改为独立 Source composite，Cell 改为 Row-local item。alpha.2 直接删除旧嵌套契约，不提供 alias、migration、fallback 或新旧双轨
