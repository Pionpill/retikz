...
FunctionBlock 是 namespace `graph` 下的独立 composite，使用 `type: 'functionBlock'`，拥有独立 schema、Definition 和 provider，复用 ADR-01 的共享数据、主题配置、完整 Block lower-facing surface 与三入口规则。
...
内部呈现由实体固定组合，使用 ADR-01 的 codeBlockTokens 统一控制视觉角色，不接受实例 presentation。签名使用 codeFontFamily，说明和类型注释使用 mutedTextColor，逻辑正文使用 textColor；不提供逐成员或逐逻辑段样式覆盖。
