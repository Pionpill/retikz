...
> 状态：进行中；v0.1 alpha.1 已完成，ADR-01～10 均为 Accepted 或 Superseded，Graph / Group / Entity / Relation 四类 Source composite 已形成闭环；alpha.2 开放 Block 进行中；alpha.3 规划代码实体 Tier 3。关联：[Schematic Graph 完备设计](../../../architecture/schematic-graph-complete.md) · [Schematic 制图能力域设计](../../../../../../notes/architecture/schematic-design.md)

## 目标

Graph v0 当前范围是 Schematic 领域的可复用关系与图式 foundation：Entity 与 Relation 表达普通节点和关系，Group 表达任意内容的可见包含，Block 表达开放内容、纵向布局与显式局部寻址的图节点；Graph 是可选的 `graphDefaults` / `graphRules` 与局部 Scope 上下文。Relation endpoint 复用 Core NodeTarget、namespace 与 anchor，Graph 不建立成员集合、私有引用索引、平行 Port / geometry / appearance model 或 Variant 视觉轴，也不拥有 Diagram 自动布局或 Editor document。
...
- `Block` 纳入 v0.1 alpha.2：面向代码与工程结构图，以开放 children 与独立可选 Header / Section / Row 表达可参与关系拓扑的节点；Blender、Gaea 一类节点图只用于验证局部连接覆盖面
- 代码实体 Tier 3 纳入 [v0.1 alpha.3](./v0.1/alpha.3/roadmap.md)：先拆分共享内容，再组合 InterfaceBlock、FunctionBlock、ObjectBlock、ClassBlock、ModuleBlock，在同一 alpha 完成；独立实体语义下沉到 Block
