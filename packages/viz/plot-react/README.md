...
- **spec wrapper** — `<Plot spec={irPlotSpec} data={datasets} />` for a hand-written / generated `IRPlotSpec`.
- **composition DSL** — `<Plot data={rows}><PathMark x y /><PointMark x y /></Plot>`; mark children are
  assembled into a normalized `IRPlotSpec` (linear scales + cartesian2D inferred) by Plot Vanilla.
