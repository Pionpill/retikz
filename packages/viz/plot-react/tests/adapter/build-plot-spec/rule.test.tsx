...
describe('buildPlotIR rule 装配（alpha.11 ADR-03）', () => {
  ...
  it('rulemark-orientation-conflict：x 与 y 皆给 → fail-loud', () => {
    expect(() => buildPlotIR(<ReferenceMark x={1} y={2} />, '__plot')).toThrow(/exactly one of x|both|neither/i);
  });

  it('rulemark-orientation-missing：x 与 y 皆缺 → fail-loud', () => {
    expect(() => buildPlotIR(<ReferenceMark />, '__plot')).toThrow(/exactly one of x|both|neither/i);
  });
  ...
  it('rulemark-extent-unpaired：仅 extentField → fail-loud', () => {
    expect(() => buildPlotIR(<ReferenceMark x="date" extentField="lo" />, '__plot')).toThrow(
