...
import type { IRChartInspection } from './schema';

import { ChartInspectionMemberKind } from '../../shared';
import { ChartInspectionSchema } from './schema';
...
export const createChartInspection = (
  ...
  return ChartInspectionSchema.parse({
    chart: { type: chart.type, ...(chart.id === undefined ? {} : { id: chart.id }) },
