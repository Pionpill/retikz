...
/** Recipe 在最终 Plot scaffold 已合并后补齐 guide 默认值的窄上下文 */
export type ChartGuideDefaultsResolveContext = Readonly<{
  /** 当前 exact schema 已 parse 的 Chart Source */
  source: IRChartSource;
  /** 当前 recipe 的 encoding 解析结果 */
  encodings: ChartEncodingResolution;
  /** authored override 合并后的最终 Chart semantic Plot marks */
  chartMarks: ReadonlyArray<IRPlotMarkOperation>;
  /** scale 默认值补齐后的最终 scale operations */
  scales: ReadonlyArray<IRPlotScaleOperation>;
  /** 最终生效的 coordinate 或 composition */
  spatial: Pick<IRPlot, 'coordinate' | 'composition'>;
  /** recipe 与 Plot extension 合并后的 guides */
  guides: ReadonlyArray<IRPlotGuide>;
  /** 与 Plot lowering 共享的 Definition 注册表 */
  runtime: ChartEncodingRuntime;
}>;
...
export type ChartRecipeDefinition<TSource extends IRChartSource = IRChartSource> = Readonly<{
  ...
  /** 按最终 scales 与 spatial 补齐 recipe guide 默认值；显式 guides 必须保持不变 */
  resolveGuideDefaults?: (context: ChartGuideDefaultsResolveContext) => ReadonlyArray<IRPlotGuide>;
}>;
