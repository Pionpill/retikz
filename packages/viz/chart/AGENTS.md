...
- **不拥有的能力**：公开统一 `ChartSchema` / `IRChart` union、family 级 schema 或解析器、Plot token / preset / resolver、Data 算法、Plot lowering / registry、Layout solver、Standard Surface lowering、Core compile、renderer、框架 authoring
...
- 根入口公开 Base Chart、Chart presentation/style 及 Base `bindChart` / `resolveChart` 能力；`/point` re-export 根入口并增加三个精确 schema、IR 与 recipe
