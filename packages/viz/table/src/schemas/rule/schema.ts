...
const uniqueArray = <T extends ZodType>(item: T, message: string) =>
  array(item)
    .min(1)
...
export const TableValueBetweenPredicateSchema = strictObject({
  ...
  includeMin: boolean().optional().describe('Whether the lower bound is inclusive. Omitted fields use true.'),
  includeMax: boolean().optional().describe('Whether the upper bound is inclusive. Omitted fields use true.'),
})
...
const selectorShape = {
  ...
  roles: strictObject({
    ...
    .optional(),
  sourceKinds: uniqueArray(zodEnum(TableCellSourceKind), 'duplicate Cell source kind')
    .optional()
  ...
  payloadKinds: uniqueArray(zodEnum(TableCellPayloadKind), 'duplicate payload kind')
    .optional()
  ...
  negate: boolean().optional().describe('Whether to negate the combined conditions inside their evaluation domain.'),
};
