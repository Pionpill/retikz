...
export const extractTableStructureKind = (definition: AnyTableStructureDefinition): string => {
  if (!(definition.schema instanceof z.ZodObject)) {
    throw new Error('table: structure definition schema must be a ZodObject with a literal kind');
  }
  ...
  if (!(kindSchema instanceof z.ZodLiteral) || typeof kindSchema.value !== 'string' || kindSchema.value.length === 0) {
    throw new Error('table: structure definition schema.kind must be a non-empty string literal');
  }
...
export const resolveTableStructureRegistry = (
  ...
  for (const definition of BUILTIN_TABLE_STRUCTURES) {
    ...
    if (registry.has(kind)) throw new Error(`table: duplicate built-in structure registration: "${kind}"`);
  ...
  for (const definition of custom ?? []) {
    ...
    if (BUILTIN_STRUCTURE_KINDS.has(kind)) {
      throw new Error(`table: structure kind "${kind}" conflicts with a built-in definition`);
    }
    if ((RESERVED_TABLE_STRUCTURE_KINDS as ReadonlyArray<string>).includes(kind)) {
      throw new Error(`table: structure kind "${kind}" is reserved for v0.1`);
    }
    if (registry.has(kind)) throw new Error(`table: duplicate structure registration: "${kind}"`);
    registry.set(kind, definition);
  }
  return registry;
};
...
export const tableStructureDefinitionOf = (
  ...
  throw new Error(`table: structure "${kind}" is not registered; pass it via options.structureDefinitions`);
};
