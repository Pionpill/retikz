...
/** Cell 最小来源信息 schema */
export const TableCellSourceSchema = union([
  strictObject({
    kind: literal(TableCellSourceKind.Manual).describe(
      'Discriminator for a source Cell authored by manual structure input.',
    ),
  ...
  strictObject({
    kind: literal(TableCellSourceKind.Field).describe('Discriminator for a Cell derived from an external data field.'),
  ...
  strictObject({
    kind: literal(TableCellSourceKind.Generated).describe(
      'Discriminator for content synthesized by a structure definition.',
    ),
...
export const TableStructureOutputSchema = strictObject({
  rows: array(
    strictObject({
      ...
      sourceIndex: NonNegativeIntegerSchema.optional().describe('Optional external source row index for a body row.'),
    }),
  ).describe('Ordered canonical row candidates.'),
  columns: array(
    strictObject({
      id: NonBlankStringSchema.optional().describe('Optional stable column id supplied by the structure definition.'),
  ...
  cells: array(
    strictObject({
      ...
      roles: array(TableCellRoleSchema).min(1).describe('Non-empty semantic Cell roles.'),
...
/** Cell 最小来源信息 */
export type TableCellSource = DeepReadonly<ZodInfer<typeof TableCellSourceSchema>>;

/** Structure Definition 的声明式 runtime output */
export type TableStructureOutput = DeepReadonly<ZodOutput<typeof TableStructureOutputSchema>>;
