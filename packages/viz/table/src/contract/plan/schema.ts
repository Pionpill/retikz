...
const TableThemeTokenKeySchema = zodEnum([
...
export const TableCellPlanSourceSchema = discriminatedUnion('kind', [
  ...
  strictObject({
    kind: literal(TableCellPlanSourceKind.StyleToken).describe('Discriminator for a resolved Table token winner.'),
    tokenKey: TableThemeTokenKeySchema.describe('Appearance Table token that supplied the winning leaf.'),
    tokenSource: zodEnum(ThemeTokenSource).describe('Winning token source relation to the Table owner.'),
