...
`0.1.0-alpha.3` 当前在 alpha.2 二维约束布局基线上实现 formatter / presentation、selector / rule、条件视觉 encoding、四种 style preset、闭合 style tokens 与同源 Legend descriptor / manifest seed。Standard Legend、外围 Box Layout composition、最终 artifact join 及完整 adapter / SSR / docs 闭环仍受 ADR-06 hard gate 阻塞；当前不得在 Table 内建立临时实现。
