...
- **输入与输出**：接收完整 Plot authoring 或某个精确 typed Chart props 与 React children，绑定后输出 `IRBaseChart` 和宿主 runtime contribution
...
- 不导出 `InputChart`、Point union 或接受通用 `type` prop；每个组件只构造并绑定自己的精确 schema
