...
- Point Chart 的 `encodings` 直接接受具体 chartType 的 exact mapping；`properties` 只接受常量；直接 `<ScatterMark>` 只表达 Chart-owned `recipe.marks` payload，普通 mark 按 authored order 追加，`override` 原样写入 Source 供 Chart resolver 匹配内建 semantic group；Path 等独立 Plot mark 通过 `plotExtension.marks` 传入
