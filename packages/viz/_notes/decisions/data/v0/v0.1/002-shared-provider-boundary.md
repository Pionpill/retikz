...
ADR-001 将 plot 的数据 schema、字段解析、format、statistics、transform registry 与 apply pipeline 迁入 `@retikz/data`。迁移后的 data 仍必须是 plot、table、geo 等宿主共用的数据层，而不是 plot 的数据实现包。输出字段、默认命名或语义直接服务 mark、scale、coordinate、relation routing 或 stat geometry 的能力，应由宿主注册自己的 transform definition。
