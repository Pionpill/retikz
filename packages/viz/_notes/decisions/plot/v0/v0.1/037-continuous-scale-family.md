...
- 状态：Accepted
- 决策日期：2026-06-08
- 关联：[plot v0.1-alpha.7 roadmap](./roadmap.md) · [plot v0 roadmap 阶段二](../roadmap.md) · [plot-design §3.4 Scale / §3.9 Guide](../../../../architecture/plot-design.md) · 下游：[ADR-038 通道→scale 抽象 + size](./038-channel-scale-resolver-size.md)（size 派生到本 ADR 的 sqrt）/ [ADR-039 color·series](./039-color-series.md) · 前身：[ADR-010 d3-scale](./010-d3-scale.md)

## 背景

1. **跨数量级**：人口、收入、计数、地震能量等跨多个 10 倍区间的数据，线性轴把小值压成一团，需 **log** 轴。
2. **感知/几何编码**：面积正确的尺寸编码（散点 bubble）要 **sqrt**（半径 ∝ √值，面积 ∝ 值）；更一般的幂次需 **pow**。alpha.7 的 `size` 通道（[ADR-038](./038-channel-scale-resolver-size.md)）**直接依赖 sqrt**。
