...
ADR-069 解决了“已有 relation rows 如何画成 source-target Path”的问题，但很多关系并不预先存在为 edge table，而是可从图表数据中计算出来，例如最低点到最高点、两个 interval 的差值、某组代表点之间的箭头。
...
本 ADR 的 mark-local transform 入口后来被 [ADR-071](./071-mark-local-transform.md) 提升为所有 mark 的公共字段。`derive-relation` 的私有统计语法后来被 [ADR-072](./072-statistical-transform-algebra.md) 收敛为通用 `relate` transform 与共享 selector / pair measure 代数。
