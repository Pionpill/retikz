...
ADR-090 已经把 axis 拆成 line、ticks、tickLabels、title、grid 五个部件槽位，并让 `ticks` 同时承担 tick source 与 tick mark 的局部样式。当前 `ticks.count` 只是目标数量提示：未提供显式 `ticks.values` 时，plot 把它交给对应 scale 的 `ticks(count)`，最终数量由 scale family 和 nice 区间决定；提供 `ticks.values` 时则完整使用用户给定值。
