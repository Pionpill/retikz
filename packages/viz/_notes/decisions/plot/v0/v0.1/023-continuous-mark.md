...
- 状态：Superseded
- 替代：[ADR-059](./059-mark-abstraction-registry.md) 与 [ADR-060](./060-mark-surface-convergence.md)；line / area 已统一为 PathMark 与 closure
- 决策日期：2026-06-06
- 关联：[plot v0.1-alpha.4 roadmap](./roadmap.md) · [ADR-021](./021-coordinate-polar.md) · [plot-design.md §3.7 mark / §8.3 投影分层](../../../../architecture/plot-design.md)

## 背景

ADR-022 的 sector 家族下沉成参数化 Node。但**连续 mark（line / area）跨多数据点、无参数化形态**（§8.3）：它们由 coordinate 逐点投影成 `Path`，连接靠 datum 锚点。alpha.1~alpha.3 的 line 已是「投影点 → move/line steps」，但写死直边、且只有 cartesian。
...
- polar guide → ADR-024；sector 家族 → ADR-022。
