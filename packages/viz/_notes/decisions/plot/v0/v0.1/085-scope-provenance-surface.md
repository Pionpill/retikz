...
- 状态：Superseded
- 替代：[ADR-088](./088-composition-api-structure.md)；scope identity 已重命名为 coordinate view，locator / provenance 使用最终 view 与 arrangement 上下文
- 决策日期：2026-06-28
- 关联：[plot v0.1 roadmap](./roadmap.md) · [alpha.14 roadmap](./roadmap.md) · [ADR-080 coordinate composition registry](./080-coordinate-composition-registry.md) · [ADR-081 facet grid data routing](./081-facet-grid-data-routing.md) · [ADR-082 same-panel multi-axis overlay](./082-same-panel-multi-axis.md) · [ADR-083 shared scaffold tracks](./083-shared-scaffold-tracks.md) · [ADR-084 composition guides layout](./084-composition-guides-layout.md)

## 背景

ADR-080～084 让 Plot 内部出现多个 coordinate scope、facet panel、overlay axis 和 scaffold track。只让 renderer 画出来还不够：locator / provenance 必须能回答“这个 datum 属于哪个 panel、哪个 coordinate scope、哪个 track”；React / Vanilla authoring surface 也必须能表达同一份 IRPlot，而不是只让手写 JSON 可用。
