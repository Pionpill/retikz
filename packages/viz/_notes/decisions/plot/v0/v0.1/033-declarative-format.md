...
- 状态：Superseded
- 替代：[Data ADR-001](../../../data/v0/v0.1/001-plot-data-migration.md)；声明式字段格式与 format registry 已迁入 `@retikz/data`
- 决策日期：2026-06-07
- 关联：[plot v0.1-alpha.6 roadmap](./roadmap.md) · 本里程碑 [ADR-029 可移植契约](./029-data-portability.md)（coercion）/ [ADR-031 resolveField](./031-field-resolver.md)（函数逃生舱）/ [ADR-032 type 可选](./032-optional-field-type.md) · [plot-design.md §3.1 数据模型](../../../../architecture/plot-design.md)

## 背景

ADR-029 的内置 coercion 只认固定形态：`temporal` 收 `Date` / epoch ms / 严格 ISO，`continuous` 收 number / 严格数字串。碰上 `'2024/01/01'`、epoch **秒**、`'50%'` 这类常见非默认格式就 NaN→跳过。ADR-031 的 `resolveField` 给了自定义解析，但它是**运行时函数、不进 IR**——一旦用上，spec 就**丢了可序列化 / 可移植 / LLM 可生成**这条 retikz 核心价值（core-design §7）。
...
- **lowering**：`coerce.ts` 加 `formatParser(type, format) → parser`；`normalizeRows` 的 per-field parser 槽接受「format 选出的内置 parser」（与 ADR-031 的 resolveField.parse 同槽，优先级 resolveField > format）；`expand.ts` / `validate.ts` 收集 format→parser 并做 type 兼容校验。
...
- **resolveField 函数逃生舱**——已是 ADR-031，本 ADR 只把它降为「词表外」兜底。
