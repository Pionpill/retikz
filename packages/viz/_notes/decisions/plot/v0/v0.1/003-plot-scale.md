...
- 状态：Accepted
- 决策日期：2026-06-03
- 关联：[plot v0.1-alpha.1 待办](./roadmap.md) · [plot v0.1 roadmap](./roadmap.md) · [plot-design.md §3.5 / §11.1](../../../../architecture/plot-design.md) · 根节点：[ADR-001 IRPlot](./001-plot-spec-root.md) · 引用方：[ADR-004 coordinate](./004-plot-coordinate.md)

## 背景 / 约束

根节点（ADR-001）的 `scales` 槽位是命名比例尺数组。scale 定义「数据值 → 绘图坐标值」的映射，是图形语法最核心一环（业务量纲经 scale 映射到绘图区量纲）。本 ADR 只管「scale 自身长什么样」与「如何按名引用」，不管「谁用它」——位置通道用哪个 scale 由 coordinate 决定（ADR-004）。alpha.1 最薄切片只需 `linear`（连续线性）。

## 决策：命名 `linear` scale，domain / range 可省（lowering 推断）

每个 scale 有唯一 `name`（被 coordinate 按名引用）。alpha.1 仅 `type: 'linear'`：`domain`（输入区间 `[min,max]`）/ `range`（输出区间 `[start,end]`）均可省略，省略时留给 ADR-006 lowering 推断——domain 从绑定的外部数据集字段推（数据不进 IR、绑定期才到位，ADR-002），range 从 coordinate extent 推。`nice` / `clamp` 为可选布尔，语义默认 false。
...
4. **scale 不持有「驱动哪个通道」**：那是 coordinate（ADR-004）的职责，scale 保持纯映射定义、可被任意通道复用。
...
- scale `name` 跨元素唯一性 schema 层**不校验**（数组层无法在单元素 schema 表达），重名检测归 ADR-006 lowering。
...
- **domain / range 推断、scale 求值、重名检测** → ADR-006 lowering。
