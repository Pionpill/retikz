...
- 状态：Superseded
- 替代：[Data ADR-001](../../../data/v0/v0.1/001-plot-data-migration.md)；字段解析与运行时 resolver 现由 `@retikz/data` 拥有
- 决策日期：2026-06-07
...
- **优先级 `resolveField.type > model > infer`**，但 **model strict 不破**：`resolveField` 能盖已声明字段的类型，**不能**让未在 model 声明的字段通过 strict 校验——catch-all resolver 不得把拼错字段洗成合法字段（守本里程碑 ADR-028 data-model 的 strict fail-loud）。strict 的「字段必须声明」检查在 resolver 之前、独立成立。
