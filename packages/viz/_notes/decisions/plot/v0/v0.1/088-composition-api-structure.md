...
- ADR-080～087 的概念决策已实现，但字段命名与公开结构以本 ADR 为准；后续不得继续扩展旧结构。
...
4. 精确 grid 目标仍属于 `AxisGuide.grid`。`grid: true` 使用 `resolve.grid`；`grid: { applyTo: 'selected', select }` 可以按 `view`、`facet` 或 `track` 精确投放。ADR-086 的 selector 能力不得在本次重构中丢失，只把旧 selector 名称迁移到 `view` / `arrangement`。
