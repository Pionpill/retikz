...
- 状态：Superseded
- 替代：[ADR-068](./068-channel-core-coverage.md)；opacity 已从 PointMark 专属能力扩展为按宿主落到 Node、Path 或 Scope 的共享 mark channel
- 决策日期：2026-06-08
- 关联：[plot v0.1-alpha.7 roadmap](./roadmap.md) · [plot-design §3.3 Aesthetics](../../../../architecture/plot-design.md) · 依赖：[ADR-038 通道→scale resolver](./038-channel-scale-resolver-size.md)（opacity 复用其 resolver + PointEncoding）

## 背景

[alpha.6 ADR](./roadmap.md) 拆 `StyleEncodingSchema` 时注释「opacity / size / shape later」。alpha.7 已补 `size`（[ADR-038](./038-channel-scale-resolver-size.md)，PointMark 专属 encoding）+ 通用「通道→scale」resolver。本轮范围调整（把原 alpha.8 的 opacity / shape 前移，见 roadmap），`opacity` 作为 resolver 的又一消费者落地。
...
// PointEncodingSchema 扩成 { ...Encoding, size?, opacity? }（shape 见 ADR-041）
```

- **field（continuous）**：经 ADR-038 resolver 合成 `linear` scale，domain = 数据 extent，range `[OPACITY_MIN, 1]`（默认下界，避免最小值全透明不可见）。continuous only——categorical opacity 无意义、顺延；temporal fail-loud。
...
1. **复用 resolver**：opacity 是 ADR-038 resolver 的直接消费者（continuous → linear），边际成本低。
