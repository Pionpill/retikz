...
- 状态：Superseded
- 替代：[ADR-088](./088-composition-api-structure.md)；same-panel overlay 保留，绑定字段统一为 `coordinateView`
- 决策日期：2026-06-28
- 关联：[plot v0.1 roadmap](./roadmap.md) · [alpha.14 roadmap](./roadmap.md) · [ADR-080 coordinate composition registry](./080-coordinate-composition-registry.md) · [ADR-081 facet grid data routing](./081-facet-grid-data-routing.md)
...
ADR-080 已让 mark 和 axis guide 能通过 `coordinateScope` 绑定不同 scope。ADR-082 需要补齐 overlay scope 的布局语义、axis placement / offset、重复轴判定和 z-order 规则，使“多 scope 在同一 panel 内叠加”成为稳定能力。
...
6. grid 默认只来自 target scope 或显式 `grid: true` 的 guide；多个 overlay grid 的合并 / 避让策略由 ADR-084 统一。
...
4. 使用 ADR-080 的 `coordinateScope`，mark / guide / locator 都能共享同一个 scope identity。
5. axis placement 不是裸四方向字段，避免把 cartesian 的 left/right/top/bottom 泄漏到 ternary / polar / custom coordinate。

## 长期边界

- 不做 axis title、label 避让、side gutter / edge gutter 自动分配；ADR-084 处理。
