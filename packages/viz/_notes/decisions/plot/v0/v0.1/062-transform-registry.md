...
状态：Accepted
决策日期：2026-06-18
关联：[plot v0.1-alpha.12 roadmap](./roadmap.md) · [ADR-057 bin / aggregate](./057-bin-aggregate.md) · [ADR-058 derive / normalize / jitter](./058-derive-normalize-jitter.md) · [ADR-061 coordinate registry](./061-coordinate-registry.md) · [ADR-063 scale registry](./063-scale-registry.md)
...
transform 是 grammar-of-graphics 的 Statistics 层，长尾需求明确，因此与 ADR-059 mark registry 不同，本 ADR 在内部 registry 收敛的同时开放公开扩展点。
...
- 后续 ADR-071/072 的 mark-local transform 与统计代数复用本 registry。
...
最终 owner 边界由 [ADR-102](./102-plot-transform-registration.md) 固定：通用 transform 属于 Data，`stack` / `bin` / `normalize` / `derive-interval` / `relate` / `jitter` / `density` / `smooth` 属于 Plot。
