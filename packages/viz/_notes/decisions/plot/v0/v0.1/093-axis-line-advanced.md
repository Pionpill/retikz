...
ADR-090 已经把 axis 拆成 line、ticks、tickLabels、title、grid 五个部件槽位，并让 `line` 支持 stroke、strokeWidth、drawOpacity、dashPattern、dashOffset 等线条样式。这个模型能覆盖常规商业图表，但数学图、函数图和平面直角坐标系还缺少两个高频表达：轴线端点箭头，以及 x / y 轴穿过原点或指定交叉值。
...
core 已经具备稳定 `dashOffset` / stroke dash offset 能力。plot 不需要在 ADR-093 里为 axis line 单独设计 `dashOffset`，而应按 ADR-090 把它作为普通 `GuideLineStyleSchema` 字段接入 axis line、tick line、grid line、legend 可描边部件与 theme line style。
...
- axis line 专用 dash offset 结构字段。`dashOffset` 已由 core 提供，plot 应通过 ADR-090 的 `GuideLineStyleSchema` 作为普通线条样式复用，不在 ADR-093 另造字段。
