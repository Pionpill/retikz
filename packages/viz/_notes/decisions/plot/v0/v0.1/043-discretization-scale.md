...
- 状态：Accepted
- 决策日期：2026-06-08
- 关联：[plot v0.1-alpha.8 roadmap](./roadmap.md) · [plot v0 roadmap 阶段二](../roadmap.md) · [plot-design §3.4 Scale](../../../../architecture/plot-design.md) · 依赖：[ADR-042 连续色阶](./042-continuous-color-scale.md)（复用 scheme/range schema）· 下游：[ADR-044 legend](./044-legend-guide.md)（分箱 swatch）

## 背景

[ADR-042](./042-continuous-color-scale.md) 把连续量编码成**连续色带**（平滑渐变）。但很多场景要**分箱离散色**——把连续量切成有限档、每档一色：
...
**配色采样**：scheme → 在 `[0,1]` 等距取 `count`（或 `breakpoints.length+1`）个点喂 interpolator，得离散色数组；`range` 给定则直接用、覆盖 scheme。采样工具与 ADR-042 共用一处（`lower/scale.ts`），不重复。
...
- **数值字段 `.finite()`**：`breakpoints` / quantize `domain`（及 ADR-042 sequential/diverging `domain`）的数值改 `z.number().finite()`，parse 期拒 `Infinity`/`-Infinity`（修 adversarial BLOCKING：裸 `z.number()` 放过 Infinity，既过 schema 又破坏 JSON round-trip）。
...
- **legend 分箱 swatch 渲染** → [ADR-044](./044-legend-guide.md)。
