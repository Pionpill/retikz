...
连续 / 时间位置 scale 提供 `domainPadding` 与 `singleValueSpan`。无论 domain 来自数据推断还是显式声明，省略 `domainPadding` 时默认值均为 `0`；调用方只有显式声明 `domainPadding` 才扩展 domain。tick 控制进入 ADR-090 的 axis 部件槽位：`ticks.count` 作为 hint，`ticks.values` 声明显式刻度，`tickLabels.format` 控制展示文本。
...
- Theme token、axis line / label / grid 外观样式；由 ADR-090 / ADR-091 处理。
