...
在 ADR-057 处理“改行数”统计 transform 后，alpha.12 还需要一组保行数的逐行派生 / 调整 transform：百分比归一化、从字段派生区间、以及确定性 jitter。它们不会改变行数，但会给后续 scale、mark、locator 提供新的字段或坐标值。
...
React 侧复用 ADR-057 引入的统一 transform 表面：
...
`derive-interval` 与 ADR-057 的连续 x 区间能力共同支撑 histogram；`normalize` 与既有 `stack` 组合支撑百分比图；`jitter` 为后续更多 mark 和 interaction parity 留下确定性约束。
