...
- 状态：Superseded
- 替代：[ADR-104](./104-plot-vanilla-plain-api.md)；React / Vanilla authoring 最终共用 framework-neutral normalization
- 决策日期：2026-06-05
- 关联：[plot v0.1-alpha.3 待办](./roadmap.md) · [plot-design.md §5.2 Primitive API / §6 包结构](../../../../architecture/plot-design.md) · 回溯：[ADR-007 bindings](./007-plot-bindings.md) · [ADR-008 react DSL](./008-plot-react-dsl.md) · [ADR-013 guide DSL](./013-guide-bindings-dsl.md) · 依赖：[ADR-014](./014-band-scale.md)~[ADR-019](./019-time-scale.md)

## 背景

塑造决策的硬约束：

- ADR-014~019 已把 band/point/ordinal/time scale、interval mark、transform、color 通道、relation 做进 `@retikz/plot`（IR + lowering），但用户表面（`@retikz/plot-react` 的 `<Plot>{marks}</Plot>` 组合 DSL、`@retikz/plot-vanilla` builder）一个都没露出。
...
3. **`<BarMark stack>` 一处声明、装配 transform+arrangement+字段对齐**：避免用户手动保证 transform 的 `startField` 与 mark 的 `y0Field` 同名（[ADR-018](./018-relation.md) 待决策点的对齐风险由 builder 兜）。
...
- **legend 组件 / 图例** → 后续（[ADR-017](./017-color-scale.md) 已划走）。
- **横向柱 / 百分比堆叠 props** → 后续。
