...
ADR-066 把内置 visual channel 收敛为 definition / registry，但公开自定义通道仍缺最后一环。若自定义 channel 只能走单独补丁路径，内置与扩展能力仍然不是同一机制。

本 ADR 在 ADR-066 的基础上开放自定义 visual channel：用户通过 runtime definition 注入通道逻辑，IR 只描述字段绑定和 JSON-safe 配置。
