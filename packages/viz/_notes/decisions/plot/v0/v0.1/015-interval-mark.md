...
- 状态：Superseded
- 替代：[ADR-059](./059-mark-abstraction-registry.md)；IntervalMark 最终使用 coordinate-independent bounds × coordinate 投影契约
- 决策日期：2026-06-05
- 关联：[plot v0.1-alpha.3 待办](./roadmap.md) · [plot-design.md §3.7 mark / §4.5 mark 构造](../../../../architecture/plot-design.md) · 回溯：[ADR-005 encoding/mark](./005-plot-encoding-mark.md) · [ADR-006 lowering](./006-plot-lowering.md) · 依赖：[ADR-014 band scale](./014-band-scale.md) · 消费方：[ADR-018 relation](./018-relation.md)

## 背景

塑造决策的硬约束：

- alpha.1 已有 `point` / `line` 两种 mark，柱状图需要第三种 **interval（bar）**——从一条 **baseline**（通常值 0）到数据值的矩形区间（plot-design §3.7）。在 cartesian2D 竖直柱里：x 是分类（[ADR-014](./014-band-scale.md) band scale，柱宽 = `bandwidth`），y 从 baseline 到 value。
- 本 ADR 只做**单系列竖直柱**：一类别一根柱、统一颜色、baseline=0。分组 / 堆叠 / 横向柱 / 自定义 baseline / start-end 区间（甘特）留 [ADR-018](./018-relation.md) 与后续。
...
2. **复用 band scale 的 `bandwidth` / `coordinate`**（[ADR-014](./014-band-scale.md)）：柱宽 / 柱位单一真源就是 x band scale，与该轴刻度严格对齐，不另算。
...
- **柱宽是否减 padding**：柱宽直接用 `bandwidth`（band scale 的 paddingInner 已在 [ADR-014](./014-band-scale.md) 留出柱间缝）；bar 层不再二次缩。

## 长期边界

- **分组（dodge）/ 堆叠（stack）柱、多系列** → [ADR-018](./018-relation.md)。
- **颜色编码（按字段着色）** → [ADR-017](./017-color-scale.md)（本 ADR 柱统一 currentColor）。
