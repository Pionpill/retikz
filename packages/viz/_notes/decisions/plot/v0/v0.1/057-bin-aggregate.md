...
状态：Superseded
替代：[ADR-072](./072-statistical-transform-algebra.md) 与 [ADR-102](./102-plot-transform-registration.md)；`aggregate` 已由 Data 的 `summarize` 替代，`bin` 保留为 Plot 自行注册的 plot-only transform
