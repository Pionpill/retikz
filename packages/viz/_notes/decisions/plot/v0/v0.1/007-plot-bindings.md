...
- 状态：Superseded
- 替代：[ADR-104](./104-plot-vanilla-plain-api.md)；三包仍保持 owner 分层，但最终 authoring 与 Tier2 adapter 公共面已重构
- 决策日期：2026-06-03
- 关联：[plot v0.1-alpha.1 待办](./roadmap.md) · [plot v0.1 roadmap 拆分策略](./roadmap.md) · [plot-design.md §6 / §13.1](../../../../architecture/plot-design.md) · 依赖：[ADR-006 lowerPlots](./006-plot-lowering.md) · 后续：[ADR-008 组合 DSL](./008-plot-react-dsl.md)

## 背景 / 约束

ADR-001~006 让 `@retikz/plot` 能「Plot IR + 数据 → core IR → 渲染」，但 authoring 表面只有手写 Plot IR 对象 + 手接 `<Layout ir={{...}} composites={lowerPlots(...)}/>` 那串——啰嗦、文档示例尤其遭殃。按 plot v0.1 roadmap「三包 lockstep」，框架绑定从 alpha.1 起就随 plot 同步出（原计划推到 v0.3，已废除）。本 ADR 落两包脚手架 + 最薄一层包装，把那串收成一个组件 / 一个函数；交互（tooltip / hover / 事件，依赖 core 水合）仍留 v0.3，本 ADR 只做静态 authoring + 渲染。
...
4. **lockstep 地基**：先把包建起来 + 最薄能渲染，ADR-008 的 DSL、后续 alpha 的新 mark/scale 都在这两包上长。

### 设计细节（具体决策）

- **`<Plot>` 的 data prop 收 `ExternalDatasets`**（`{ name: rows }`，对齐 `spec.data.ref` 与 lowerPlots）——最直接；裸 rows 便利留 ADR-008 的组合 DSL。
...
- **React 组合 DSL（`<LineMark>` / `<PointMark>` + builder + 自动推断 scale/coordinate）** → ADR-008。
