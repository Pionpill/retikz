...
- 状态：Superseded
- 替代：[alpha.12 ADR-13](../alpha.12/13-relation-mark-anchor.md) 与 [alpha.13 ADR-01](../alpha.13/01-relation-ribbon.md)；独立 ribbon mark 已收敛为 `RelationMark` 的 ribbon geometry
- 决策日期：2026-06-16
...
2. **半宽沿 orientation 垂向 normal**（horizontal→(0,1)、vertical→(1,0)），**不用「源→目标弦方向」的法向**——弦法向在对角流（源/目标几乎竖直对齐而水平跨度小）时让封口歪斜、半宽朝向错，会造成「流带丑」的病根。改用 orientation 垂向后封口始终沿垂向，干净的 sankey / alluvial 形态。
