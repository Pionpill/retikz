...
- 状态：Accepted
- 决策日期：2026-06-03
- 关联：[plot v0.1-alpha.1 待办](./roadmap.md) · [plot v0.1 roadmap](./roadmap.md) · [plot-design.md §3 / §8 / §8.1 / §11 / §13.1](../../../../architecture/plot-design.md) · 子结构：[ADR-002 data](./002-plot-data.md) · [ADR-003 scale](./003-plot-scale.md) · [ADR-004 coordinate](./004-plot-coordinate.md) · [ADR-005 encoding+mark](./005-plot-encoding-mark.md)
...
- `id` / `meta` 为预留位，让 alpha.5 的 anchor / scope-aware 与 ADR-006 的 provenance **非破坏接入**；`marks.min(1)` 锁定「至少一层」根级不变量。

## 长期边界

- **lowering / 数据绑定 `lowerPlots(datasets)`** → ADR-006。本 ADR 无运行时行为，只定根 schema 与 JSON 透传约束。
- **data 引用 / 数据模型字段、外部数据契约** → ADR-002；**scale / coordinate / encoding / mark 字段** → ADR-003 ~ ADR-005。
- **guide（axis / grid / legend）schema** → alpha.2。
- **anchor 命中 / datum locator / scope-aware 解析** → alpha.5（本 ADR 只留 `id` / `meta` 字段位、不解析）。
- **框架绑定 authoring（`<Plot>` JSX / data prop 拆分 / vanilla plot builder）** → ADR-007 / ADR-008。
