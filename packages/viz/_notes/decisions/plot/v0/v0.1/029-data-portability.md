...
- 状态：Superseded
- 替代：[Data ADR-001](../../../data/v0/v0.1/001-plot-data-migration.md)；数据归一化、字段映射与外部数据集契约已迁入 `@retikz/data`
- 决策日期：2026-06-07
- 关联：[plot v0.1-alpha.6 待办](./roadmap.md) · [plot v0 roadmap 阶段二](../roadmap.md) · [plot-design §3.1 Data / §8.2 数据绑定](../../../../architecture/plot-design.md) · 依赖：[ADR-028 数据模型类型层](./028-data-model.md) · 前身：[ADR-002 DataRef/DataModel](./002-plot-data.md)
...
三条的共同前提是**声明了 model**（都说「类型相同 / 同一 DataFieldType」=model 的声明）。故本 ADR 的核心判断：**model 是可移植性的开关**——声明 model，spec 即跨源可移植（恒等 / 映射 / 强制三档）；不声明，退回当前「物理名直取 + 推断」（ADR-028）。
...
**(1) 逻辑字段模型（解决需求 1）**：`data.model` 的字段 `name` 是**逻辑标识**，encoding 引用逻辑名（ADR-028 已按逻辑名校验 / 推断）。无映射时逻辑名 = 物理路径（恒等），故「同名同类型」数据源直接生效——`lowerPlots` 换个同形 `datasets[ref]` 即重新出图，spec 与 model 不变。
...
6. **temporal 收口**：扩展现有 `coerceTimestamp`（加 `Date` 分支 + 严格 ISO guard）成统一 temporal coercion，与 ADR-028 推断 guard **同一套正则**，不打架。
...
- **type-driven scale 选型 / 类型↔scale 校验** → ADR-030。
- **远程 / 流式数据源、大数据采样** → 后续。
