...
- 状态：Superseded
- 替代：[ADR-090](./090-axis-guide-style.md) 与 [ADR-092](./092-legend-palette-guide-theme.md)；axis / legend 判别保留，公开 guide 字段已收敛为最终部件槽位
- 决策日期：2026-06-04
- 关联：[plot v0.1-alpha.2 待办](./roadmap.md) · [plot v0.1 roadmap](./roadmap.md) · [plot-design.md §3.9 guide / §3.6 coordinate scope / §7 多坐标 / §14 anchor](../../../../architecture/plot-design.md) · 根节点：[ADR-001 IRPlot](./001-plot-spec-root.md) · 坐标系：[ADR-004 coordinate](./004-plot-coordinate.md) · 消费方：[ADR-012 guide lowering](./012-guide-lowering.md)

> 本 ADR 只定 guide 的 **IR 声明形态**（画什么轴、是否带网格、绑哪个维度），不含刻度怎么算（[ADR-010](./010-d3-scale.md)）、画在哪（[ADR-011](./011-plot-area-layout.md)）、怎么 lower（[ADR-012](./012-guide-lowering.md)）。
...
`guides` 省略 / 为空 = 不画任何 guide（IR 显式所得）。**「默认自动出轴」是 [ADR-013](./013-guide-bindings-dsl.md) 的 DSL builder 行为**，不是 IR / lowering 的隐式默认——保证 vanilla 直写 spec 时显式可控、`bare` 易表达。
...
- **刻度算法（auto-tick）** → [ADR-010](./010-d3-scale.md)；**轴/网格画在哪（plot area / margin）** → [ADR-011](./011-plot-area-layout.md)；**guide → core IR 的 lowering** → [ADR-012](./012-guide-lowering.md)；**`<Axis>` 子组件 / 默认自动出 / `bare`** → [ADR-013](./013-guide-bindings-dsl.md)。
...
- **同维度多轴（dual-axis）+ `placement`（top/bottom/left/right）+ 副轴 `scale?` 绑定** → 后续；当前每维一轴（lowering 拒重复，见 [ADR-012](./012-guide-lowering.md)），多轴是非破坏加可选字段 + 放宽该断言。
