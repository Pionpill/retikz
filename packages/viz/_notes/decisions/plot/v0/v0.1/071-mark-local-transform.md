...
状态：Accepted
决策日期：2026-06-26
关联：[plot v0.1-alpha.12 roadmap](./roadmap.md) · [ADR-062 transform registry](./062-transform-registry.md) · [ADR-064 custom mark registry](./064-mark-custom-registry.md) · [ADR-070 relation derived data](./070-relation-derived-data-routing.md)

## 背景

ADR-070 为 RelationMark 引入了局部 transform，使 relation 可以从 root transform 后的 rows 继续派生只服务自己的数据视图。实现后这个能力不应继续作为 relation 特例存在：point、path、interval、reference 与 custom mark 同样需要“只对当前图层筛选、排序、聚合、派生字段”的数据视图。
