...
ADR-01 已经给 Plot 内部 coordinate scope 建立了 identity 和 mark / axis guide 绑定。ADR-02 在其上增加 facet grid 的数据路由：从一份 IRPlot 数据中按 row / column 字段派生 panel key，给每个 panel 生成局部 rows 和 coordinate scope，再按共享或独立 position scale 策略训练坐标。
