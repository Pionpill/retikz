...
状态：Accepted
决策日期：2026-06-18
关联：[plot v0.1-alpha.12 roadmap](./roadmap.md) · [ADR-059 mark registry](./059-mark-abstraction-registry.md) · [ADR-062 transform registry](./062-transform-registry.md) · [ADR-063 scale registry](./063-scale-registry.md) · [plot-design.md §8.3](../../../../architecture/plot-design.md)

## 背景

coordinate 是 mark、guide、locator 的投影底座。早期实现把 cartesian2D、polar2D、cartesian1D、polar1D、ternary2D 与 custom 分支写死在 coordinate resolution 中，并用多张表维护 roles、required channels 与 guide dimension 规则。
...
coordinate resolution 收敛为 registry 查找：内置坐标系也注册为 `CoordinateDefinition`，自定义坐标系通过 `defineCoordinate` 提供同形 definition。registry 以 schema 中的 `type` literal 提取注册键，内置为底，自定义合并；重复 type 或未注册 type 均 fail-loud。
...
v0.1 stable 的内置坐标集合为 `cartesian2D`、`polar2D`、`cartesian1D` 与 `polar1D`。`ternary2D` 已由 [ADR-105](./105-ternary2d-removal.md) 移除；三角色及其它特殊投影继续通过同一 `CoordinateDefinition` 扩展路径表达。
