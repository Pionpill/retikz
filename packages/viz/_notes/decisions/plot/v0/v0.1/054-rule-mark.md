...
- 状态：Superseded
- 替代：[ADR-059](./059-mark-abstraction-registry.md) 与 [ADR-060](./060-mark-surface-convergence.md)；rule surface 已收敛为 `ReferenceMark`
- 决策日期：2026-06-16
- 关联：[plot v0.1 roadmap](./roadmap.md)「Geometry 基础」 · [alpha.11 roadmap](./roadmap.md) · [plot-design.md §3.7 mark 表 rule / §3.9 guide / §3.10 layer](../../../../architecture/plot-design.md) · 同 milestone [ADR-052 区间几何投影](./052-cell-geometry-projection.md)
...
## 决策：rule 作为 mark，line 下沉 core `Path`、band 复用 ADR-052 `projectCell`；reference-line / band guide 暂不引入
...
- **rule mark = 数据驱动的常量位置参考标注**——值来自 `encoding`（`field` per-datum 每行一条、或 `value` 常量单条），属 **mark 层**，与 point / line / interval 并列参与 layer z-order（§3.10 `threshold rule layer`），可被 `color` 通道编码、可被 scope / anchor 引用。两种形态：
  ...
  - **band**：常量轴给一个 **`[lo,hi]` 区间** → 一片**填充区域**。band 几何 = ADR-052 的正交 cell（primary = 对侧轴 full extent / extent 截断、secondary = 常量轴 `[lo,hi]`），喂 `frame.projectCell(cell)` → cartesian 出 rect、polar 出环带 sector、曲线出 contour，是**可连接 Node**（守 §8.1），**零新增几何**（直接复用 ADR-052）。
...
- **band 坐标系支持面 = 完全继承 ADR-052**：frame 实现了 `projectCell` 才支持（cartesian2D → rect band、polar2D → 环带 sector、出 contour 的曲线 frame → contour band），未实现则 fail-loud，无引擎自动兜底。不为 band 单开坐标系矩阵。
...
2. **按形态分流下沉，零新增几何契约**——line 是 1D 线、无 2D cell 结构，下沉 core `Path`，复用 line mark 的 `pointsToSteps` / `densifyPolarSegments`、坐标系无关性靠 `frame.project` + polar 段采样承接；band 恰好是 ADR-052 的正交 cell，直接喂 `frame.projectCell`，与 interval / rect 共用同一套 cell 几何并产出可连接 Node（守 §8.1）。这把 §3.9「reference line / band」两种形态统一进同一个 rule mark，band 还把 ADR-052 cell 投影复用到「常量轴区间」新场景。故「rule 不走 `projectCell`」不是绝对结论：line 不走、band 走，由有无 `xTo/yTo` 判定。
...
- **1D / ternary / custom 坐标系下的 rule line**：几何语义（对侧轴域 / 满铺定义）未定，fail-loud；band 支持面随 `projectCell`（继承 ADR-052），无 `projectCell` 的坐标系 band 同样 fail-loud；后续按需求驱动单独评估。
- **dash / 线型 / 标签**（rule 上挂文字标注）：样式归 Theme（alpha.15）、标签归 text mark（ADR-055）+ anchor 引用 rule，本 ADR 只画线 / 带。
