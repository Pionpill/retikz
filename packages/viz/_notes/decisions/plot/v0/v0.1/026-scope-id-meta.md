...
- 状态：Accepted
- 决策日期：2026-06-07
- 关联：[plot v0.1-alpha.5 roadmap](./roadmap.md) · [plot-design.md §7 anchor / §8.1 id 绑定与可连接性](../../../../architecture/plot-design.md) · **core 依赖**：[core v0.3-ADR-016 meta provenance](../../../../../../kernel/_notes/decisions/v0/v0.3/016-meta-provenance.md)（已回灌 next-plot）· **预留来源**：[plot v0.1-ADR-001 根节点](./001-plot-spec-root.md)（`id` / `meta` 字段位埋点）
...
2. **meta（不可连接的来源标签）**——core v0.3-alpha.4 [ADR-016](../../../../../../kernel/_notes/decisions/v0/v0.3/016-meta-provenance.md) 新增的 `Node` / `Scope` / `Path` `meta`，compile 原样 stamp 进 Scene 图元、renderer 忽略。下沉元素写 meta ⇒ 交互层命中图元即可读 `prim.meta` 反查来源。零命名空间代价（meta 不进 nodeIndex），是 v3 hit-test 的**主通道**。
...
- **内部 `<plotId>.mark.0` / `.series.north` / `.datum.Q1` **不**自动对外可见**——它们活在 plot 的 local namespace 里。其用途是**三项 plot-local 能力**：① plot 内部连接（同 namespace 内 path step 可引用）；② [ADR-027](./027-datum-locator.md) locator 寻址（locator 是 plot 侧纯函数、不走 core 命名空间查找，故不受 localNamespace 限制）；③ 与 meta 关联的稳定命名。
...
- **datum 级 id**：`LowerPlotsOptions.datumIdField`（数据属性名，opt-in）——把该字段值绑成 `<plotId>.datum.<值>` 的 `Node.id`。**缺字段 / 字段值重复 → fail loud**（抛清晰错误，见决策末）。高基数只需定位 → 不绑 id，走 [ADR-027](./027-datum-locator.md) locator 按需算。
...
`expand.ts` 现把投影帧（cartesian / polar）构造**内联**在 `expandPlot` 里。本 ADR 抽成可复用纯函数 `resolveFrame(spec, rows, options) → { frame, … }`，使 mark 下沉与 [ADR-027](./027-datum-locator.md) locator **共用同一投影**——locator 正向解析的位置必须与 lowering 实际摆放一致，共用 frame 是杜绝两套投影漂移的唯一可靠手段。本 ADR 只做抽取（产物等价），locator 在 ADR-027 消费。
...
3. **frame 单一真源**——locator 与 lowering 共用投影，命中预演的「位置一致」断言才有意义；抽取是 ADR-027 的前置。
...
- **datum locator 正向解析**：[ADR-027](./027-datum-locator.md)（本 ADR 只抽出共用 frame 构造）。
