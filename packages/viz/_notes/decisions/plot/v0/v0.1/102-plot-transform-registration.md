...
ADR-101 让 `@retikz/plot` 从 `@retikz/data` 消费数据模型、字段解析、format、statistics、transform pipeline。随后对 data 内置能力复盘发现，部分 transform 只是 rows in / rows out，但输出字段和语义直接服务 plot mark、scale 或 geometry。把这些 transform 作为 data 内置项，会把 plot 的 stat-geom 词汇强加给 table / geo。
...
data 的默认内置集合已按 [data ADR-002](../../../data/v0/v0.1/002-shared-provider-boundary.md) 收窄，plot-only transform 不再污染 data 包。
