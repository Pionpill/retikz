...
- 状态：Accepted
- 决策日期：2026-06-08
- 关联：[plot v0.1-alpha.8 roadmap](./roadmap.md) · [plot v0 roadmap 阶段二](../roadmap.md) · [plot-design §3.4 Scale / §3.6 Encoding](../../../../architecture/plot-design.md) · 依赖出口：[ADR-039 color·series](./039-color-series.md)（continuous/temporal color 本轮兑现）· 下游：[ADR-043 离散化 scale](./043-discretization-scale.md)（复用本 ADR 的 scheme/range schema）/ [ADR-044 legend](./044-legend-guide.md)
...
- **离散化 scale**（quantize / threshold / quantile）→ [ADR-043](./043-discretization-scale.md)（复用本 ADR scheme/range）。
- **legend**（连续色带 ramp）→ [ADR-044](./044-legend-guide.md)。
