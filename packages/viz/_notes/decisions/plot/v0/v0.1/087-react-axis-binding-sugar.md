...
- 状态：Superseded
- 替代：[ADR-088](./088-composition-api-structure.md) 与 [ADR-104](./104-plot-vanilla-plain-api.md)；axis binding sugar 现由共享 framework-neutral normalization 展开
- 决策日期：2026-06-29
- 关联：[plot v0.1 roadmap](./roadmap.md) · [alpha.14 roadmap](./roadmap.md) · [ADR-080 coordinate composition registry](./080-coordinate-composition-registry.md) · [ADR-082 same-panel multi-axis overlay](./082-same-panel-multi-axis.md) · [ADR-085 scope provenance surface](./085-scope-provenance-surface.md) · [ADR-086 axis-level grid targeting](./086-axis-grid-targeting.md)

## 背景

ADR-080 到 ADR-086 已经把 IRPlot 内部的 `composition`、`coordinateScope`、overlay scope、guide binding 和 grid targeting 串起来。这个模型适合作为 IR 和程序化底层契约，但 React DSL 用户写常见双轴图时仍然要理解并手写：
