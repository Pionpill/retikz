...
- 状态：Proposed
- 日期：2026-08-10
- 关联：[plot v0.2-alpha.1 roadmap](./roadmap.md) · [ADR-001 Plot 主题 token 所有权](./001-chart-layering.md) · [ADR-002 继承 Theme Token Scope](./002-inherited-theme-token-scope.md) · [Plot 可视化完备设计](../../../../architecture/plot-visualization-complete.md) · [通用视觉主题设计](../../../../../../../notes/architecture/visual-theme-design.md)
...
本 ADR 取代早期 ADR-005 的复合 `axis.grid` token、`PlotGridType` 与 Grid 专用 dimension map；ADR-001～002 关于 Plot token 所有权、Core effective Theme 投影、local token 与 native theme cascade 的其余决策继续有效。
