...
- 状态：Accepted
- 决策日期：2026-08-06
- 关联：[plot v0.2-alpha.1 roadmap](./roadmap.md) · [plot v0.2 roadmap](./roadmap.md) · [Plot 可视化完备设计](../../../../architecture/plot-visualization-complete.md) · [Chart 基础设施 ADR-001](../../../chart/v0/v0.1/001-chart-infrastructure.md) · [通用视觉主题设计](../../../../../../../notes/architecture/visual-theme-design.md) · [原子契约与组合设计](../../../../../../../notes/architecture/atomic-contract-design.md)
- Superseded-by：关于 inherited token scope、局部 token 输入名称与 Core namespace context 的冲突边界，已由 [ADR-002：继承 Theme Token Scope 与 Plot owner contribution](./002-inherited-theme-token-scope.md) supersede；关于 Plot static label、`plot.label.*` token 与 `IRPlotTheme.labelText` 的决策，由 [ADR-003：Plot 绘图边界与 Chart presentation 归属](./003-plot-presentation-boundary.md) supersede；关于 `IRPlotTheme.background`、`plot.surface.fill` 名称与整块 Plot surface 范围的决策，由 [ADR-004：Plot background 限定为绘图区背景](./004-plot-area-background.md) supersede；其余 Plot / Chart 主题所有权与消费边界继续有效
- Superseded in part by：[ADR-011：Plot Source 默认片段与 Axis 规则](./011-theme-source-fragments.md) 取代 flat token、native `plotTheme`、token rule 与无消费者 `plot.palette.sector` 的输入契约；Plot owner 与 Chart consumer 边界继续有效
