...
- 状态：Accepted
- 决策日期：2026-08-10
- 关联：[plot v0.2 roadmap](./roadmap.md) · [alpha.1 roadmap](./roadmap.md) · [ADR-005：Axis 主题 Token 作用域规则](./005-axis-theme-token-rules.md) · [ADR-006：Axis grid 包含值域端点](./006-axis-grid-domain-endpoints.md) · [Plot 可视化完备设计](../../../../architecture/plot-visualization-complete.md)
- Superseded in part by：[ADR-011：Plot Source 默认片段与 Axis 规则](./011-theme-source-fragments.md) 把 token 形态迁为 Source-shaped Axis defaults / rules；`grid.includeDomain` 对已有 Axis 的默认语义和显式 guide 优先级继续有效

## 背景与目标

ADR-006 已允许单个 Axis guide 通过 `grid.includeDomain` 保留常规网格，同时保证 effective scale domain 首尾位置具有主网格线。该字段默认关闭，并被限定在 guide 局部结构语义中；Plot Theme 只能决定网格是否默认启用及其线条视觉样式。
...
本决策窄化取代 ADR-005 中“Axis token 只覆盖视觉默认、grid token 不改变 source 语义”的限制，以及 ADR-006 中“Theme 不得提供端点默认”的限制。ADR-005 的统一 Axis rule 机制与 ADR-006 的 guide 字段、端点解析、投影去重和非目标继续有效。
...
- 端点语义：启用后的 source → density → endpoints 顺序、effective domain、有限投影、`bandPosition`、循环坐标去重与 major-over-minor overlap 继续由 ADR-006 决定。
...
- 外部扩展与下游闭环：自定义 Plot style 和 Plot-local Axis rule 使用同一 canonical token；解析后映射为原生 Axis theme，再由既有 guide merge 写入 ADR-006 的 guide 字段，最终沿同一 Plot lowering 输出 Core Path。
...
完整 resolved token map 必须包含该 boolean；自定义 Plot style 可以省略该 token，由默认 preset 与 Axis rule 补全。持久化稀疏 token、Axis rule 与原生 Theme 允许省略字段，但拒绝显式 `undefined`、未知字段和错误类型；runtime style definition 显式返回 `undefined` token 时按省略处理。端点追加、投影去重、density 顺序与 minor overlap 继续完全由 ADR-006 的既有 lowering 决定。
...
- 改变 ADR-006 的端点投影、去重、排序与 minor overlap 规则。
