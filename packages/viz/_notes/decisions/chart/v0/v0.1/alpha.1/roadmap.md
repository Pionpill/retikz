...
| 02  | Chart token 与 Plot token 转发   | Core effective Theme；`chartThemeTokens`；`plotThemeTokens` / `colors` / Plot `plotTheme` 转发；Chart + Plot definition aggregation；canvas surface gate | ADR-01、Plot theme ownership ADR-01～02、Core ADR-13、Standard arbitrary-child surface composite                                   | Proposed / 待人工确认，Plot 主题实现与 surface 底座仍阻塞 |
