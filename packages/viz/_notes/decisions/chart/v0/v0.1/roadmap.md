...
| ------------------ | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| chart v0.1-alpha.1 | **点图 · Scatter & Points** | [alpha.1 roadmap](./alpha.1/roadmap.md)：先建立 ChartSpec / recipe resolution / inspection、三 adapter、Chart-owned token 与 Plot token 转发、presentation + Standard layout / surface，再按 `scatter`、`bubble`、`connected-scatter`、`regression`、`ranged-dot`、`strip` 的顺序逐 type 闭环；Scatter 与 Bubble 是平级 Canonical Type | 前五个可实施 type 从同一 JSON-safe 语义生成完整 PlotSpec；`strip` 等待 Plot position-offset；完整 canvas 等待 Standard arbitrary-child surface composite；Core effective Theme、Chart / Plot token owner、presentation 与 identity 规则确定 | 起草中 |
