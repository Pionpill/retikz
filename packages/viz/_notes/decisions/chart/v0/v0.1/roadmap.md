...
| ------------------ | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| chart v0.1-alpha.1 | **点图 · Scatter & Points** | ChartSpec / recipe resolution / PlotSpec inspection；chart、chart-react、chart-vanilla 三包入口；`scatter`、`bubble`、`connected-scatter`、`regression`、`ranged-dot`、`strip`；有限 style preset 枚举、自定义样式覆盖、自定义颜色数组；Chart-level label / presentation + Plot 的 Standard 布局 | 六个点图 type 从同一 JSON-safe 语义生成完整 PlotSpec；preset、style override 与 palette 规则确定；label 与 Plot 形成单一结果且不丢失 Plot 内部 identity                | 待起草 |
