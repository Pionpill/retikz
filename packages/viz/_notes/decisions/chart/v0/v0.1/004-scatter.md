...
Point recipe properties 顶层可以用 `domainPadding` 调整连续位置 scale 的留白；完整 shape、最大 Point 半径默认、Core spacing 优先级与 range / ratio 单位由 [ADR-014](./014-point-radius-domain-padding.md) 统一规定。该字段属于 recipe scaffold，不进入 Scatter 或其它 authored Chart mark 的 properties
