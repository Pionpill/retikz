...
| [alpha.2](./alpha.2/roadmap.md) | **二维约束布局**   | auto/fraction/minmax、span、border、bounds-aware alignment、fit/overflow                                | 通用 `IRChild` constrained layout               | 设计已接受 |
| alpha.3                         | **呈现语法**       | formatter、presentation、selector/rule、条件视觉 scale、theme、Legend descriptor / Standard Legend 消费 | alpha.1 Cell 与 registry 边界；Standard alpha.3 | 计划中     |
...
- Table 拥有 visual encoding 到 Legend descriptor / Standard Legend 输入的领域解析、theme mapping 与 lineage；通用 Legend 视觉结构、内部布局和 lowering 由 Standard 拥有
