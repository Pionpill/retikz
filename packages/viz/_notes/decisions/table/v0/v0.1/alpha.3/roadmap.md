...
| [04](./04-conditional-visual-encoding-and-scale.md)        | 条件 color scale、visual encoding 与 Legend descriptor | red    | ADR-02～03；Core gradient-stop semantics gate              | Proposed |
| [05](./05-theme-definition-and-style-resolution.md)        | Theme Definition、palette 与最终 style precedence      | red    | ADR-02～04                                                 | Proposed |
| [06](./06-standard-legend-consumption-and-traceability.md) | Standard Legend 消费、外围组合与 artifact lineage      | red    | ADR-04～05；Standard ADR-09 + Core artifact link hard gate | Proposed |
        ...
        └─▶ 03 selector + rule cascade
              ...
              └──────────────┐
                       ...
                       05 theme resolution
                             │
                             ▼
       Standard ADR-09 + Core artifact link gate
                             │
                             ▼
...
ADR-01～03 可以在 Table 内独立形成 presentation 基线；ADR-04 的 ordinal / threshold 映射仍属 Table，continuous 映射必须先消费由独立 Core ADR 冻结的 gradient-stop 求值语义，ADR-05 再在完整 ADR-04 上解析 theme。ADR-06 只有在 Standard Legend schema、definition、layout-aware compile、artifact 与 capability loading 已 Accepted，且 Core replayed-child artifact link 在当前分支可消费后才能进入实现；gate 未满足时不建立 Table-local 临时 Legend，也不预测 child occurrence path。ADR-07 等 ADR-06 闭环后统一完成三包 authoring 与文档。
...
- visual scale 单次 resolution：declarative mapping 同时驱动 Cell appearance 与 Legend descriptor，opaque evaluator 仅实绘且不能 opt-in Legend
- continuous 从 Cell evaluator 到 SVG / Canvas gradient 使用同一 Core gradient-stop 求值语义，不在 Table 保留第二插值真源
...
- [ ] declarative visual mapping 使 appearance 与 Legend descriptor 同源，continuous 共用 Core gradient-stop 语义，opaque 仅实绘，Table 不依赖或特判 Plot
- [ ] Standard Legend 与 Core replayed-child artifact link hard gate 满足；Table 不复制 Legend schema、layout-aware compile 或 artifact
