...
- 决策日期：2026-07-23
- 接受日期：2026-07-26
- 关联：[alpha.2 roadmap](./roadmap.md) · [Core constrained layout gate](./009-core-constrained-layout-gate.md) · [Table 完备设计](../../../../architecture/table-visualization-complete.md) · [alpha.1 固定轨道](./004-table-layout.md)
...
column-first、受约束内容后的 row contribution、span 传播、最终 replay 与 manifest 同源性由 ADR-011 和 ADR-014 集成，不进入本 numeric solver。
...
`columnSize`、`rowSize`、`headerRowSize`、`columns` 与 `rows` 的激活，以及旧三个固定字段的删除，必须与 `resolveTableLayout()`、`layoutTable()`、lowering、manifest 和 adapters 在 ADR-014 同一次原子迁移中完成，不提交半迁移状态。届时 `columns` / `rows` 省略值解析为 `[]`。
...
- span、Cell box、fit / overflow / clip、wrap、border、manifest 与 adapter 接线继续由 ADR-011～015 完成
