...
`contentCenter` 只作为几何产物。ADR-003 presentation 返回局部 `[0,0]` 内容，ADR-005 才把 Presented Cell 包在 translate Scope 中并对齐该中心；layout 本身不接收内容。direct content 作者也遵守同一局部原点合同。该规则只形成 alpha.1 固定轨道的最小闭环，不宣称任意 nested composite 已正确 fit 到 Cell。
