...
alpha.1 当前只有一个顶层 `TableSpecSchema`：`data` 在根上统一声明为 optional，`structure` 是 manual、detail 与 custom 的 union，再由根 schema 的 `superRefine` 约束 detail 必须有 data、manual 不能有 data。运行时可以拒绝非法组合，但 `IRTableSpec` 只能得到宽类型，无法在 TypeScript 和生成的 JSON Schema 中直接表达 `detail => data 必填`、`manual => data 不存在`。

这种宽根 schema 会把结构差异藏在跨字段校验中。LLM、schema registry 和只处理单类表格的作者工具拿到 `TableSpecSchema` 时，需要同时理解所有 structure 分支和额外 refinement；`createDetailTableSpec()` / `createManualTableSpec()` 也只能返回宽泛的 `IRTableSpec`。未来加入 pivot、matrix 后，根 `superRefine` 会继续累积 kind 与根字段的组合规则。
...
## 决策：精确变体 schema 组成统一 TableSpec union

保留 `namespace: 'table'`、`type: 'table'` 和 `structure.kind` 的现有持久化形态。把共享根字段放入内部 `TableSpecBaseSchema`，为每个已实现的基础 structure 定义精确顶层 schema，再由公开 `TableSpecSchema` 组成 canonical union：
...
export const TableSpecSchema = z.union([DetailTableSpecSchema, ManualTableSpecSchema, CustomTableSpecSchema]);
```

`TableSpecBaseSchema` 只用于复用共同字段，不从 table schema barrel 或包根导出。三个变体 schema 与四个派生类型公开：

```ts
type IRDetailTableSpec = z.infer<typeof DetailTableSpecSchema>;
type IRManualTableSpec = z.infer<typeof ManualTableSpecSchema>;
type IRCustomTableSpec = z.infer<typeof CustomTableSpecSchema>;
type IRTableSpec = z.infer<typeof TableSpecSchema>;
```
...
- `TableSpecSchema` 是 Core composite definition、通用 `<Table spec>`、Vanilla embed、持久化读取和通用 pipeline 的唯一 canonical 根 schema
- detail/manual 的 schema 和类型供窄 authoring API、LLM tool schema、schema registry 与单类表格校验直接使用
- `TableSpecSchema` 使用普通 union，不增加第二个根判别字段；三个 schema 分支在运行时通过 `structure.kind` 的 literal 或保留名拒绝规则保持互斥

canonical `IRTableSpec` 不承诺按嵌套 `structure.kind` 自动缩窄外层 spec：TypeScript 不会把嵌套判别传递到父 union，且 custom 的开放 `kind: string` 在类型层与内置值有重叠。只处理单类表格的代码应直接接收对应 `IRXxxTableSpec`，或通过对应 `XxxTableSpecSchema.parse()` / `safeParse()` 取得窄类型。manual 的 `data?: never` 让通用 `IRTableSpec.data` 继续保持 `IRDataReference | undefined`，因此现有通用 pipeline 无需依赖外层自动缩窄。
...
未来新增基础 structure 时，必须在同一改动中新增对应 `XxxTableSpecSchema` / `IRXxxTableSpec` 并加入 `TableSpecSchema`。v0.1 已保留但尚未实现的 pivot、matrix 在拥有精确顶层 schema 前继续被 custom 分支拒绝。group、hierarchy、summary、transpose 是可组合 operation 或 authoring façade，不因场景名称自动成为新的 canonical 根变体。
...
```ts
type DetailTableSpecInput = Omit<
  IRDetailTableSpec,
...
type ManualTableSpecInput = Omit<
  IRManualTableSpec,
  'namespace' | 'type' | 'data' | 'structure'
> & {
  rows: number;
...
const createDetailTableSpec = (input: DetailTableSpecInput): IRDetailTableSpec;
const createManualTableSpec = (input: ManualTableSpecInput): IRManualTableSpec;
```

两个构造函数分别使用自己的精确 schema parse；Vanilla `detailTable()` / `manualTable()` 保留同样的窄返回类型。React `<DetailTable>` / `<ManualTable>` 的运行时行为不变，内部可直接依赖 detail 的必填 `data`；通用 `<Table>` 仍接收 `IRTableSpec`。
...
```ts
const detail: IRDetailTableSpec = {
  namespace: 'table',
  type: 'table',
...
const manual: IRManualTableSpec = {
  namespace: 'table',
  type: 'table',
  structure: {
    kind: 'manual',
...
const persisted: IRTableSpec = TableSpecSchema.parse(JSON.parse(json));
```

LLM 生成明细表时优先提供 `DetailTableSpecSchema`；只有允许生成多种基础表格时才提供 `TableSpecSchema`。持久化内容仍是同一份 JSON，不保存 schema 名称。

## 最终实现

- `@retikz/table` 导出三个精确 schema 与对应 IR 类型，`TableSpecSchema` / `IRTableSpec` 聚合三种变体
- detail / manual authoring helper 与 Vanilla helper 返回精确变体；React 复用相同 normalization，通用入口继续接收 `IRTableSpec`
...
- 新增公开 `DetailTableSpecSchema`、`ManualTableSpecSchema`、`CustomTableSpecSchema` 与对应 IR 类型；helper 返回类型由 `IRTableSpec` 收窄为具体变体
- ⚠️ BREAKING：`IRTableSpec` 从“宽根对象 + structure union”变为三个精确根对象的 union，部分依赖对象结构赋值或泛型变换的 TypeScript 代码可能需要改用具体 variant type；通用 `IRTableSpec.data` 仍为 `IRDataReference | undefined`，JSON 形态不变
- 删除 `TableSpecSchema` 对 detail/manual data 共现关系的根 `superRefine`，由三个对象分支的字段契约承担
- `@retikz/table` pipeline 仍只注册一个 `TableSpecSchema`，Table lowering、manifest 与 Data 公开契约不变
...
- 内部表达链路：精确 Table spec schema → `TableSpecSchema` union → Core `defineComposite()` 提取共同 key → 现有 structure registry / normalize → presentation / layout / lowering
...
- pipeline / lowering 与下游消费：Core 仍只注册 `table.table` + `TableSpecSchema`；union 的所有 object 分支声明同一 key，所有变体进入同一 `normalizeTableStructure()` 与后续阶段
- React / Vanilla adapter 等价性：窄 sugar 使用对应精确 helper，通用入口使用 `IRTableSpec`；两边不拥有另一套 schema
...
- 为每个用户自定义 structure 动态改写 canonical `TableSpecSchema`
...
- `IRTableSpec` 是共享顶层 key 的普通 union，调用方不能依赖 `structure.kind` 自动缩窄外层类型；单类调用方应持有精确变体
