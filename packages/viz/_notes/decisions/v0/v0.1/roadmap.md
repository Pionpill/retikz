...
| v0.1-alpha.15 | **Guide + Theme 主题样式**      | axis 值域弹性与 tick 策略；轴线 / 刻度 / label / grid / legend 外观；调色板（categorical / sequential / diverging）；默认样式 token；series / sector 配色；plot labels 与 layer zIndex。Interaction 顺延到 v0.3 能力线。`theme` 模块（[plot-design §11.1](../../../architecture/plot-design.md)） | [`alpha.15/`](./alpha.15/roadmap.md)（12 ADR 全部 Accepted） |
