...
| Grid       | 二维 Plot area 使用 mode 背景色（Light 白、Dark 黑）的不透明 grid，在 tinted surface 上承担稳定的尺度参照    |
...
- 高色度集中于 marks 和有明确语义的强调；文字、axis、legend 与 surface 使用受控色度，grid 以背景色从 tinted plot surface 中分离。
...
- 第一注意力落在 marks，tinted surface、背景色 grid 和 legend 只建立清晰的屏幕层级。
