...
> **状态：长期架构草案，已确认分层、IR 与扩展边界。** 本文定义 `@retikz/chart` 的长期定位、ChartSpec 语义、默认解析、Plot 混合与 lowering 关系，不冻结具体字段、首批类型清单或版本排期。当前公开契约以后续 Accepted ADR、代码与用户文档为准。
>
...
> Chart 用稳定的 `type` 选择一套完整、隐式的 Grammar-of-Graphics 配方，再把用户写出的差异配置和追加 Plot 内容合并为完整 PlotSpec。
...
Chart 不直接 lower 到 Core，不拥有 renderer，也不建立与 Plot 平行的 Mark、Transform、Scale、Coordinate、Guide、Composition 或交互语义。
...
| Chart 核心          | `@retikz/chart`             | ChartSpec、封闭 Canonical Type 目录、类型配方、默认解析、配方所需的具体 Plot definitions、ChartSpec 到 PlotSpec lowering | 新 GoG 能力轴、renderer、Chart 扩展 registry |
| 通用图形能力        | `@retikz/standard`          | 被 Plot 解析后消费的领域无关 composite、布局与呈现                                                                       | Chart / Plot IR 与类型配方                   |
...
Chart 可以依赖 Data 与 Plot 的公开契约，但不复制它们的 schema、definition、registry、pipeline 或算法。
...
ChartSpec 不是缩减版 PlotSpec。它应沿用 Plot 的结构轴，允许配置 data、transform、encoding、scales、coordinate / composition、mark、guides、theme、layout、labels 和追加 marks，但不要求用户重写 `type` 已经确定的内容。
...
| `coordinate` / `composition`  | 调整类型默认空间与组合；结构性不变量不能被破坏                                     |
| `guides`                      | 调整、关闭、替换或追加表现性 guide                                                 |
| `theme` / `layout` / `labels` | 复用 Plot 的呈现能力，不建立 Chart 平行样式系统                                    |
...
- Guide、Theme、Layout 与 Label 默认值
...
表现性默认负责开箱可读性，例如 guide 可见性、label、theme、palette、spacing 和部分 scale 呈现。它们允许调整、关闭或替换。
...
不支持 `<Plot><Chart /></Plot>`，因为 Chart 需要先选择并展开完整 PlotSpec；把 Chart 作为 Plot 子成员会混淆根数据、默认配方、composition 与 lowering owner。组合多个完整 Chart / Plot 应使用 Core / Standard 的通用外层组合能力，而不是让 Plot 解释 Chart。
...
## 9. Lowering 不得产生第二套语义

`lowerChartSpec` 的职责仅是确定性地生成合法 PlotSpec：

1. 校验 ChartSpec 与 Canonical Type 数据角色及核心配方不变量
2. 选择完整 type recipe
3. 应用用户对隐式成员的稀疏覆盖
4. 追加显式 Plot operations
5. 解析默认优先级并产出完整 PlotSpec
6. 交给 PlotSpec schema 与 Plot lowering 继续处理

Chart lowering 之后不得再通过 adapter、renderer 或隐藏 side channel 修改 Mark、Scale、Guide 或 geometry。相同 ChartSpec、datasets 与 definitions 必须生成等价 PlotSpec，并在 React、Vanilla、SSR 与手写 JSON 入口保持一致。
...
- Plot provenance / locator / lineage 在 Chart lowering 后继续可用
...
三条入口共享同一 ChartSpec 和 `lowerChartSpec`。React children 中出现的 Plot 内容必须有可序列化对应物；Vanilla 不得只能消费展开后的 PlotSpec，React 也不得拥有 ChartSpec 无法表示的额外类型语义。
...
3. 多个隐式同类成员的稳定覆盖定位
4. 表现性默认的关闭 / 替换语法
5. ChartSpec 到 PlotSpec 的 diagnostics、provenance 与 inspection 产物
6. React children、Vanilla builder 与 JSON IR 的精确 parity
7. Chart Pattern 的文档元数据、分组与搜索策略
