...
| Presentation | 将值格式化并呈现为 Core 内容                           |
| Layout       | 求解轨道、span、padding、alignment、border 和 fragment |
...
任意 `IRChild` 的通用测量能力属于 Core；若当前能力不足，应先补 Core 合同，而不是在 Table 内建立私有测量系统。
...
4. Selector、Rule 与样式级联
5. Track sizing、span 和内容测量合同
...
10. 首个 Alpha 的能力范围与实现顺序

这些议题在 ADR 中可以调整或推翻本文的候选方向；本文只提供讨论坐标，不作为实现细节承诺。

## 10. 非目标

- 不成为 DOM data grid 或电子表格引擎
- 不复制 Data 的通用数据算法
