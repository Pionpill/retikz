...
- Chart 根固定使用 `namespace + type + recipe.chartType`：`type` 是 family，`chartType` 是全局唯一 recipe key 并唯一映射到一个 family。根 `coordinate` 是 Chart 的高频坐标选择，operation / Definition / registry / resolve / lowering 仍由 Plot 拥有；`recipe` 保存精确 field-only `encodings`、可选 `properties` 与有序 `marks`，其它显式 Plot 调整进入可选 `plotExtension`。每个 chartType 拥有独立 strict schema；`OpenString` 只开放已注册 key，不建立接受全部 payload 的宽 union。
