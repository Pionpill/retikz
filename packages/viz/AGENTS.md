...
| `@retikz/chart`         | 把 Base 或封闭 Chart 类型解析为完整 IRPlot | Base 与逐类型精确 schema、`plot` / `config` 边界、recipe、统一 bind / resolve、Chart style / presentation、单一 `chart.base` composite               | 公开 Chart union、family 级 schema / resolver、Plot token / preset / resolver、Data / Plot 算法、Core 编译、adapter、renderer  |
| `@retikz/chart-react`   | 用 React authoring 和运行 Chart            | Base `ChartProps`、逐类型精确 props、presentation marker、已绑定 Chart runtime 接线                                                                  | 通用 `type` prop、Chart / Plot 算法、依赖聚合、Core 编译、renderer                                                             |
| `@retikz/chart-vanilla` | 无框架 authoring、SSR 和运行 Chart         | `createChart`、逐类型精确 factory、plain presentation、已绑定 Chart adapter 与 SSR                                                                   | 公开输入 union、Chart / Plot 算法、identity bypass、Core 编译、renderer                                                        |
...
- `@retikz/plot` 是 plot 语义、composition、guide 领域解析、Plot surface / typography / Axis / Legend 视觉 token、palette、style definition / preset / resolver / mapping / inspection 和 lowering 真源；它只拥有可持久化的 `IRPlot*` schema / 类型，不拥有 `InputPlot*` authoring 类型或 normalize。Chart 只转发 Plot 公开 `plotThemeTokens` / `colors` / `plotTheme` 与 `plotThemeStyles`，adapter 不复制 data、scale、coordinate、mark、guide、Theme 或 lowering 算法。完整图表 title / subtitle / caption / note / source / credit 由 Chart presentation 通过 Standard 组合，Plot 只保留 Axis、Legend、Facet、datum、mark、reference 与 annotation 文本。
- `@retikz/chart` 只拥有 `chartThemeTokens` 对应的 Chart canvas、presentation 与 recipe-default token，以及 Chart style definition / resolver；Core effective Theme 是 style / mode 真源，Plot 输入仍由 Plot owner 解析。Chart 与 Plot 以同名 style definition 协作，不共享或复制 token schema。
- Chart 的每个公开 `type` 独立拥有精确 Source schema：Chart-owned 字段在根，Plot-owned 字段在 `plot`，类型独有数据在 `config`。`namespace + type` 只用于选择对应 schema/recipe，所有类型绑定后统一解析为 `IRBaseChart`；不建立公开 `ChartSchema` / `IRChart` union 或 family 级解析主链。
