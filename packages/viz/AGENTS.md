...
| `@retikz/plot`          | 把数据语义映射成可组合的 Core 图形语义 | Plot IR、channel / scale / coordinate / mark / guide 领域解析、Plot theme token / preset / resolver、layout、lowering     | 通用数据算法、Chart presentation、通用 Legend 呈现、Core IR 语义、renderer、框架 authoring      |
...
| `@retikz/chart`         | 把封闭 Chart 类型解析为完整 PlotSpec   | Chart schema fragments、recipe、Chart canvas / presentation / recipe token、Plot token 转发、inspection、逐类型 composite | Plot token / preset / resolver、Data / Plot 算法、Standard layout、Core 编译、adapter、renderer |
