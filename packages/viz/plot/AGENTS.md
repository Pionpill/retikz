...
- Plot Composite 从 Core context 消费 effective Theme；IRPlot 不重复 Core style / mode。省略 `style` 时使用随 `mode` 变化的 Plot 默认视觉基线；显式 `PlotThemeStyleDefinition` 经 Plot registry 解析相对该基线的稀疏覆盖。`plotDefaults`、`plotRules`、colors 与 local guide / mark / scale config 按公开 cascade 覆盖该结果。resolver 接收 Core shared colors 并负责默认 palette，不在后续阶段无条件重写 palette
