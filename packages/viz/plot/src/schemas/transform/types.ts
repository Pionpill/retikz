...
/** 分箱变换（连续分箱，改行数） */
export type IRPlotBinTransform = ZodInfer<typeof BinTransformSchema>;

/** 归一化变换（组内百分比归一化，保行数） */
export type IRPlotNormalizeTransform = ZodInfer<typeof NormalizeTransformSchema>;

/** 区间派生变换（单行派生区间，保行数） */
export type IRPlotDeriveIntervalTransform = ZodInfer<typeof DeriveIntervalTransformSchema>;

/** relate 端点投影（每组选择 source / target 行并映射字段） */
export type IRPlotEndpointProjection = ZodInfer<typeof EndpointProjectionSchema>;

/** 配对度量（从 source / target 行派生差值等字段） */
export type IRPlotPairMeasureOperation = ZodInfer<typeof PairMeasureOperationSchema>;

/** 关系变换（从数据动态派生 relation rows） */
export type IRPlotRelateTransform = ZodInfer<typeof RelateTransformSchema>;

/** 抖点变换（确定性位置抖动，保行数） */
export type IRPlotJitterTransform = ZodInfer<typeof JitterTransformSchema>;

/** density 带宽策略（Silverman 默认或显式正数带宽） */
export type IRPlotDensityBandwidth = ZodInfer<typeof DensityBandwidthSchema>;
...
export type IRPlotBuiltinTransform = ZodInfer<typeof PlotBuiltinTransformSchema>;

/** plot transform operation（data 内置 ∪ plot 内置 ∪ 外部注册 kind 开放配置） */
export type IRPlotTransform = ZodInfer<typeof TransformSchema>;
