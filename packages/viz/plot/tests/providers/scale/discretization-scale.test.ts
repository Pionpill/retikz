...
describe('离散色 · quantize 等宽切档求值（contract）', () => {
  ...
  it('quantize range 覆盖 scheme（取 range 档色）', () => {
    ...
    const fills = nodeFills(
      firstLayer(
        pointSpec({ type: 'quantize', domain: [0, 100], scheme: 'blues', range: ['#111111', '#999999', '#eeeeee'] }),
        { d: data },
      ),
    );
...
describe('离散色 · threshold 阈值断点求值（contract）', () => {
  ...
  it('threshold 断点 [60,80] + 3 色落档', () => {
    ...
    const fills = nodeFills(
      firstLayer(pointSpec({ type: 'threshold', breakpoints: [60, 80], range: ['#e74c3c', '#f1c40f', '#2ecc71'] }), {
        d: data,
      }),
    );
    const lower = fills.map(f => f?.toLowerCase());
...
describe('离散色 · count 与 range 长度冲突 fail-loud（与 threshold 对称，自 adversarial W 提升）', () => {
  ...
  it('quantize count 5 + range 3 色 抛错', () => {
    expect(() =>
      firstLayer(pointSpec({ type: 'quantize', domain: [0, 100], count: 5, range: ['#111', '#888', '#eee'] }), {
        d: data,
      }),
    ).toThrow(/range length/);
  });

  it('quantile count 5 + range 2 色 抛错', () => {
    expect(() => firstLayer(pointSpec({ type: 'quantile', count: 5, range: ['#111', '#eee'] }), { d: data })).toThrow(
      /range length/,
    );
  });
