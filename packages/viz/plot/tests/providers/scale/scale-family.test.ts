...
describe('scale family · log (contract)', () => {
  ...
  it('log_explicit_nonpositive_domain_fails_loud', () => {
    ...
    expect(() => expandOf(lineSpec({ type: 'log', domain: [-1, 100] }), { d: data }, cartOpts)).toThrow(
      /strictly positive/,
    );
  });
});
  ...
  it('radial_drives_polar_rose_radius', () => {
    ...
    const walk = (children: ReadonlyArray<unknown>): void => {
      for (const child of children) {
        const node = child as {
          type?: string;
          shape?: { type?: string; params?: Record<string, number> };
...
describe('scale family · L1 baseline guard (contract)', () => {
  ...
  const areaSpec = (yScale: Record<string, unknown>): IRPlot =>
    PlotSchema.parse({
      ...
      coordinate: { type: 'cartesian2D', x: 'x', y: 'y' },
      marks: [
        { type: 'path', order: 'i', closure: { kind: 'baseline' }, encoding: { x: { field: 'i' }, y: { field: 'v' } } },
      ],
    });
