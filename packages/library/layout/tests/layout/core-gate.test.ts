...
describe('Layout Core layout capability gate', () => {
  ...
  it('distinguishes explicit alignment guides from missing guides without child-type inference', () => {
    ...
    const parent = defineComposite({
      ...
      compile: (_node, context) => {
        const withGuide = context.layoutChild(
          { namespace: 'layout-core-gate', type: 'explicit-guide' },
          NaturalLayoutProposal,
        );
    ...
    compileToScene(sceneOf({ namespace: 'layout-core-gate', type: 'guide-parent' }), {
  ...
  it('keeps custom providers, TeX, nested Scope and selected artifacts in one probe/replay environment', () => {
    ...
    const result = compileToScene(sceneOf({ namespace: 'layout-core-gate', type: 'nested-parent' }), {
  ...
  it('isolates discarded failures, preserves raise ownership and rejects duplicate replay', () => {
    ...
    const discarded = compileToScene(sceneOf({ namespace: 'layout-core-gate', type: 'discard-failure' }), {
    ...
    expect(() =>
      compileToScene(sceneOf({ namespace: 'layout-core-gate', type: 'raise-failure' }), {
        composites: [failure, raise],
      }),
    ).toThrow(/layout-core-gate\.failure.*gate child failure/i);
    expect(() =>
      compileToScene(sceneOf({ namespace: 'layout-core-gate', type: 'duplicate-replay' }), {
  ...
  it('rejects replay results outside their callback and compile session and rejects forged results', () => {
      ...
      compileToScene(
        {
          ...
          type: 'scene',
          children: [
            { namespace: 'layout-core-gate', type: 'result-producer' },
            { namespace: 'layout-core-gate', type: 'result-consumer' },
          ],
        },
    ...
    expect(() =>
      compileToScene(sceneOf({ namespace: 'layout-core-gate', type: 'forged-result' }), {
        composites: [forged],
      }),
  ...
  it('rejects malformed proposals with occurrence-aware diagnostics', () => {
    ...
    for (const fixture of invalidProposals) {
      ...
      expect(() =>
        compileToScene(sceneOf({ namespace: 'layout-core-gate', type: fixture.name }), {
          composites: [invalid],
        }),
      ...
      ).toThrow(new RegExp(`layout-core-gate\\.${fixture.name}.*children\\[0\\].*proposal`, 'i'));
    }
  });
});
