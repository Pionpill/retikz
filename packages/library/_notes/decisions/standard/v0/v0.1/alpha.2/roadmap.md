...
>
> 关联：[Standard v0.1 roadmap](../roadmap.md) · [Standard library design](../../../../../architecture/standard-library-design.md) · [Core layout-aware composite ADR](../../../../../../../kernel/_notes/decisions/v0/v0.5/alpha.1/07-layout-aware-composite.md) · [Table track sizing ADR](../../../../../../../viz/_notes/decisions/table/v0/v0.1/alpha.2/02-track-sizing-schema-and-solver.md)
...
| ADR | 主题                                | 主要决策                                                                                                                                                                                    | 依赖                                                    | 初始状态 |
| --- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | -------- |
| 01  | Box Layout Profile 与 Core Gate     | 冻结 alpha.2 能力矩阵、CSS 差异边界、有限 / 非有限 available space 语义，并以行为 Gate 确认 Core 双轴约束、intrinsic contributions、allocated box、alignment metrics 与 replay wrapper 能力 | alpha.1 capability loading；Core layout-aware composite | 待启动   |
...
Core 已有且 alpha.2 必须直接复用：

- `CompositeDefinition.expand | compile` 互斥分支与唯一 composite registry
- `layoutChild()` 的 intrinsic / `maxWidth` constrained probe
- allocation bounds、保守 visual bounds 与一次性 compile-local replay
- replay placement 的数值 transform、typed artifacts、occurrence locator 与同次 compile 原子提交
- `lowerIRToKernel()` 对 layout-aware composite fail-loud，以及 React / Vanilla 对同一 `CompileResult` 的接线

上述能力足以支撑内容反馈式单轴原型，但不足以宣称 Flex / Grid / Overlay profile 完整。ADR-01 必须以可观察行为判断 Core 是否需要补齐以下通用能力；最终 API、命名和最小字段由独立 Kernel ADR 冻结：

1. **双轴约束**：父 composite 能对任意 child 表达 width / height 两轴的有限 available size，以及必要的 min / max / exact allocation 语义；显式 `0`、字段省略和 unconstrained 必须严格区分
2. **intrinsic contributions**：除当前单一 natural size 外，Flex / Grid 若需要 min-content、max-content 或等价的最小可收缩 / 自然贡献，Core 必须通过同一 host measurer 和 provider 环境返回，Standard 不得估算文本或按 child 类型特判
3. **allocated box 与 stretch**：probe 结果必须区分 child 实际占用与父级分配槽；可伸缩 child 在 exact / bounded slot 内重新布局，固定内容可以拒绝 stretch 并显式报告 overflow，父级不能用 Scene scale 伪造 CSS-style stretch
4. **高度反馈**：给定宽度后的文本换行高度、显式高度约束、交叉轴 stretch 和二维 row contribution 必须形成确定闭环；不能只把 `maxHeight` 加入类型而没有 child 消费语义
5. **alignment metrics**：baseline 或其它共享 alignment guide 若进入 profile，必须来自 Core layout result 的可选通用 metric；只支持 start / center / end 时应在 Standard profile 中明确收缩，不从 Text primitive 或 Node 内部结构反推
6. **replay wrapper**：父 composite 必须能在 replay 外按正确坐标顺序组合 transform、clip 与必要的 Scope 语义，使 item overflow、Overlay clip 和 fit 不要求二次 compile，也不能把 clip 错放进 child-local transform
7. **约束传播与失败诊断**：nested layout-aware composite 收到同一双轴合同；非法 / 非有限约束、不可满足 allocation、布局循环、非局部引用和 replay 误用保持 fail-loud，并保留 provider key 与 occurrence
...
- measure、constrained layout 与最终输出来自同一次 Core compile；Standard 不重复 compile、不从 Scene primitive 反推尺寸，也不保存跨 compile replay
- finite / indefinite available space 下的 grow、fraction、auto-flow、stretch 与 overflow 有确定语义；不能依赖 DOM viewport 或 renderer 回读
- allocation bounds、visual bounds、allocated slot、baseline / guide 与 clip 的选择有明确坐标空间和领域理由
...
- Core Gate tests 使用任意合法 `IRChild`、nested layout-aware composite、文本 / TeX、custom provider、双轴约束、baseline / fallback、clip wrapper 和失败路径，不用 Standard 私有实现证明上游能力
...
- [ ] 任意 `IRChild`、nested layout、文本双轴约束、空 / 单项输入、overflow / clip 和失败路径均有自动化证据
