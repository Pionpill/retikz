...
- 接入按项 capability module、显式 logic-diagram preset、React / Vanilla 等价 authoring 与双语文档 dogfood

## 能力边界

- Standard 拥有局部逻辑角色、headless Block 外壳、显式连接呈现、Callout 布局、适用布局组件的 typed artifact 与 capability loading
...
- Process、Class、Data recipe 只消费公开能力，不成为 Standard schema、module、preset 或 adapter 的前置依赖

## ADR 顺序

| ADR                                     | 主题                    | 主要决策                                                                                | 依赖                                                | 状态     |
| --------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------- | -------- |
| [01](./01-logic-diagram-profile.md)     | Logic Diagram Profile   | 冻结能力归属、identity、共享 target、开放 role、外观与 locator / artifact 公共边界      | alpha.1 capability loading；Core target / composite | Proposed |
...
| [05](./05-capability-and-authoring.md)  | Capability 与 Authoring | 冻结按项 module、logic-diagram preset、React / Vanilla / 直接 IR 等价与内部 recipe 边界 | ADR-01～04；alpha.2 module identity merge           | Proposed |

```text
01 shared profile
├─→ 02 LogicBlockBase ─┐
└─→ 03 logic units ────┼─→ 04 Connector / Callout
                       └─→ 05 capability / adapters
```
...
- 直接 IR、React、Vanilla 与 capability preset 进入同一 schema、definition 与 compile 主链；布局组件 artifact 和 Connector lowered Scene 主体 id 保持跨入口等价
- 内部 Process / Class / Data recipe 不进入 package exports、schema registry 或 capability module
...
- capability / adapter 证据覆盖按项 module、同一 identity 幂等、同名不同 identity 冲突、logic-diagram preset、all preset、直接 IR、React 与 Vanilla 等价
...
- [ ] `LogicBlockBase`、四个基础逻辑单元与 Callout 的 schema、definition、artifact、module 与 factory 形成闭环；Connector 的 schema、definition、同 id Core Path lowering、module 与 factory 形成闭环
