...
- 决策日期：2026-07-30
- 关联：[alpha.2 roadmap](./roadmap.md) · [ADR-006](./006-layout-profile-core-gate.md) · [ADR-007](./007-box-layout-item-vocabulary.md) · [ADR-008](./008-flex-layout.md) · [Core ADR-017](../../../../../../kernel/_notes/decisions/v0/v0.5/017-layout-proposal-probe-contract.md)
- 后继：[Layout ADR-001](../../../layout/v0/v0.1/001-layout-package-family.md) 接管当前 owner；本 ADR 保留 Standard 验证期历史
...
- `GridAutoFlow` 公开 `row | column` const object enum，不进入ADR-007 shared vocabulary
...
所有water-fill、求和、epsilon、freeze、index tie-break与residual receiver复用ADR-006/007的Neumaier与稳定规则。每轮至少冻结一条track或消除deficit。

axis `minimumContribution` 是minimumProfile sizes、gaps与padding的补偿和；`naturalContribution`同理来自naturalProfile。每次`resolveLayoutAxisSize()`都同时接收两者：父`intrinsic.minimum`选择minimum，父`intrinsic.natural`选择natural，content在range/exact下先以natural为候选再按ADR-007 clamp。fixed/fill仍计算两套profile，但只用于artifact/overflow诊断，不改写authored/resolved allocation。
...
- negative free space遵循 ADR-008 的 distribution fallback，不反向压缩 fixed/minimum base
