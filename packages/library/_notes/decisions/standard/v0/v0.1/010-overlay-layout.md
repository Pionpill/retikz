...
- 决策日期：2026-07-30
- 关联：[alpha.2 roadmap](./roadmap.md) · [ADR-006](./006-layout-profile-core-gate.md) · [ADR-007](./007-box-layout-item-vocabulary.md) · [ADR-008](./008-flex-layout.md) · [ADR-009](./009-grid-layout.md)
...
Overlay固定采用物理x→y feedback。`finiteX/YLimit`来源与ADR-008/009相同；include/exclude使用完全相同的probe链，participation只决定结果是否进入container contribution。
...
Overlay同时形成minimum与natural两套content contribution，并把两个有限非负结果一并传给`resolveLayoutAxisSize()`：父`intrinsic.minimum`消费minimum profile，父`intrinsic.natural`消费natural profile，range/exact下由ADR-007从natural候选开始clamp。每个profile只消费proposal矩阵中同名probe解析出的outer slot：
...
4. first/last baseline按下述独立group聚合公共guide；缺失guide使用ADR-007边缘fallback
