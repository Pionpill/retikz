...
ADR-004 在 Core anchor-to-anchor placement 可用后，将 Frame 重构为“border + body Scope + Node-like Title / Description”，并用 `padding` 与 `gap` 分离边框外扩和 header 排列。v0.x 不保留首版 `label` / `border` / 旧 `gap` alias。
...
- 该实现随后由 ADR-004 的最终 Frame 公开契约替换；ADR-003 仅保留为设计演进记录

## 遗留边界

- 最终 Frame 的 header、padding、corner radius、z-index 与保留 id 契约以 ADR-004 为准
