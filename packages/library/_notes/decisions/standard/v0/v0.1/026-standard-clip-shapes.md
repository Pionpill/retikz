...
- 决策日期：2026-08-16
- 关联：[Standard v0.1 roadmap](./roadmap.md) · [alpha.3 roadmap](./roadmap.md) · [Standard 拓展库设计](../../../../architecture/standard-library-design.md) · [Core ADR-030](../../../../../../kernel/_notes/decisions/v0/v0.5/030-extensible-clip-shapes.md) · [ADR-023](./023-core-minimal-builtins-and-standard-provider-entrypoints.md)

## 背景与目标

Standard v0.1 ADR-023 已把 `polygon`、`path`、`compound` 的 Clip operation schema 与 definitions 迁入 `@retikz/standard/clip`，但它们仍只能返回 Core 封闭的 Scene `ClipShape`。`circle`、`ellipse` 也继续由 Core 同时拥有 spec、shape 和实现，因此 Standard 只取得部分输入 API，第三方和官方扩展仍受 Core 内置形状集合限制。

[Core v0.5 ADR-030](../../../../../../kernel/_notes/decisions/v0/v0.5/030-extensible-clip-shapes.md) 建立 `ClipDefinition → ClipShapeDefinition → SceneClipPath` 两级扩展链，并将 Core 默认裁剪收敛为 `rect`。本决策在该底座上让 Standard 完整拥有 `circle`、`ellipse`、`polygon`、`path`、`compound` 的 operation spec、ClipShape、两级 definitions 和静态 providers，同时保持直接 Core、provider contribution、React、Vanilla、SSR 与官方 Tier 2 的显式装配闭环。
...
本决策演进并取代 ADR-023 表格中 Clip 的内置所有权结论：Core 默认内置从 `rect`、`circle`、`ellipse` 收敛为仅 `rect`，Standard 官方扩展从 `polygon`、`path`、`compound` 扩展为五种形状。ADR-023 的能力子入口、无全局注册、显式 provider graph、冲突和跨入口装配原则保持有效。
...
- 失败与诊断：无效 spec 由对应 Standard schema 诊断；缺少 operation/shape provider、重复 kind、非法 shape output、递归 cycle/depth 与非法 canonical path 沿 [Core v0.5 ADR-030](../../../../../../kernel/_notes/decisions/v0/v0.5/030-extensible-clip-shapes.md) 的统一错误边界处理。Standard 不捕获后降级为 `rect`、跳过 clip 或偷偷安装定义
