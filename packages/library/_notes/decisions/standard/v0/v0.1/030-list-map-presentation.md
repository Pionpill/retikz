...
| 字符串值               | 带引号和必要转义的 JSON 字面量，区分空字符串、`"null"` 与 `null` |
...
style / layout 默认仅控制当前层生成的格，嵌套结构使用自身默认。唯一例外是 List 的 `data` 入口在整体 `layout.width: 'content'` 时：该内容宽度策略沿自动生成的 List / Map 数据树传递，让其中自动生成的 List 各格按自身内容定宽；Map 仍按自身列宽规则排布。显式 `items` / `entries` 或 React 组合中嵌入的组件不继承该策略。数字固定宽度、height、padding、gap、direction、overflow、style 与 label 均不递归传递。字体、textColor、currentColor、theme / defaults 沿 Core 继承，外层 opacity、变换和 clip 作用于子树而不重复施加。父格固定宽高裁切整个嵌套图，不将尺寸或外观递归复制到后代。

默认完整展开，不提供深度限制、折叠、路径样式、自定义 formatter 或编辑功能。局部展示先投影数据；逐格样式、id、drawable 使用显式结构，也可在其中嵌套带 data 的组件。
...
| `layout`        | width / height 为非负有限数或 auto；List 的 width 另可为 content；padding / overflow 复用 Surface；gap 仅属于整体 |
...
| List 排列与索引 | direction 默认 row，可为 column，不折行；index 默认 false，对象配置位置、起点与文本样式，见 ADR-033               |
| Map 间距        | gap 接受数字或 `{ row, column }`                                                                                  |

覆盖顺序为 **结构默认 → 整体 → Map 角色 → 单格**。只合并显式字段再补默认；font 按字段合并，padding 对象整体替换，width / height 独立继承，auto 解除该轴固定尺寸。List 的 `width: 'content'` 让当前格按自身内容自然宽度加 padding 定宽，可在整体或单格设置；单格优先。只有整体设置与 `data` 输入组合时才向自动生成的嵌套 List 传递，单格设置不改变显式嵌套组件。Map 不接受该值。0、none 保留，覆盖不改写 content Source。
...
List 的 `auto` 轴取各格有效需求最大值；`width: 'content'` 的格只取自身自然宽度加 padding，不拉伸到该最大值。Map 自适应列宽取列最大需求，行高取本行最大需求。固定轴以指定尺寸参与计算，较小格从轨道起点放置而不拉伸。内容在 padding 后区域居中，padding 超出尺寸时居中点限制在格内；固定格保留自然内容并裁切，不缩放或自动折行。

List 索引按 index.position 放在横排上下侧或竖排左右侧，距离为 gap；文字样式与继承规则见 ADR-033。索引参与整体需求，不扩大单格 allocation，不登记身份，隐藏后不保留空间。
...
未知配置、非 JSON Source、错误 data 根类型、入口混用、非法 content、负或非有限尺寸 / gap / padding、非法 index 配置、重复格 id 在 Source 边界失败。缺失 / 冲突定义、测量、proposal 与引用失败保留下层原因和位置，不返回部分结构。
...
List / Map 提供与 Node 同形的可选 `label`，接受单个标签或标签数组，完整复用 Core Node label 的位置、间距、内外侧、文字、旋转及引线契约。省略样式时使用 Core Node label 的默认值，不设置 Standard 专属字号、颜色或间距。`defaults.label`、主题与显式标签样式沿 Core 规则解析；容器共用 style 的字体、文字颜色和主色作为标签宿主外观，角色与单元格覆盖不影响容器标签。

标签附着于容器 allocation 矩形；List 开启索引时该矩形包含索引条。标签不参与单元格尺寸、容器 allocation 或命名连接边界计算，但参与最终可见范围。空容器也可附着标签，标签不生成额外 id。标签宿主不继承后代 Node 的几何与外观默认，容器尺寸与标签外观分别由容器布局、容器共用文字样式及 label 通道控制。整体变换、裁切与命名隔离继续沿根 Scope 语义；单格裁切不裁切容器标签。

React、Vanilla、直接 IR 和 data / 显式单元格入口表达同一契约；data 不把根 label 隐式传播给嵌套容器。复杂标题内容仍使用外部布局组合。

## 实现摘要

List 已在整体与单格宽度配置中支持 `content`，直接 IR、React、Vanilla 与 data 入口共用同一排布语义；data 自动生成的嵌套 List 沿用整体内容宽度策略，手写嵌套组件保持独立。原有 `auto` 及 Map 宽度契约保持不变。当前无已知契约缺口。
