...
| [alpha.3](./alpha.3/roadmap.md) | **语义逻辑图组件**                                       | headless `LogicBlockBase`、`Terminal`、`Stage`、`Decision`、`Junction`、`Connector`、`Callout`、布局 artifact / Scene identity、按项 capability、React / Vanilla authoring 与 docs recipe                                              | alpha.1 loading；alpha.2 Box Layout；Core composite-owned subtarget / Path host label / target-aware replay；语义不由 shape、颜色或坐标代替                |
...
- Process、Class 与 Data Block 只作为 docs 内部 recipe 组合 `LogicBlockBase`，不进入 package exports、Standard IR discriminator、schema registry 或 capability module
...
- 每个 Standard composite 各自拥有 JSON-safe schema、composite definition 与 capability module。轻量、无布局能力通过 expand 输出公开 Core `IRChild` contribution；需要测量或 artifact 的能力通过 Core layout-aware compile 输出 Core children 与 typed artifact。两条分支都不得构造平行 Standard Scene 或 renderer descriptor
...
- `standard`：Grid、Axes、Frame、Flex、GridLayout、Overlay、Legend 与 Logic Diagram 组件的 schema、JSON round-trip、输入错误、identity / Target、layout artifact、route lowering、composite compile 与 bundle 合并不变量
