...
- 每个 Standard composite 各自拥有 JSON-safe schema、composite definition 与 capability module。无上下文能力通过 expand 输出公开 Core `IRChild` contribution；需要测量或 artifact 的能力通过 Core layout-aware compile 输出 Core children 与 typed artifact。两条分支都不得构造平行 Standard Scene 或 renderer descriptor
