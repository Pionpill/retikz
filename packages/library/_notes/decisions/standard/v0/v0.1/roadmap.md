...
6. 建立可由直接作者、Plot 与 Table 复用的通用 Legend 呈现；领域包保留 scale、visual encoding、formatter、provenance 与交互解析，Standard 统一 schema、布局、lowering 与领域无关 artifact
...
- **Legend 边界**：Standard 拥有已经解析好的 title、entries、swatch / ramp / symbol、布局与 artifact；Plot/Table 拥有 channel / scale、visual encoding、formatter、theme mapping、领域 provenance 与交互意图
...
| [alpha.3](./alpha.3/roadmap.md) | **通用 Legend**                                          | 领域无关 Legend schema、definition、Box Layout、lowering、artifact、capability loading、React / Vanilla authoring，以及 Plot 迁移与 Table 首次消费                                                                                     | alpha.2 Box Layout；Plot/Table 领域解析边界；传递 capability loading 与 provenance / locator Gate                                                          |
...
- **领域包依赖**：Plot/Table 可以把所需 Standard module 组合进自己的 compile / bundle 入口；传递引入与调用方直接引入同一 module 的幂等或冲突语义由消费该能力的 ADR 冻结，不使用全局自动注册
...
- Legend 以领域无关 Standard 输入保存 title、entries、swatch / ramp / symbol 与呈现布局；Plot/Table 只把各自领域解析结果转换为该输入，不把 channel、scale、Cell selector、formatter 函数或 interaction state 注入 Standard schema
