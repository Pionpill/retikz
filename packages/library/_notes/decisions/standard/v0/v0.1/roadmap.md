...
9. alpha.4 根据直接作者与 Graph 的真实消费补齐参数化梯形、平行四边形、长六边形、圆柱、椭圆端胶囊以及 bar / crow-foot / Kite / Square 端点 Marker与 open 变体，继续复用 Core Shape / Arrow host 与按需 provider 装配
...
| [alpha.4](./alpha.4/roadmap.md) | **参数化图式 Shape 与端点 Marker**                    | 梯形、平行四边形、长六边形、圆柱、椭圆端胶囊 Shape，bar / crow-foot / Kite / Square Marker与 open 变体，TikZ 扁菱形，Graph 等 Tier 2 的按需 provider consumption                                        | ADR-01 Accepted；Definition/provider、测试、双语文档与真实页面闭环                                                                                       |
