...
/** Standard 发布包的隔离 consumer 三态结果 */
export type StandardPackageBoundaryResult = Readonly<{
...
const createConsumer = (
  ...
  const root = mkdtempSync(join(tmpdir(), 'retikz-standard-consumer-'));
...
export const runStandardPackageBoundary = (
  packageRoot: string,
  packageName: string,
  call: string,
): StandardPackageBoundaryResult => {
  const dist = join(packageRoot, 'dist');
