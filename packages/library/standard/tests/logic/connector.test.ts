...
const connector = (input: Parameters<typeof Standard.createConnector>[0]): IRChild => Standard.createConnector(input);
...
describe('Connector Core Path lowering contract', () => {
  ...
  it('registers as an independent lightweight Definition without a typed artifact', () => {
    ...
    expect(definition).toMatchObject({ namespace: 'standard', type: 'connector' });
  ...
  it('stamps the Connector id on exactly one outer Path primitive and emits no typed artifact', () => {
    ...
    expect(
      artifacts.filter(
        artifact => artifact.kind === 'composite' && artifact.namespace === 'standard' && artifact.type === 'connector',
      ),
    ).toHaveLength(0);
  });

  it('keeps connector lowering independent from other Standard Definitions', () => {
  ...
  it('uses Core registration diagnostics when the Connector Definition is omitted', () => {
    ...
    expect(() => lowerIRToKernel(sceneOf([value]), { composites: [] })).toThrow(/standard\.connector.*not registered/i);
  });
});
...
describe('Connector target diagnostics', () => {
  ...
  it('delegates a self target with an explicit offset to Core without Standard fallback', () => {
