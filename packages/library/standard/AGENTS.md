...
- **拥有的契约**：通用 Tier 2 composite schema / contract / pipeline、factory、任意 child Surface presentation，以及动画效果纯工厂
...
- 单一任意 `IRChild` 的 background / padding / border / corner radius / overflow 包装属于 `composites/presentation/surface/`；不得扩张 Frame 或在 Chart / Table adapter 复制 Surface
...
- package exports 为根入口与 Tier 2 `/shape`；可选 Core provider 归 `@retikz/extension`，通过其根入口消费，不转手导出
