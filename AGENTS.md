...
- 同一轮评审必须冻结同一快照并并发调度，优先使用 2–3 个不同于主 agent 的实际可用模型；调度能力支持时 subagent 按风险使用 `reasoning_effort: high` 至 `max`，并记录实际值。不得用主 agent 同模型 subagent 填位，不得串行喂结论，不得硬编码或伪造工具未暴露的模型。至少两个不同外部模型实际完成才称为交叉评审；Beta 三能力域审计应最大化模型多样性，可用模型不足时允许跨独立能力域复用，但必须如实记录。
