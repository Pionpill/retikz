...
`<emoji> <scope>: <简短描述>`：emoji 用 [gitmoji](https://gitmoji.dev/) 的 `:slug:` 或对应 Unicode 均可；`scope` 是受影响的**分组 / 包名前缀**（见下）；描述用中文、一般 ≤ 50 字。示例：

```
:sparkles: react: 添加 connectAs 连接面 prop
:bug: core: 修复 star shape 锚点偏移
♻️ core: anchor cache 细粒度拆分
📝 plot: alpha.4 mark×coordinate 路线落定
```
...
- **版本号不进 message**：版本是 tag 的事，`git log <旧tag>..<新tag>` 即可按版本筛，每条都写是冗余噪声
- **ADR 引用放正文 body，不挤进 subject**（源码 / 注释 / describe 里禁写 ADR，commit body 是它该待的地方）：
  ...
  subject 保持干净可读；要追 ADR 的人看 body 或 `notes/decisions/` 索引
