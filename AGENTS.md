...
- 规范沉淀检查是每次任务收尾的必做项，不限于问答。执行过程中发现可复用的约束、用户偏好、流程或操作模式，或实际采用了与现有规范 / skill 不同的命令、参数、步骤时，必须记录候选项，并在完成当前任务后检查是否值得抽象或修正规范。
- 有值得沉淀的候选项时，必须在最终回复中简述发现、与现有规则的差异及建议写入的 `AGENTS.md` / skill，并明确询问用户是否写入；不得只报告任务完成而省略询问。仅提出已执行验证、具有复用价值的内容，不将一次性变通直接固化；未经用户明确同意不得修改规范 / skill。没有候选项时无需额外汇报。
