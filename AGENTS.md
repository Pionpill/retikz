...
- **枚举字面量联合用 `as const` 对象 + 派生类型，不用 TS `enum`**
  ...
  - **判别字段（discriminated union 的 `type` / `kind`）同样用 `as const` 对象 + 派生类型**，**但因 `z.discriminatedUnion` 不接受 `z.nativeEnum`**，成员里写 `z.literal(X.member)`（非 `z.nativeEnum`）。模式：`const SCALE_TYPES = { linear: 'linear' } as const` → 成员 `type: z.literal(SCALE_TYPES.linear)`、union `z.discriminatedUnion('type', [LinearScaleSchema, …])`、派生 `type ScaleType = ValueOf<typeof SCALE_TYPES>`。这类「可枚举的判别字段」很多——坐标系（cartesian2D / polar2D…）、mark 类型（point / line / bar…）、scale 映射（linear / log / ordinal…）等都按此办，好处是类型名单一真源 + 可复用的联合类型（lowering 的 switch / 推断都能 import）。
  - core 现存判别字段多为裸 `z.literal('node')` / `z.literal('step')` 等（遗留，写在 const 规则确立前）：**新代码一律按上条 const 形式**，重构旧文件时顺手统一，不强制立即全仓改。
