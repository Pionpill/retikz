...
`<emoji> <scope>: <改动内容>`：emoji 用 [gitmoji](https://gitmoji.dev/) 的 `:slug:` 或对应 Unicode 均可；`scope` 是受影响的**分组 / 包名前缀**（见下）；subject 只写这次真正改变了什么，描述用中文、一般 ≤ 50 字。示例：

```
:sparkles: react: 添加 connectAs 连接面 prop
:bug: core: 修复 star shape 锚点偏移
♻️ core: anchor cache 细粒度拆分
📝 docs: 补充 core 快速开始学习路径
```

emoji 已占了"改动性质"这一维（✨=feat、🐛=fix、📝=docs…），所以**不写 Conventional Commits 的 `feat` / `fix` 关键字**，只用 `<emoji> <scope>:` 的组合。

**subject 禁止塞索引信息**：不写 `alpha.1` / `beta` / `v0.3` / `ADR-xx` / “按 ADR 实现”这类阶段、版本或来源标签。它们在很多 commit 里都会重复，放在第一行无法帮人快速定位问题。第一行服务 `git log --oneline`，应该像一句可搜索的变更摘要。

需要细节时写 body：说明为什么改、行为 / API / 文件边界、兼容性影响、测试与文档同步情况。body 用短段落或短列表，避免把实现流水账搬进 commit。
...
Node / Path / Scope 的事件回调现在接收第二参 context，
用于读取命中图元信息并控制当前 id 的动画。

- onClick 等 handler 保持旧 `(event) =>` 写法兼容
- context.animation 提供 restart / play / pause / seek
...
scope 是冒号前的分组前缀，让 `git log` 一眼看出改的哪块。取值就是**包名 / 分组名**（不带 `@retikz/` 前缀）：

| scope | 对应 |
| --- | --- |
| `core` | `@retikz/core`（IR / 编译器） |
| `render` | `@retikz/render`（SVG / Canvas 后端） |
| `react` | `@retikz/react`（Kernel + Sugar） |
| `vanilla` | `@retikz/vanilla` |
| `plot` | `@retikz/plot`（Tier 2） |
| `docs` | `apps/docs` 文档站 |

- **跨整个基础设施分组**（core/render/react/vanilla 联动改）可用 `core:` 作分组名，或拆成多条按包提交（优先后者，见上文多块 staging）
- **纯工程 / 仓库级改动**（行尾、根 eslint、CI、`.gitignore` 等无单一归属）**可省略 scope**：`🔧 统一行尾为 LF`、`📝 补充 commit 规范`
- **代码 + 同改动集文档**算同一 scope（文档是该改动的一部分），按代码所在包写，不拆 `docs:`

#### 版本与 ADR

- **版本号 / ADR 不进 subject**：版本、ADR、模块、包都是追溯信息，不是变更摘要。版本是 tag / changelog 的事，ADR 是设计来源；放进 subject 会让 `git log --oneline` 充满重复前缀。
- **追溯信息放 footer 最后一行**，统一用一行 `Refs:` trailer，字段顺序固定，缺失写 `-`：
  ...
  - 补“如何学习”分组说明
  - 增加 AI 面板 BYOK / 静态转发说明
  - 快速开始入口改为 4 步交互示例

  Refs: module=core; packages=apps/docs; version=v0.3; adr=-
  ```

  多包用逗号连接：`packages=@retikz/core,@retikz/react,apps/docs`。纯仓库工程改动无模块时写 `module=-`。release / tag commit 是例外，可以在 subject 写版本：`🔖 core: 发布 v0.3.0`。
...
- **枚举字面量联合用 `as const` 对象 + 派生类型，不用 TS `enum`**
  ...
  - 模式（值枚举：某字段取值集）：`const X = { a: 'a' } as const` → `type T = ValueOf<typeof X>`；schema 用 `z.nativeEnum(X)`。已有用例：`ARROW_SHAPES`（`packages/core/core/src/ir/path/arrow.ts`）、`DrawWay`（`packages/core/core/src/parsers/parseWay.ts`）
  - **判别字段（discriminated union 的 `type` / `kind`）同样用 `as const` 对象 + 派生类型**，**但因 `z.discriminatedUnion` 不接受 `z.nativeEnum`**，成员里写 `z.literal(X.Member)`（非 `z.nativeEnum`）。
  - **命名走 `DrawWay` 风格**（`packages/core/core/src/parsers/parseWay.ts`）：常量用 **PascalCase 域前缀名 + PascalCase 成员**、**`export` 暴露给用户**（IDE 补全 + 单一真源），不用 `SCREAMING_SNAKE`。成员**值保持干净的判别串**（不像 DrawWay 那种刻意写丑的哨兵串），这样**裸字面量仍是第一形态**——JSON / IR / LLM 生成写 `{ type: 'point' }`，手写 TS 的人可选 `PlotMark.Point` 拿补全，两者等价。派生类型与常量**不同名**（避开碰撞）：
    ```ts
    /** mark 类型关键字（暴露给用户；值即判别串，裸 'point' 同样可用） */
    export const PlotMark = { Point: 'point', Line: 'line' } as const;
    export type MarkType = ValueOf<typeof PlotMark>;          // 'point' | 'line'
    // 成员：type: z.literal(PlotMark.Point)；union：z.discriminatedUnion('type', [...])
    ```
    已落地：`@retikz/plot` 的 `PlotMark` / `PlotScale` / `PlotCoordinate` / `PlotFieldType` / `PlotComposite`（派生 `MarkType` / `ScaleType` / `CoordinateType` / `FieldType` / `PlotNodeType`）。坐标系 / mark / scale 映射等「可枚举判别字段」都按此办，好处是单一真源 + 可复用联合类型（lowering 的 switch / 推断都能 import）。
  - 旧的 `SCREAMING_SNAKE`（如 `ARROW_SHAPES` / `NODE_SHAPES`）与 core 裸 `z.literal('node')` / `z.literal('step')` 是**遗留写法**（写在本规则确立前）：**新代码一律按上面 `DrawWay` 风格**，重构旧文件时顺手统一，不强制立即全仓改。
